...
# ADR-006: `service` 為唯一業務契約,CLI 採內嵌/遠端雙模式
...
**收緊(service-and-interfaces/F003)**:「明確加旗標並顯示警告」在實作時被收緊為**綁非 loopback 時強制要求
...
- 內嵌模式與遠端模式的並發語意不同(內嵌是單行程,遠端可能多客戶端)。樂觀鎖(ADR-003 的
