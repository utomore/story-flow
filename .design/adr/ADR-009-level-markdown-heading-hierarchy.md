...
# ADR-009: Level 檔以 Markdown 標題階層表達樹結構
...
ADR-002 決定 Markdown 是真相來源,ADR-004 決定 Level 是嚴格樹。但兩者交會處有一塊空白:
...
- **一個 Node 一個檔案**:定位最直接。但一個場景會有數十個 Node,檔案數量爆炸,而且
  「一份檔案看完整個場景」這個核心寫作體驗完全消失——這與 ADR-002 否決「一片段一檔」
  的理由相同。
...
- 搬動子樹時要重新調整一整段的標題層級,純文字編輯器沒有自動化;S2 的 CLI 應提供
  `level move` 指令代勞
