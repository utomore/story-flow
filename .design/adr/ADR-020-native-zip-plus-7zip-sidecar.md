...
> **P0 搬入**(2026-08-23):原 assetdb `ADR-005`,依主架構「與既有文檔的關係」原樣搬入為 `ADR-020`,
> 純技術決策不受合併影響。內文保留當時的套件與模組名稱(`assetdb-*` / `AssetDB.*`),
> 對應到 aapms 的位置見 `system.md`「子系統劃分」。

# ADR-020: ZIP 用原生 Haskell 處理,RAR/7z 外包給 7-Zip Sidecar 行程
