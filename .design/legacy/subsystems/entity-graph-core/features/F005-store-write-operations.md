...
related-adr: [ADR-002, ADR-003, ADR-005, ADR-009, ADR-010]
related-feature: []
---
...
F004 完成後,`storyflow-store` 的**寫入面只有兩個函式**:`writeEntityMeta`(改既有節的
...
的事全部收在 `store` 一層(這是本文檔 的前提)。

同時補掉 system.md「已知缺口」記的那個洞:檔案層主體(frontmatter)的 meta 改不動,
`writeEntityMeta` 對它一律回 `FrontmatterWriteUnsupported`。

驗收標準:

1. 從一個空 Vault 出發,只呼叫本文檔 新增的函式,能建出 system.md 範例裡的
   `characters/琳達.md`(主體 + 兩個片段,含 `links`)與 `levels/教室.md`(六個 Node 的樹),
   且解析回來的結果與範例等價
2. 每一個寫入函式走完後,索引與檔案一致——`rebuildIndex` 之後的查詢結果與寫入當下相同
3. 未被修改的區塊逐字不變(ADR-010):對一份含 YAML 註解、混合行尾、非標準縮排的檔案做
   任一種寫入,只有目標區塊的位元組改變
4. 樂觀鎖在所有會改動既有實體的路徑上生效,`StaleRevision` 時**一個位元組都不寫**
5. `FrontmatterWriteUnsupported` 建構子從 `StoreError` 消失,system.md 的「已知缺口」
   一節刪除

明確**不做**的:Node 的移動與重排(`moveNode` / `reorderNode`)。標題階層即樹(ADR-009),
作者直接改檔案的標題層級就是移動,工具層先不介入。跨檔案的交易(刪 Entity 時順手清掉別的
檔案裡指向它的 link)也不做——見「刪除策略」。

## 相依性

`depends-on: [F002, F003, F004]`,三者**都已 done**,沒有等待對象。

- **F002**(`storyflow-core`):`Meta` / `Entity` / `Level` / `Node` / `Link` / `Id` 型別
  與 `TypeRegistry`。本文檔 要**擴充** `EntityTypeSpec`(加 `dir` / `owner_type`),因此不只
  是使用,還會回頭改 F002 交付的模組
- **F003**(`storyflow-md`):`Document` / `Section` 與既有的節層編輯純函式。本文檔 要
  **擴充** md 的編輯面(frontmatter 序列化與改寫、正文改寫、新檔產生)
- **F004**(`storyflow-store`):`Vault` 定位、索引維護、原子寫入、樂觀鎖模式。本文檔
  的每一個新函式都是「先寫檔、再更新索引」這條既有紀律的複製品

`storyflow-types`(F002 的一部分)的 TOML 載入器要跟著新欄位改,`types/registry/*.toml`
五份檔案要補欄位。

**可否平行開發**:本文檔 動到 core / types / md / store 四個套件,是 P2 的地基。
service-and-interfaces/F001(`service`)在介面約定確定前無法開工,**必須等本文檔 完成**。與本文檔 平行可做的
只有不碰寫入的東西(例如 P3 的 servant 型別草稿),實務上建議序列進行。

## 對應的 Level 2 契約

補齊 `design.md`「對外契約」落地操作組的寫入半邊:`createEntityFile` 與 `writeEntityPatch`
的完整能力,加上節與 Node 的新增/刪除,對應資料流管線寫入方向的全段。不新增查詢介面,
也不觸及 `service-and-interfaces` 的任何契約。
...
節層編輯是完整的。缺的是三件事,全部加在 `StoryFlow.Md.Render`,理由與 ADR-010 一致:
...
「開頭界線的行尾字元起算、到結尾界線的 `---` 之前為止」(F003 實作備註 4)。
...
註解會被抹掉;節層的位元組保留不受影響(那才是 ADR-010 真正在保護的東西,因為片段是
被工具高頻改寫的那一種)。frontmatter YAML 壞掉時回 `Left`,**不覆蓋**。

> **與 system.md 的偏差**:「已知缺口」一節開的簽名是
...
> 空值」混成同一件事。此偏差需開發者確認後同步更新 system.md。
...
`Level` 檔沒有 `Meta` 以外的檔案層欄位需要序列化——`lvlRoot` 由標題階層推導(ADR-009),
...
仍有毫秒級窗口,與 F004 已接受的競態同一種,不另外加鎖。
...
4. `mkSection docEnding 2 newId title (Just override) body`——片段一律 `##`(二級);
   system.md 的規則是「第一個帶 `{#id}` 的標題才開始分節」,Entity 檔的節之間沒有階層
...
節層 `MetaOverride` 只寫**與檔案層不同**的欄位,其餘留 `Nothing` 讓繼承生效(F003 的
...
`addNode` 的 `Id` 是**父 Node**。ADR-009 的規則反過來用:新節的標題層級 = 父節點層級 + 1,
插入位置 = 父節點子樹的**最後一節之後**。「子樹的最後一節」= 從父節點往後掃,直到遇到
`secLevel <= 父層級` 的節為止,取前一節——這是 `insertSection` 的註解已經指出的用法
(「之後」是該節本身之後,不是它整棵子樹之後)。

限制檢查:新層級 > 6 時回 `NodeDepthExceeded`(Markdown 只有六級標題,system.md 已載明
此限制與繞道方式)。
...
|---|---|---|---|
| `data Meta = Meta { metaId :: Id, metaVault :: Text, metaType :: Text, metaTitle :: Text, metaSummary :: Text, metaTags :: [Text], metaStatus :: Status, metaTimeline :: Timeline, metaAliases :: [Text], metaLinks :: [Link], metaSource :: Source, metaRevision :: Int, metaCreated :: Day, metaUpdated :: Day }` | `core/src/StoryFlow/Core/Meta.hs` | F002 | `renderFrontmatter` 的輸入;新建實體時組出來 |
| `bumpRevision :: Day -> Meta -> Meta` | `core/src/StoryFlow/Core/Meta.hs` | F002 | 主體 revision +1 與 `updated` 改今天,不自己寫一份 |
| `metaFieldNames :: [Text]` | `core/src/StoryFlow/Core/Meta.hs` | F002 | `frontmatterFieldOrder` 的欄位名以它為準,拼錯即編譯期外的靜默錯誤 |
| `data Entity = Entity { entMeta :: Meta, entBody :: Text }` | `core/src/StoryFlow/Core/Entity.hs` | F002 | `parseEntityFile` 的產物,取 `entMeta` 比對 revision |
| `data Level = Level { lvlMeta :: Meta, lvlRoot :: Id }` | `core/src/StoryFlow/Core/Level.hs` | F002 | 建 Level 檔時確認根 Node 存在 |
| `data Node = Node { nodMeta :: Meta, nodLevel :: Id, nodParent :: Maybe Id, nodOrder :: Int, nodKind :: NodeKind, nodEntities :: [Ref] }` | `core/src/StoryFlow/Core/Level.hs` | F002 | `addNode` / `removeNode` 之後驗證樹的輸入 |
| `renderNodeKind :: NodeKind -> Text` | `core/src/StoryFlow/Core/Level.hs` | F002 | 新 Node 的 `kind` 欄位序列化(經由 `renderMetaBlock`) |
| `data Link = Link { linkKind :: LinkKind, linkTarget :: Ref, linkNote :: Maybe Text }` | `core/src/StoryFlow/Core/Link.hs` | F002 | `addEntityLink` / `removeEntityLink` 的操作對象 |
| `data IdPrefix = PEnt \| PLvl \| PNod \| PVlt` | `core/src/StoryFlow/Core/Id.hs` | F002 | `allocateId` 的前綴:Entity 用 `PEnt`、Level 用 `PLvl`、Node 用 `PNod` |
| `mkId :: IdPrefix -> Text -> UTCTime -> Int -> Id` | `core/src/StoryFlow/Core/Id.hs` | F002 | 經 `allocateId` 間接使用,不直接呼叫 |
| `renderId :: Id -> Text` | `core/src/StoryFlow/Core/Id.hs` | F002 | 檔名全部被替換掉時的退路;frontmatter 的 `id` 欄位 |
| `data Ref = Ref { refVault :: Maybe Text, refId :: Id }` | `core/src/StoryFlow/Core/Id.hs` | F002 | `linksTo` 的參數;`removeEntityLink` 的配對鍵 |
| `localRef :: Id -> Ref` | `core/src/StoryFlow/Core/Id.hs` | F002 | 刪除前查誰指向我,本 Vault 內定址 |
| `data EntityTypeSpec = EntityTypeSpec { etsKey :: Text, etsName :: Text, etsFields :: [FieldSpec], etsAllowedLinks :: [LinkKind], etsStages :: [Text] }` | `core/src/StoryFlow/Core/Registry.hs` | F002 | **要擴充**:加 `etsDir` / `etsOwnerType` |
| `lookupType :: Text -> TypeRegistry -> Maybe EntityTypeSpec` | `core/src/StoryFlow/Core/Registry.hs` | F002 | `lookupDir` 的精確查詢那一半 |
| `listTypes :: TypeRegistry -> [EntityTypeSpec]` | `core/src/StoryFlow/Core/Registry.hs` | F002 | `lookupDir` 掃 `owner_type` 時用它取得穩定排序 |
| `validateRegistry :: [EntityTypeSpec] -> Either [RegistryError] TypeRegistry` | `core/src/StoryFlow/Core/Registry.hs` | F002 | **要擴充**:加 `ConflictingOwnerDir` 檢查 |
| `reservedTypeKeys :: [Text]` | `core/src/StoryFlow/Core/Registry.hs` | F002 | 確認 `level` 仍是保留鍵,Level 目錄因此只能硬編 |
| `buildTree :: Level -> [Node] -> Either [TreeError] NodeTree` | `core/src/StoryFlow/Core/Tree.hs` | F002 | `addNode` / `removeNode` 寫檔**之前**驗證樹合法 |
| `loadRegistry :: FilePath -> IO (Either [LoadError] TypeRegistry)` | `types/src/StoryFlow/Types/Loader.hs` | F002 | **要擴充**:解析 `dir` / `owner_type` 兩個鍵 |
| `data Document = Document { docPath :: FilePath, docFrontRaw :: Text, docPreamble :: Text, docSections :: [Section], docEnding :: LineEnding, docFinalNL :: Bool }` | `md/src/StoryFlow/Md/Document.hs` | F003 | `mkDocument` 產生它;`replacePreamble` 改 `docPreamble` |
| `data Section = Section { secLevel :: Int, secHeadingRaw :: Text, secTitle :: Text, secId :: Id, secMetaRaw :: Maybe Text, secBodyRaw :: Text, secLine :: Int }` | `md/src/StoryFlow/Md/Document.hs` | F003 | `secLevel` 是子樹掃描的依據;`replaceSectionBody` 改 `secBodyRaw` |
| `sectionById :: Id -> Document -> Maybe Section` | `md/src/StoryFlow/Md/Document.hs` | F003 | 定位目標節與父 Node |
| `sectionIds :: Document -> [Id]` | `md/src/StoryFlow/Md/Document.hs` | F003 | `addFragment` 找檔尾最後一節 |
| `detectLineEnding :: Text -> LineEnding` | `md/src/StoryFlow/Md/Document.hs` | F003 | 既有檔的新產生行沿用原風格 |
| `renderLineEnding :: LineEnding -> Text` | `md/src/StoryFlow/Md/Document.hs` | F003 | `mkDocument` 組三段切片 |
| `parseDocument :: FilePath -> Text -> Either [MdError] Document` | `md/src/StoryFlow/Md/Parse.hs` | F003 | 每個寫入路徑的第一步:重讀檔案 |
| `parseEntityFile :: Document -> Either [MdError] (EntityFile, [MdWarning])` | `md/src/StoryFlow/Md/Parse.hs` | F003 | 取 `efMain` 的 revision 做樂觀鎖比對 |
| `parseLevelFile :: Document -> Either [MdError] (LevelFile, [MdWarning])` | `md/src/StoryFlow/Md/Parse.hs` | F003 | Level 檔寫回前的樹驗證 |
| `data EntityFile = EntityFile { efMain :: Entity, efFragments :: [Entity] }` | `md/src/StoryFlow/Md/Parse.hs` | F003 | 刪主體時要逐一檢查 `efFragments` 的被引用狀況 |
| `data LevelFile = LevelFile { lfLevel :: Level, lfNodes :: [Node] }` | `md/src/StoryFlow/Md/Parse.hs` | F003 | 直接餵給 `buildTree` |
| `documentKind :: Document -> Either [MdError] DocKind` | `md/src/StoryFlow/Md/Parse.hs` | F003 | 對 Entity 檔誤用 `addNode` 時提早擋下 |
| `renderDocument :: Document -> Text` | `md/src/StoryFlow/Md/Render.hs` | F003 | 所有寫入路徑的最後一步 |
| `updateSection :: Id -> (MetaOverride -> MetaOverride) -> Document -> Either MdError Document` | `md/src/StoryFlow/Md/Render.hs` | F003 | 節層 meta 與 links 的改寫 |
| `insertSection :: Maybe Id -> Section -> Document -> Either MdError Document` | `md/src/StoryFlow/Md/Render.hs` | F003 | `addFragment` / `addNode` 的插入 |
| `removeSection :: Id -> Document -> Either MdError Document` | `md/src/StoryFlow/Md/Render.hs` | F003 | 刪片段;`removeNode` 對子樹逐一呼叫 |
| `mkSection :: LineEnding -> Int -> Id -> Text -> Maybe MetaOverride -> Text -> Section` | `md/src/StoryFlow/Md/Render.hs` | F003 | 組新片段 / 新 Node,`Int` 就是標題層級 |
| `renderMetaBlock :: MetaOverride -> LineEnding -> Text` | `md/src/StoryFlow/Md/Render.hs` | F003 | `renderFrontmatter` 共用它的私有純量與 links 輔助函式 |
| `metaFieldOrder :: [Text]` | `md/src/StoryFlow/Md/Render.hs` | F003 | `frontmatterFieldOrder` 以它為子序列 |
| `data MetaOverride = MetaOverride { moKind, moType, moVault, moSummary, moTags, moStatus, moTimeline, moAliases, moLinks, moSource, moRevision, moCreated, moUpdated }`(全部 `Maybe`) | `md/src/StoryFlow/Md/Inherit.hs` | F003 | 新片段與新 Node 的節層 meta |
| `emptyOverride :: MetaOverride` | `md/src/StoryFlow/Md/Inherit.hs` | F003 | 組 override 的起點 |
| `decodeFrontmatter :: Text -> Either Text Meta` | `md/src/StoryFlow/Md/Yaml.hs` | F003 | `updateFrontmatter` 解出目前的 `Meta` |
| `data MdWarning = MissingSummary Id \| CustomLinkKind Id Text \| EmptyBody Id`(`renderMdWarning :: MdWarning -> Text`) | `md/src/StoryFlow/Md/Error.hs` | F003 | 缺 `summary` 之類的警告一併回傳,不吞掉 |
| `atomicWriteText :: FilePath -> Text -> IO (Either StoreError ())` | `store/src/StoryFlow/Store/Atomic.hs` | F004 | 所有寫檔;覆蓋語意,故建檔前要自己檢查存在 |
| `readTextFile :: FilePath -> IO (Either StoreError Text)` | `store/src/StoryFlow/Store/Atomic.hs` | F004 | 重讀檔案,不信任索引裡的 revision |
| `indexFile :: Connection -> Vault -> FilePath -> IO (Either StoreError ())` | `store/src/StoryFlow/Store/Index.hs` | F004 | 寫檔成功後更新索引;失敗回 `IndexUpdateFailed` |
| `unindexFile :: Connection -> FilePath -> IO ()` | `store/src/StoryFlow/Store/Index.hs` | F004 | 刪整份檔案後清掉該檔的所有索引記錄 |
| `linksTo :: Connection -> Ref -> IO [(Id, Link)]` | `store/src/StoryFlow/Store/Query.hs` | F004 | 刪除前的被引用檢查,反向查詢只有索引做得到 |
| `linksFrom :: Connection -> Id -> IO [Link]` | `store/src/StoryFlow/Store/Query.hs` | F004 | `removeEntityLink` 找出目前的 links |
| `allocateId :: Connection -> IdPrefix -> Text -> UTCTime -> IO (Either StoreError Id)` | `store/src/StoryFlow/Store/Write.hs` | F004 | 產生索引裡沒人用的 id,撞了 salt+1 重試 |
| `writeEntityMeta :: Connection -> Vault -> Id -> Int -> (MetaOverride -> MetaOverride) -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Write.hs` | F004 | **要擴充**:支援 `section_anchor` 為 NULL 的主體 |
| `data WriteResult = WriteResult { wrNewRevision :: Int, wrPath :: FilePath }` | `store/src/StoryFlow/Store/Write.hs` | F004 | 新寫入函式沿用同一個回傳型別 |
| `data StoreError = VaultNotFound Text \| VaultConfigInvalid FilePath Text \| VaultAlreadyExists FilePath \| EntityNotFound Id \| StaleRevision Id Int Int \| IdCollision IdPrefix \| FileReadFailed FilePath Text \| FileWriteFailed FilePath Text \| IndexUpdateFailed FilePath Text \| ParseFailed FilePath [MdError] \| FrontmatterWriteUnsupported Id \| SqliteError Text`(`renderStoreError :: StoreError -> Text`) | `store/src/StoryFlow/Store/Error.hs` | F004 | **要擴充**:九個新建構子,刪掉 `FrontmatterWriteUnsupported` |
| `trySqlite :: IO a -> IO (Either StoreError a)` | `store/src/StoryFlow/Store/Error.hs` | F004 | 新增的查詢一律包它,`SQLError` 不得外洩 |
| `data Vault = Vault { vaultName :: Text, vaultRoot :: FilePath, vaultCfg :: VaultConfig }` | `store/src/StoryFlow/Store/Vault.hs` | F004 | 所有寫入函式的 Vault 上下文 |
| `vaultAbsPath :: Vault -> FilePath -> FilePath` | `store/src/StoryFlow/Store/Vault.hs` | F004 | 索引相對路徑 → 可開檔的絕對路徑 |
| `vaultRelPath :: Vault -> FilePath -> FilePath` | `store/src/StoryFlow/Store/Vault.hs` | F004 | 新建檔的絕對路徑 → 存進索引的相對路徑 |
| `vaultSubdirs :: [FilePath]` | `store/src/StoryFlow/Store/Vault.hs` | F004 | 已含 `levels`,Level 目錄硬編的依據 |
| `openVaultIndex :: Vault -> IO (Either StoreError (Connection, [IndexIssue]))` | `store/src/StoryFlow/Store/Index.hs` | F004 | 測試建臨時 Vault 時取得連線 |
| `rebuildIndex :: Connection -> Vault -> IO (Either StoreError [IndexIssue])` | `store/src/StoryFlow/Store/Index.hs` | F004 | 驗收標準 2:重建後結果須與寫入當下相同 |
...
- [x] T17: 端到端:從空 Vault 建出 system.md 範例的 `characters/琳達.md` 與 `levels/教室.md`,`rebuildIndex` 後結果等價  `dep: T15, T16`
...
| T17 | `store/test/.../EndToEndSpec.hs` → `從空 Vault 建出琳達與教室並通過索引重建等價` | 只用本文檔 的函式建出 system.md 的兩份範例檔;記下全部查詢結果 → `rm index.db` → `rebuildIndex` → 結果逐項相同 |

## 實作備註

1. **`addFragment` / `addNode` 改回 `CreateResult`(與開發者確認過的偏差)**。規格書給的是
   `WriteResult`,而它只有 `wrNewRevision` 與 `wrPath` —— **新片段 / 新 Node 的 id 拿不回來**。
   那是呼叫端唯一沒有其他來源的資訊,少了它 service-and-interfaces/F001 的 service 與 service-and-interfaces/F002 的 CLI 只能
...
2. **`updateFrontmatter` 的 `Meta -> Meta` 偏差已確認並同步進 system.md**。原本
   system.md「已知缺口」開的簽名是 `MetaOverride -> MetaOverride`。已依規格書的理由改為
   `Meta -> Meta`,並把「已知缺口」整節刪除、把 frontmatter 整段重寫的代價寫進
   system.md 的「Markdown 分節格式 → 寫回策略」。
...
9. **`removeEntityLink` 比對的是檔案裡寫的那個 `Ref`**。索引為了反向查詢會把本 Vault 的
   ...
   service 層(service-and-interfaces/F001)若要放寬,該在那一層做正規化,不該讓 store 猜。
...
13. **後補兩個函式(service-and-interfaces/F001 補的)**。本規格收工時,落地層還缺兩條路,兩條都是在 P2 的業務
    層才踩到:
    ...
    - `writeEntityPatch :: Connection -> Vault -> Id -> Int -> Maybe Text ->
      ...
      (見 F003 實作備註 10)。兩件事收在同一次寫入裡:分兩次寫的話,第二次失敗會留下
    ...
    - `registerVaultIn :: FilePath -> Text -> FilePath -> IO (Either StoreError ())`
      ——**把 Vault 登記進全域註冊表**。`initVault` 只建骨架,而 ADR-008 的第一段規則
