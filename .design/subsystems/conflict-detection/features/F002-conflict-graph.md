...
`conflict-detection` 階段一的第二項。給一組「草稿已引用的片段 id」,順著關聯圖找出兩種
**完全確定性、零成本**的命中:

1. **已知矛盾**——草稿引用的片段與另一個片段已被標記 `contradicts`
2. **已被取代**——草稿引用的片段已被別的片段 `supersedes`,亦即「你正在引用一個已經被推翻的設定」

這一層是 ADR-007 三層中唯一**不需要任何模型、也不需要檢索**的一層。它的輸出是**事實**
(作者自己標註過的關聯),不是判斷——這正是 `HitLayer` 必須把層級寫進輸出的理由。

本文檔 交付的是**純函式**:吃一張 `LinkGraph` 與一組起點 id,吐 `[ConflictHit]`。
**不碰 `service`、不碰 `store`、不開索引連線**,`storyflow-conflict` 的 `build-depends`
在本文檔 之後仍然只有 `storyflow-core`(F001 的 `CabalSpec` 必須保持綠燈)。
圖從哪裡來是後續文檔 的事:`storyflow-store` 已有 `loadLinkGraph`,但 `StoryFlow.Service`
目前沒有把它匯出,**補上那條 service 出口屬於 conflict-retrieval / context-command**
(conflict-detection 功能規劃 #3 / #4)。
...
3. 每一筆命中的 `chLayer` 都是 `ByGraph`,且 `GraphEvidence` 三元組足以讓後續 feature
   直接反問「要不要建立這條關聯」,不必回頭再查圖(F001 驗收標準 2)
...
- **F001(conflict-types)**:本文檔 的輸出型別 `ConflictHit` / `HitLayer` / `GraphEvidence`
  與輸入選項 `ConflictOpts` 全部由它定義,套件 `storyflow-conflict` 本身也是它建立的。
  這是一個真正的前置,不是排序偏好——**它已實作完成並併入 main**,因此本文檔 現在就能開工
- **entity-graph-core/F002(core-types-and-registry)**:`LinkGraph` / `Id` / `Ref` / `Link` / `LinkKind`
  與 `localRef` / `renderId` / `renderRef` 都來自 core,且**已實作完成**

**可否平行開發**:F001 完成之後,本文檔 可與 `conflict-retrieval`(第 2 層)**完全平行**
——兩者都只依賴 F001,彼此不互相引用,連合流都要等 `Conflict.Pipeline` 才發生。
與 `llm-workshop-mcp` 的任何工作也無相依(本層不碰 LLM)。

**本層對 F001 的一條要求,它的實作已經滿足**:`sortHits` 用 `sortOn`(穩定排序),
且 `ByGraph` 的排序鍵給 `0` 而不是捏一個假分數(F001 實作備註 5)。本層已經把命中排成
確定順序交出去,`Conflict.Pipeline` 合流時因此不會把 `ByGraph` 之間的相對順序打散
——驗收標準 4 在合流那一步撐得住。`ByGraph` 沒有分數,F001 的「同層依分數遞減」
對它本來就無從適用。

## 對應的 Level 2 契約

實作 `design.md` 資料流管線的第 1 層。不新增對外契約條目:消費 `storyflow-core` 的
`LinkGraph`,產出 `ConflictHit`(`HitLayer` 為 `ByGraph`)。`checkConflict` /
`gatherContext` 兩個出口由後續 feature 接線,本規格只交付純函式。

## 實作方式

### 一、模組與資料流

`storyflow-conflict` 新增一個模組,不動 F001 建立的兩個:
...
`LinkGraph = M.Map Id [Link]`,關聯只存在**來源端**(ADR-002)。第 1 層要回答的兩個問題
都需要反著問:
...
ADR-005 的「B 被 A 取代後不再當比對基準」在 core 是 `supersededSet` 的遞移閉包,本層做的是
...
2. `geKind`:`Contradicts` 先於 `Supersedes`。直接用 `LinkKind` 的 derived `Ord`
   (建構子順序即 system.md 的關聯詞彙表順序);測試把這個假設釘住,
   將來有人重排建構子會立刻紅燈
...
`chSnippet` **一律 `Nothing`**:第 1 層命中的是一條關聯,不是某一段文字(F001 已言明)。

### 七、空起點的回報

`unlinkedRefs` 回報「在圖上一條關聯都沒有」的起點——既不是 `LinkGraph` 的鍵(或鍵對應空清單)、
也不是任何本地關聯的目標。ADR-007「中立」那一條說第 1 層的價值取決於作者是否勤於標註;
...
|---|---|---|---|
| `type LinkGraph = M.Map Id [Link]` | `core/src/StoryFlow/Core/Graph.hs` | entity-graph-core/F002 | 本層唯一的輸入圖結構,不另造 |
| `supersededSet :: LinkGraph -> Set Ref` | `core/src/StoryFlow/Core/Graph.hs` | entity-graph-core/F002 | 測試 oracle:本層回報的每個被取代端都必須落在它裡面 |
| `follow :: [LinkKind] -> Int -> Id -> LinkGraph -> Set Ref` | `core/src/StoryFlow/Core/Graph.hs` | entity-graph-core/F002 | 不直接呼叫;`coGraphDepth` 的「展開輪數」語意與只展開 `refVault == Nothing` 的規則都對齊它 |
| `newtype Id = Id Text`(`deriving newtype (Eq, Ord)`) | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 起點、`chTarget`、`geFrom` 的型別;排序第三鍵用它的 `Ord` |
| `data Ref = Ref { refVault :: Maybe Text, refId :: Id }` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `geTo`;反向索引以 `refVault == Nothing` 判定本地 |
| `localRef :: Id -> Ref` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 反向命中與取代命中構造 `geTo` |
| `renderId :: Id -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 理由文案 |
| `renderRef :: Ref -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 理由文案的跨 Vault 目標顯示成 `vault:id` |
| `data Link = Link { linkKind :: LinkKind, linkTarget :: Ref, linkNote :: Maybe Text }` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | 走訪的邊;`linkNote` 進理由文案 |
| `data LinkKind = Contradicts \| Supersedes \| … \| Custom Text`(`deriving stock (Show, Eq, Ord)`) | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | 過濾兩種核心關聯;排序第二鍵用它的 derived `Ord` |
| `data ConflictOpts = ConflictOpts { coTopN :: Int, coExpandBody :: Bool, coTimelineWindow :: Maybe Int, coGraphDepth :: Int }` | `conflict/src/StoryFlow/Conflict/Types.hs` | F001 | 本層只讀 `coGraphDepth`;整包收下是為了三層共用同一組選項 |
| `data GraphEvidence = GraphEvidence { geFrom :: Id, geKind :: LinkKind, geTo :: Ref }` | `conflict/src/StoryFlow/Conflict/Types.hs` | F001 | 每一筆命中的證據 |
| `data HitLayer = ByGraph GraphEvidence \| ByRetrieval Double \| ByJudge Double` | `conflict/src/StoryFlow/Conflict/Types.hs` | F001 | 本層一律產出 `ByGraph` |
| `data ConflictHit = ConflictHit { chTarget :: Id, chLayer :: HitLayer, chReason :: Text, chSnippet :: Maybe Text }` | `conflict/src/StoryFlow/Conflict/Types.hs` | F001 | 本層的最終輸出型別 |
| `data Draft = Draft { drText :: Text, drRefs :: [Id] }` | `conflict/src/StoryFlow/Conflict/Types.hs` | F001 | 呼叫端傳的是 `drRefs`;本層不吃 `Draft`(第 1 層用不到 `drText`) |

> F001 已實作完成(`status: done`),上列四列的簽名是從
