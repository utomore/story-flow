...
`docs/_archive/report-2026-08-11-console-encoding-and-db-path.md` 已提出修正方案(拒絕建立 + 啟動時印出實際路徑與筆數),但尚未套用。
...
與 [delivery/B001](./B001-resolve-db-path-silent-empty-db.md) 同一個模式:把「資料庫不存在」預設當成
...
**根因**:與 [delivery/B001](./B001-resolve-db-path-silent-empty-db.md) 同一個模式 —— 把「資料庫不存在」
