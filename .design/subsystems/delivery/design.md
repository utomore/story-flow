...
**未預期失敗的狀態碼(G-E003)**:每個 handler 都有出口,逃出來的例外不得變成
空白的 500。可重試的資料庫忙碌(`SQLITE_BUSY` / `SQLITE_LOCKED`,判準是 catalog 的
`isBusy`)回 **503**,其餘回 **500**;兩者的 body 都是繁中訊息,不是空的。
忙碌的典型成因是背景掃描正在寫入,那是預期中的並行(ADR-009)而不是故障 ——
前端據此知道該稍後重試而不是回報「伺服器壞了」。
...
- **驗收標準**:
  ...
  - 資料庫出事時 `/api/*` 回 5xx 且 body 是有內容的繁中(忙碌 503、其餘 500),不是空白的 500
