...
updated: 2026-08-19
depends-on: [F002]
related-adr: [ADR-002, ADR-003, ADR-004]
related-feature: [F001, F002, F004]
---

# F003: P1 Markdown 分節格式解析

## 功能概述

`storyflow-md` 負責 Markdown 檔與核心型別之間的雙向轉換,純函式、吃 `Text` 吐型別。
Markdown 檔是唯一的真相來源(ADR-002),所以這個套件的正確性直接等於資料的安全性:
解析漏一個欄位就是設定遺失,寫回破壞排版就是作者的手稿被工具改壞。

本規格同時**確定 Level 場景樹的 Markdown 格式**——system.md 只給了 Entity 的分節
...
不做:任何檔案 IO(`storyflow-md` 吃 `Text`,讀檔是 F004 的事)、SQLite、
關聯的跨檔驗證(目標 Entity 是否存在要查索引,屬 F004)、型別註冊表的合規檢查
(`checkEntity` 在 core,由 service 於 P2 呼叫)。

**驗收標準**

- system.md 的琳達範例檔解析後得到 1 個主體 Entity + 2 個片段 Entity,欄位全對
- 本規格定義的教室 Level 範例檔解析後,交給 `buildTree` 能建出 ADR-004 的那棵樹
...
`depends-on: [F002]` —— 需要 core 的全部型別(`Meta` / `Entity` / `Level` / `Node` /
`Link` / `Id`)與其 aeson 實例。

**本規格不依賴 F004,但 F004 依賴本規格**:相依是單向的,`storyflow-md` 純函式、
不碰 IO;`storyflow-store` 則在型別層就引用本套件的 `MdError` 與 `MetaOverride`,
`build-depends` 必須含 `storyflow-md`。

兩份規格由不同人同時開工是安全的,但要清楚交會面有多寬:F004 只有 T1–T4
...
「新增的介面」已經定死,F004 先寫呼叫端再等本規格補上實作是可行的做法。

## 對應的 Level 2 契約

實作 `design.md`「模組間公開介面」的 `storyflow-md` ↔ `storyflow-core` 一條:`Document` ↔
`Entity` / `Level` / `Node` 的雙向轉換、節層繼承規則與位元組保留的寫回,對應資料流管線的
「解析 → 節層繼承」與「位元組保留寫回」兩段。不新增對外契約條目。
...
system.md 只寫了「節層未寫的欄位繼承檔案層(`vault`/`type`/`timeline`/`status` 等)」。
...
| `revision` | **不繼承**,未寫時視為 `1` | 繼承會讓多個片段共用同一個 revision 值,樂觀鎖(F004)就失去意義 |
...
否則走 Entity 解析。`level` 因此是保留型別鍵,不可用於 `types/registry/`(由 F002 的
`validateRegistry` 檢查)。
...
`timeline` → `aliases` → `source` → `revision` → `links`),與 system.md 的範例一致;
...
F002 在 `StoryFlow.Core.Json` 定義的 `FromJSON` 實例,編碼規則因此全系統只有一份。
...
F001 的建置骨架:`storyflow-md` 套件與其 test-suite、UTF-8 測試進入點。
...
**格式契約**(本規格新確立、需回寫 system.md):Level 檔的 Markdown 格式為
...
| T6 | `StoryFlow.Md.ParseEntitySpec` | system.md 的琳達範例檔解析得 1 個主體(`ent-7f3a`,body 為 preamble)+ 2 個片段(`ent-7f3b` / `ent-7f3c`),逐欄比對預期值;`ent-7f3c` 的三筆 links 與 `timeline` 正確 |
...
2. **`timeline` 接受純字串簡寫**。system.md 的琳達範例寫
...
10. **後補 `renameSection`(service-and-interfaces/F001 補的)**。
    ...
    沒問題,節層就沒有路——service 的 `EntityPatch.epTitle` 因此在 service-and-interfaces/F001 才踩到。

**回寫 system.md**:Level 檔的格式契約在 bcd20cc 已經寫進 system.md
