...
| E001 | 2 | `BoundarySpec.hs:102` L1、`:113` E1:`exposed-modules` 實得 15 項 | L1 / E1 原文 | **qa 誤讀** | qa 的 `fieldSection "exposed-modules:" "build-depends:"` 硬編下一個欄位當終點,而 library stanza 的順序是 `exposed-modules:` → `other-modules:` → `build-depends:`,區段因此**跨過中間整個 `other-modules` 區塊**,拿到兩份清單的聯集。被測的 `.cabal` 早已符合 L1/E1(11 + 4),紅的是量測工具。**附帶發現**:同一個取法用在 L2 上是綠的,但綠得沒有道理——`other-modules:` 剛好緊鄰 `build-depends:` 而已。qa 改用縮排邊界法(不假設欄位順序與名稱)後 L1/L2/E1/L3/L4 全綠 |
| E001 | 3(spec 修訂後重跑,計數重新起算) | (無紅燈) | — | — | G20 裁決「改 spec 文字、不動實作」後,spec 修訂完成(語意欄改 `(mtime, size)`、R4 補機械可判定子句、新增 E6、`Walk.hs` haddock 補順序與奈秒理由)。本輪**只派 qa**——`store/src` 已無任何 `undefined`,實作一直是對的,沒有東西要 impl 做。qa 把 R4 的解構改成 `Right (_mtime, size)` 並補上 E6,一次過。編排者實跑 **872 examples / 0 failures**,五個 suite 全 PASS |
| E001 | 2 | `WalkSpec.hs:87` R4:`statOf` 第一分量實得 `1787759…`(奈秒時間戳)而非位元組數 | 「數據與介面變動」語意欄 vs R4 | **spec bug** | **spec 自己前後矛盾**:語意欄與骨架 haddock 寫 `(size, mtime)`,R4 卻要求「與搬移前相同」而搬移前(`git show HEAD:.../Index.hs`)是 `pure (floor (utcTimeToPOSIXSeconds t * 1e9), fromIntegral s)` = `(mtime, size)`。兩條不可能同時成立。impl 逐字搬移(編排者以 `git show` 比對過 byte-identical)正是 R4 要的,qa 逐字照抄語意欄也沒錯。**簽名比對擋不住**:`(Int64, Int64)` 兩分量同型別,順序寫反型別檢查照過,`/enhance-design` 的一致性檢查只比對簽名原文,因此沒攔下來。**錯在編排者本人寫 spec 時未查證 tuple 順序**。停下,記 spec-gaps **G20**,等開發者裁決;未自行修 spec、未讓兩邊協商 |
