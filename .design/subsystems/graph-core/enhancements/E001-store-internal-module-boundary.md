...
build-log DEC-11 記過這件事,處置是「S3 重建 `service` 時改這一行」——修了個案,沒修讓個案發生的原因。
...
所以關掉 `Node` 與 `Row` 在 `aapms-store` 之外零成本;只有 `Edit` 有一個外部消費者,而它是 S3 會
重寫的凍結程式碼。
...
- **契約 E 一個字不動**。開發者已裁決 `vaultMarkdownFiles` 不收進契約 E:S3 設計 `service` 時
  「`IndexReport` 要的檔案數」該由真正的消費者在 `/subsys-design service` 正式提出需求,不由本次
  替一個還沒設計的子系統猜它要什麼形狀
- **`service/` 一行不改**(凍結中,S3 重寫)。DEC-11 那條穿透 import 留給 S3
...
驗收:LAW-1–LAW-4 全綠(新 law),REG-1–REG-6 全綠(回歸 law),`cabal test all` 全綠。
...
- **`service/` 凍結中,一行不改。** `Service.hs:130` 的穿透 import 與 `:231` 的
  `vaultMarkdownFiles` 都留給 S3 重建 `service` 時處理;本次不為了讓它編得過而放寬任何界線
- **`Walk` 的兩個函式是原樣搬移**,不是重寫:搬過去之後行為必須與 `Index.hs:85` / `Index.hs:104`
  的現有實作完全相同(REG-3 / REG-4 在釘這件事)
...
- **不可逆決定**:**無**。可見度是 `.cabal` 的一個欄位,要改回去是一行的事。若 S3 發現 `service`
...
可與其他任務平行:可以。本次只碰 `store/`,與 S2 的一次性匯出器、S3 的 `workspace` / `service`
設計都不重疊。唯一的排序要求是**要在 S3 重建 `service` 之前完成**——否則新寫的 `service` 會再一次
...
| `data WriteResult = WriteResult { wrId :: Id, wrPath :: FilePath, wrRevision :: Revision, wrIssues :: [IndexIssue] }` | `store/src/Aapms/Store/Edit.hs:96-104` | graph-core/F008 契約 E | 不動;確認它仍經 `Write.hs:3` re-export 到門面(REG-2) |
...
`Aapms.Store.Walk` 可以 import(否則 REG-3 / REG-4 寫不出來):

| 檔案 | 變動 |
|---|---|
| `store/src/Aapms/Store/Walk.hs` | **新檔**:兩個函式的簽名完整,本體 `undefined` |
| `store/aapms-store.cabal` | `Aapms.Store.Walk` 暫時加進 `exposed-modules`,讓現行 test-suite(仍依賴 `aapms-store` 套件)import 得到;**impl 要把它與另外三個一起移進 `other-modules`**(LAW-1 / LAW-2 就是在釘這一步) |

編譯檢查:`cabal build aapms-store` 通過,無警告(`[12 of 15] Compiling Aapms.Store.Walk`)。

**2026-08-27 更新(GAP-20 裁決後)**:上表描述的是**第一版骨架**的狀態,已由 `/spec-build` 的
...
- REG-1、REG-2、REG-5、REG-6:**全綠**(捕捉現況)
- REG-3、EX-3、EX-4、EX-5:**全綠**——`Walk.hs` 的本體已經填好了,不再是 `undefined`
- **REG-4 與新增的 EX-6:目前紅,修完就綠。** 紅的原因是上一版 spec 把 tuple 順序寫反,qa 據此
  斷言了第一個分量等於位元組數。正確順序是 **`(mtime, size)`**:請只對**第二個**分量斷言長度,
  第一個分量是奈秒時間戳。**這是本次修訂唯一要動的測試**,其餘斷言一律不得更動
- LAW-1–LAW-4:**全綠**(第一輪已完成那些 cabal 與匯出清單的變動)
- `IndexSpec.hs` 的 STEP-3 已由第一輪的 qa 遷移到 `Aapms.Store.Walk`,無須再動

## 實作備註

**2026-08-27 spec 修訂(GAP-20)—— `statOf` 的 tuple 順序寫反**
...
語意欄說 `(size, mtime)`,REG-4 卻要求「與搬移前相同」而搬移前就是 `(mtime, size)`。
qa 照語意欄寫斷言 → 紅;impl 照 REG-4 逐字搬移 → 正確。兩邊都沒錯,錯的是 spec。

**裁決(2026-08-27,開發者)**:**改 spec 的文字,不動實作。** 實作、`Index.hs:129` 的
`Right (mtime, size)`、`:412` 的 `m/m'`、`s/s'` 比對、REG-4 四者本來就一致;過時偵測比的是兩個分量
...
| REG-4 | 補上 tuple 順序的機械可判定子句(只對**第二**分量斷言長度) |
| Examples | 新增 **EX-6**,把順序釘成一個具體例子 |
