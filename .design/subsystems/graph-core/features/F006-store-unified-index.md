...
- `rm index.db` 後 `openVault` + `rebuildIndex` 與刪除前的 `listNodes` / `linksFrom` 結果相同
  (S0 契約測試精神,套件內版本——見「待確認假設」ASM-8 說明與真正的 contract/ 套件測試的關係)
...
**ASM-1 已由開發者裁決並落地(2026-08-23 階段二),不再是阻塞項**:契約 E 改成
...
擴充(不重新定義)F005 留下的 `IndexIssue`(契約 G「骨架」原則,ASM-6);擴充(不重新定義)F005
...
### 0. `checkMeta` 整合(ASM-1 已解除,兩條驗收標準都完整實作)

`checkMeta :: TypeRegistry -> AnyNode -> [MetaWarning]`(`core/src/Aapms/Core/Registry.hs:180`)
需要的 `TypeRegistry` 現在由 `VaultHandle` 直接攜帶(`vhRegistry`,DEC-9 裁決,F005 落地
...
原則,ASM-6)。**警告不擋索引**——`checkMeta` 本身的契約是「只回警告,不決定要不要擋」,節點正常
...
data IndexIssue
  ...
  | MetaWarningsFound FilePath Id [MetaWarning]  -- checkMeta 警告,不擋索引(ASM-1)
...
- `renderDocKind` / `parseDocKind :: DocKind <-> Text`(待確認假設 ASM-6)
- `SQLData` 輔助沿用 legacy `Row.hs` 的 `sText` / `sInt` / `sMaybeText` / `sMaybeInt` / `dayText`
  形狀(legacy 檔案本身在 STEP-1 的委派決策下不重用,只重用**寫法**)
...
的收集成 `MetaWarningsFound`(ASM-1,不影響是否寫入)→ 組出待寫入的列 → 一個 SQLite transaction
內:先刪掉這個 `file_path` 既有的 `files` 列(級聯清掉舊資料,見上方 Schema 一節),寫入新
`files` 列,寫入 `nodes` + 對應專屬表 + `links`;若途中撞到 `assets.name UNIQUE` 衝突,**整個
transaction 回滾**、回報 `DuplicateAssetName`(待確認假設 ASM-2:granularity 是整檔,不是單筆
...
ASM-4,契約未明定);`nfOwner` 是 `owner = ?`;`nfLicense` 是 `assets.license = ? OR packs.license =
...
`owner` 指向這種 pack 的節點」(待確認假設 ASM-3:適用範圍包含 pack 本身,不只它的 asset)。
...
（待確認假設 ASM-5:`PEnt`/`PAst`/`PPck` 每次 `lookupNode` 都重新讀檔+解析整份檔案才能取得 body,
...
`(Id, Link)`(與 legacy `Query.hs:237` 的 `(Id, Link)` 不同,見待確認假設 ASM-7)——依 `Ref` 的
...
| `data IndexIssue = SchemaRebuilt { irOldVersion :: Maybe Int, irNewVersion :: Int }` | `store/src/Aapms/Store/Schema.hs:66-71` | F005 | 本 feature 擴充的既有型別(ASM-6 原則) |
...
data IndexIssue
  ...
  | MetaWarningsFound FilePath Id [MetaWarning]   -- ASM-1 解除後新增,不擋索引
  deriving stock (Show, Eq)
...
emptyNodeFilter :: NodeFilter    -- 全部欄位取最寬鬆的預設值(nfLimit 需要一個有限預設值,
                                  -- 見待確認假設 ASM-9);契約 F 沒有明列這個輔助值,但 NodeFilter
...
-- 只是為了讓 STEP-2 的 row-roundtrip 測試能從 test-suite 白箱直接 import——other-modules 對
...
- [x] STEP-1: `Aapms.Store.Schema` 擴充:`indexTables`/`schemaDDL` 加 11 張業務表(含 `ON DELETE
  CASCADE` 外鍵)、`schemaVersion` 改 2、`IndexIssue` 加**四個**建構子(ASM-1 解除後多一個
  `MetaWarningsFound`)、`renderIndexIssue` 補四個 case  `dep: -`
- [x] STEP-2: 新建 `Aapms.Store.Row`:`nodeColumnList`/`nodeFields`/`NodeRow`/`hydrateMeta`、六個
  專屬表的 row 型別與 `FromRow` 實例、`renderDocKind`/`parseDocKind`、`SQLData` 輔助
  `dep: T1`
- [x] STEP-3: 新建 `Aapms.Store.Index`:內部檔案掃描(`vaultMarkdownFiles` 等價,略過 `.` 開頭目錄、
  只收 `.md`、排序)與 `statOf`(mtime 奈秒 + size)  `dep: T1`
- [x] STEP-4: `Aapms.Store.Index`:`indexOne`(內部)——讀檔 → `parseDocument` → `docKind` 分支 →
  `to*` → `LevelDoc` 額外 `buildTree` → 對每個節點 `checkMeta` → 一個 transaction 內刪舊插新;
  `assets.name` 衝突整檔回滾回報 `DuplicateAssetName`  `dep: T2, T3`
- [x] STEP-5: `Aapms.Store.Index`:`indexFile`/`unindexFile` 依 `indexOne`/`DELETE FROM files`
  組出  `dep: T4`
- [x] STEP-6: `Aapms.Store.Index`:`rebuildIndex`(清空業務表 → 排序掃描 → 逐檔 `indexOne` → 收集
  `IndexIssue`,`StoreError` 直接中止)  `dep: T4, T5`
- [x] STEP-7: `Aapms.Store.Index`:`refreshStale`(`files` 表 vs 磁碟現況的 mtime/size 差集 →
  消失的 `unindexFile`、過時/新增的 `indexOne`)  `dep: T4, T5`
- [x] STEP-8: 新建 `Aapms.Store.Query`:`NodeFilter`、`emptyNodeFilter`、WHERE 子句組裝(`nfStatus =
  []` 排除 missing、`nfTags` AND、`nfNamedOnly`/`nfIncludeReference`/`nfLicense` 的 JOIN 邏輯)、
  `listNodes`  `dep: T2`
- [x] STEP-9: `Aapms.Store.Query`:`lookupNode`(依 prefix 分七支,`PEnt`/`PAst`/`PPck` 回讀檔案取
  body,其餘純索引)  `dep: T2, T8`
- [x] STEP-10: `Aapms.Store.Query`:`lookupByName`(查 `assets.name` → 走 `PAst` 同一條路徑)
  `dep: T9`
- [x] STEP-11: `Aapms.Store.Query`:`childrenOf`(`owner = ?` 查詢)  `dep: T2`
- [x] STEP-12: `Aapms.Store.Query`:`linksFrom`/`linksTo`/`loadLinkGraph`  `dep: T2`
- [x] STEP-13: `store/aapms-store.cabal`:library `exposed-modules` 加 `Aapms.Store.Index`/
...
- [x] STEP-14: `store/test/Aapms/Store/Fixtures.hs` 擴充:story vault 與 asset vault 各一份最小但
...
- [x] STEP-15: 改寫 `store/test/Aapms/Store/IndexSpec.hs`/`RebuildSpec.hs`/`StaleSpec.hs`:
  ...
  `dep: T14`
- [x] STEP-16: 改寫 `store/test/Aapms/Store/QuerySpec.hs`/`NodeSpec.hs`:`lookupNode` 七種 prefix
  ...
  `dep: T14`
- [x] STEP-17: `store/test/Spec.hs` 註冊新 Spec;`cabal build aapms-store`/`cabal test aapms-store`
...
| Todo | 測試 | 說明 |
|------|------|------|
| STEP-1 | test_schema_has_all_tables | `openIndexAt` 建出的資料庫含 12 張表(`sqlite_master` 查詢),`schema_version = 2`(`SchemaSpec.hs`) |
| STEP-1 | test_index_issue_render_new_cases | `renderIndexIssue` 對 `ParseFailed`/`TreeInvalid`/`DuplicateAssetName`/`MetaWarningsFound` 輸出非空、含中文、指出檔案路徑(`SchemaSpec.hs`) |
| STEP-2 | test_row_roundtrip | 對合成的 `Meta`(含 tags/aliases/links/timeline)寫入 `nodes`+附屬表後 `hydrateMeta` 讀回,欄位相等(`RowSpec.hs`,新增檔案——原表未列 STEP-2 的所在檔案,補上) |
| STEP-2 | test_dockind_text_roundtrip | `parseDocKind . renderDocKind` 對四種 `DocKind` 是 identity(`RowSpec.hs`) |
| STEP-3 | test_scan_skips_dot_dirs_and_non_md | 臨時目錄含 `.aapms/`、`.git/`、`foo.txt`、`bar.md`,掃描只回 `bar.md`(`IndexSpec.hs`) |
| STEP-4 | test_indexOne_topic | story fixture 的主題檔索引後,`nodes` 有主體(owner NULL)+ 片段(owner = 主體 id) |
| STEP-4 | test_indexOne_pack | asset fixture 的 pack.md 索引後,`nodes`/`assets` 有 pack(owner NULL)+ 全部 asset(owner = pack id),含 `status = missing` 的那筆 |
| STEP-4 | test_indexOne_level_tree_invalid | **實作備註**:`toLevel` 自己的 `structure`/`rootId` 已把「跳級」「root 不符第一節」擋在 `MdError` 那層,由合法標題長出的 `[Node]` 天生滿足 `buildTree` 的不變量——真正能讓 `buildTree` 失敗的只有「frontmatter 宣告 `root` 但檔案零個 Node」。測試改用這個 fixture,索引結果含 `TreeInvalid`,`nodes` 沒有該檔案的殘留 |
| STEP-4 | test_indexOne_parse_failed | 造一個 YAML 壞掉的檔案,索引結果含 `ParseFailed`,`nodes` 沒有殘留 |
| STEP-4 | test_indexOne_duplicate_asset_name | 兩個不同檔案的 asset 撞同一個 `name`,後索引的那個檔案整檔回滾並回 `DuplicateAssetName`,先索引的那個保留 |
| STEP-5 | test_indexFile_reindex_replaces | 對已索引的檔案改內容後重新 `indexFile`,舊記錄被整檔替換而非疊加 |
| STEP-5 | test_unindexFile_cascades | `unindexFile` 後該檔案的 `nodes`/`assets`/`links`/`node_tags` 等全部記錄消失,`files` 也消失 |
| STEP-5 | test_unindexFile_idempotent | 對不存在的路徑呼叫 `unindexFile` 不報錯 |
| STEP-6 | test_rebuild_idempotent | 對兩個 fixture vault 各跑兩次 `rebuildIndex`,兩次的 `listNodes emptyNodeFilter`/`linksFrom`/`childrenOf` 結果相同 |
| STEP-6 | test_rebuild_not_aborted_by_content_issues | fixture 混入一個解析失敗的檔案,`rebuildIndex` 仍把其餘檔案正確索引且回傳含該筆 `IndexIssue` |
| STEP-7 | test_rm_index_db_rebuild_equivalent | S0 契約測試精神(套件內版本):索引後拍下 `listNodes`/`linksFrom` 快照,`rm index.db` → `openVault` → `rebuildIndex`,快照相同 |
| STEP-7 | test_refresh_stale_only_rereads_changed | 索引後只改一個檔案的 mtime/size,`refreshStale` 只重讀那一個,其餘檔案的 `nodes.revision` 等欄位不變(可由 rowid 或內容比對驗證未被觸碰) |
| STEP-7 | test_refresh_stale_removes_gone_files | 索引後刪除磁碟上一個檔案,`refreshStale` 把該檔案的記錄移除 |
| STEP-8 | test_nodeFilter_status_default_excludes_missing | `nfStatus = []` 的 `listNodes` 不回 `status = missing` 的 asset,`nfStatus = [Missing]` 才回 |
| STEP-8 | test_nodeFilter_named_only | `nfNamedOnly = True` 只回 `assets.name` 非 NULL 的節點 |
| STEP-8 | test_nodeFilter_include_reference | pack 路徑含 `library/reference/` 的 fixture(`is_reference = True`),預設 `listNodes` 排除該 pack 與其 asset,`nfIncludeReference = True` 才回 |
| STEP-8 | test_nodeFilter_prefixes_types_tags | 組合 `nfPrefixes`/`nfTypes`/`nfTags` 驗證交集語意 |
| STEP-9 | test_lookupNode_all_prefixes | 對 fixture 裡的 `ent`/`ast`/`pck`/`lic`/`lvl`/`nod` 六種 id 呼叫 `lookupNode`,回傳對應的 `AnyNode` 建構子且欄位(含回讀的 body)正確;`vlt-`/`prj-` 開頭的 id 回 `Nothing` |
| STEP-10 | test_lookupByName | 已命名的 asset 查得到,未命名(`astName = Nothing`)或不存在的名稱回 `Nothing` |
| STEP-11 | test_childrenOf_pack_and_entity | `childrenOf` 對 pack id 回全部 asset、對主體 entity id 回全部片段,對一個沒有子節點的 id 回 `[]` |
| STEP-12 | test_linksFrom_linksTo_loadLinkGraph | fixture 建兩條關聯,`linksFrom` 依來源查、`linksTo` 依目標查(含 `Meta` 正確)、`loadLinkGraph` 回傳的 map 與兩者一致 |
| STEP-13 | test_cabal_wiring | `aapms-store.cabal` 原始碼含 `Aapms.Store.Index`/`Query`/`Row`/`aapms-md`;`Aapms.Store` 門面可 import 到 `rebuildIndex`/`listNodes`(`Aapms/StoreSpec.hs`) |
| STEP-14 | test_fixtures_parse | 兩個 fixture vault 的全部檔案能被 `parseDocument`+對應 `to*` 成功解析(供其餘測試使用的前提健檢) |
| STEP-15 | (併入上方 STEP-3/STEP-4/STEP-5/STEP-6/STEP-7 各條,`IndexSpec.hs`/`RebuildSpec.hs`/`StaleSpec.hs` 是這些測試實際所在檔案) | - |
| STEP-16 | (併入上方 STEP-8/STEP-9/STEP-10/STEP-11/STEP-12 各條,`QuerySpec.hs`/`NodeSpec.hs` 是這些測試實際所在檔案) | - |
| STEP-17 | cabal test aapms-store exit 0 | 全套件測試通過(75 examples, 0 failures);`Spec.hs` 註冊新 Spec 且不含已刪除的舊 `describe` |

## 待確認假設

- **ASM-1(已由開發者裁決並落地,不再是待確認項,保留記錄供追溯)**:設計階段原判斷契約 E 給定的
  ...
  `checkMeta`,有警告的收進回傳清單(不擋索引)。測試見 STEP-1(`renderIndexIssue`)與 STEP-4(fixture
...
- `store/src/Aapms/Store/Row.hs`(改寫:節點列轉換,`vault` 欄位處理見待確認假設 ASM-10)
...
- 新增測試:`IndexSpec.hs`(STEP-3/STEP-4/STEP-5/STEP-14)、`RebuildSpec.hs`(STEP-6)、`StaleSpec.hs`(STEP-7)、
  `QuerySpec.hs`(STEP-8/STEP-12)、`NodeSpec.hs`(STEP-9/STEP-10/STEP-11)、`RowSpec.hs`(STEP-2,原表未列所在檔案,
  補一個新檔)
...
- `Aapms.Store.Row` 放進 cabal 的 `exposed-modules`(文檔原寫 `other-modules`)——原因是
  `other-modules` 對同一個套件的 test-suite component 也不可見,STEP-2 的白箱測試需要直接
...
- `Row.hs` 的 `hydrateMeta` 簽名是 `VaultId -> Connection -> NodeRow -> IO Meta`(文檔原寫
  `Connection -> NodeRow -> IO Meta`),多一個 `VaultId` 參數——見待確認假設 ASM-10,`nodes` 表不存
...
筆測試,含 STEP-2 的 `RowSpec.hs` 2 筆是文檔對照表沒有原列的新增檔案)。`cabal build all`/
