...
| E | `checkReferences :: VaultSet -> VaultHandle -> IO [DanglingRef]` | 逐字實作;`DanglingRef` 的形狀由本 feature 定(DEC-3;2026-08-26 ASM-3 裁決後已回寫契約 E) |
| E | `closeVaultSet` / `vaultSetIds` / `maxAttachedVaults` | 逐字實作(2026-08-26 ASM-2 裁決後已回寫契約 E) |
| E | `DanglingRef` / `DanglingReason` | 逐字實作(2026-08-26 ASM-3 裁決後已回寫契約 E) |
...
| G | `VaultIdCollision VaultId FilePath FilePath` | 新增 `StoreError` 建構子與 `renderStoreError` 分支(2026-08-26 ASM-5 裁決) |
| 模組間公開介面 | MultiVault → Query:`listAcross` 重用 `whereOf` 條件片段對加了 schema 前綴的 UNION 視圖執行;`searchAcross` 各 vault 各自取命中後在 Haskell 合併分數 | 逐字實作(2026-08-26 ASM-1 分案裁決後已回寫) |
...
> 否則斷言恆真、證明不了任何事):
>
> - **S1**:`vs` 至少含**兩個** vault,且每個 vault 的索引都至少有一個符合當次過濾條件的節點
> - **S2**(排序鍵交錯):存在 vault A 的兩筆與 vault B 的一筆,使 B 的那一筆的排序鍵**落在 A 的
>   兩筆之間**(`listAcross` 的鍵是 `metaId`;`searchAcross` 的鍵是「`shScore` 遞減、`metaId` 遞增」)
> - **S3**(跨 vault 同短 id):存在一個 `Id i` 同時是 A 的節點與 B 的節點(ADR-014 明訂短 id 只在
>   vault 內唯一,這是合法狀態)
> - **S4**(三種關聯都在場):`checkReferences` 的目標 vault 裡同時有「解析得到」「目標 vault 在
...
- **LAW-1**(去重與上限):對所有**沒有撞號**的把手清單 `hs`(定義:任兩筆 `h`、`h'`,
...
- **LAW-1b**(撞號):對所有存在兩筆 `h`、`h'` 使 `vid h == vid h'` 且 `vhRoot h /= vhRoot h'` 的 `hs`,
  ...
  *非退化*:樣本要同時含「撞號且清單長度 `<=` 上限」與「撞號且清單長度 `>` 上限」兩組——只有後者
  ...
  東西」這個症狀在測試裡看不出來。
- **LAW-2**:對所有 `hs`、所有 `h ∈ hs`、所有 `f`:`closeVaultSet vs` 之後 `listNodes h f` 的結果與
  `openVaultSet hs` 之前逐筆相同,且其後的 `closeVault h` 仍正常完成。
  *非退化*:S1(每個 vault 的索引非空——兩邊都是 `[]` 的話這條恆真)。
- **LAW-3**:對所有 `h ∈ hs`、所有 `f` 與 `q`:在 `openVaultSet hs` 之後、以及任意次數的 `listAcross` /
  `searchAcross` / `lookupRef` / `checkReferences` 之後,`listNodes h f` 與 `search h q` 的結果都與
  `openVaultSet` 之前逐筆相同(`VaultSet` 不改變單一 vault 的讀取行為)。
  *非退化*:S1;且 `q` 的樣本要有文字條件(只測 `sqText = Nothing` 等於沒測到 FTS 那條路)。

**`listAcross`**

- **LAW-4**(過濾語意只有一份):對所有 `f`,`listAcross vs (wide f)` 的**多重集合**等於
  `concat [ [(vid h, m) | m <- listNodes h (wide f)] | h <- hs ]` 的多重集合。
  *非退化*:S1;且樣本必須讓 `nfPrefixes` / `nfTypes` / `nfStatus` / `nfTags` / `nfOwner` /
  ...
  **兩處**直接寫出表名(其餘全用 `n` / `a` / `p` 別名),schema 前綴漏掉時它們會拿**別的
  ...
  1. **`nfTags` → `node_tags` 存在性子查詢**。兩個 vault **各自都要有標籤,而且標籤集合要能
     ...
     **只掛在該 vault 的一個節上**——它必須是**節層**標籤,寫在檔案層會被同檔每一個節繼承
     ...
     「真的篩掉東西」這個非退化條件當場失效。見 Examples 的 fixture 前提與 EX-17
  2. **`nfIncludeReference` → `packs` 的 reference 子查詢**。兩個 vault **各自都要有**一個位於
     ...
     `False`,這條路是**預設路徑**。見 EX-16
- **LAW-5**(全域排序):對所有 `f`,`listAcross vs (wide f)` 相鄰的兩筆 `(v1, m1)`、`(v2, m2)` 滿足
  `metaId m1 < metaId m2`,或 `metaId m1 == metaId m2 && v1 < v2`。
  *非退化*:S2 + S3。少了 S2,「各 vault 各自排完再接」也會滿足這條。
- **LAW-6**(分頁是對整體切窗):對所有 `f` 與所有 `j >= 0`、`k >= 0`,
  `listAcross vs f { nfOffset = j, nfLimit = k } == take k (drop j (listAcross vs (wide f)))`。
  *非退化*:S2;且 `(j, k)` 的樣本必須含「視窗**跨越** vault 邊界」與「視窗**完全落在後一個
  vault 的節點上**」兩種——只取 `j = 0` 的話「各自排完再接」在前綴上可能剛好一致。
- **LAW-7**(vault 欄一致):對所有 `f`,`listAcross vs f` 的每一筆 `(v, m)` 滿足 `metaVault m == v`,
  且 `v ∈ vaultSetIds vs`。
  *非退化*:S1。單一 vault 下這條永遠成立,證明不了「`Meta` 是用哪個 vault hydrate 出來的」。

**`searchAcross`**

- **LAW-8**(逐 vault 等價,四欄逐欄相同):對所有 `q`,令 `q' = (wideQ q) { sqFacets = False }`,則
  ...
  *非退化*:S1;且樣本必須含一個「在**兩個** vault 都有命中」的 `sqText`,以及一個 `sqText = Nothing`
  的樣本。
- **LAW-9**(全域排序):`srHits (searchAcross vs q')` 相鄰兩筆 `a`、`b` 滿足
  `shScore a > shScore b`,或分數相同且 `metaId (shMeta a) < metaId (shMeta b)`,或兩者都相同且
  `shVault a < shVault b`。
  *非退化*:S2 的分數版(A 的一筆分數落在 B 的兩筆之間)+ S3。
- **LAW-10**(分頁與 `srTotal`):對所有 `q` 與所有 `j`、`k`,令
  ...
  *非退化*:S2;`(j, k)` 的樣本要求同 LAW-6。
- **LAW-11**(vault 欄一致,檢索側):對所有 `q`,`srHits (searchAcross vs q)` 的每一筆 `x` 滿足
  `metaVault (shMeta x) == shVault x` 且 `shVault x ∈ vaultSetIds vs`。
  *非退化*:S1。
- **LAW-12**(facet):對所有 `q`,`sqFacets q == False` ⟺ `srFacets == Nothing`。`sqFacets q == True`
  ...
  2. `fcTypes` / `fcTags` / `fcOwners` / `fcLicenses` 的每一筆 `(v, n)`,`n` 等於**各 vault 對同一個
     值的計數之和**
  3. 四個維度都維持 F007 LAW-17 的性質:`fcTypes` 不因 `nfTypes` 改變、`fcTags` 不因 `nfTags` 改變、
     `fcOwners` 不因 `nfOwner` 改變、`fcLicenses` 不因 `nfLicense` 改變
  *非退化*:S1;**至少一個 type 與一個 tag 要在兩個 vault 都出現**(fixture 的共同標籤
...
> **facet 走哪一條**(對應 ASM-1 的 `searchAcross` 走 Haskell):跨 vault 的 facet 是
...
> 子句 3(facet 排除自己的條件)因此是 F007 LAW-17 的直接後果:每個 vault 各自已經排除過,求和
> 保持這個性質。
...
- **LAW-13**(帶 vault 的 `Ref` 只認自己那個 vault):對所有 `h ∈ hs`、所有 `Id i`、所有
  `v ∈ vaultSetIds vs`:
  `lookupRef vs v (Ref (Just (vid h)) i)` 的結果等於 `fmap ((,) (vid h)) <$> lookupNode h i`。
  *非退化*:S3;且 `v` 的樣本必須同時含 `v == vid h` 與 `v /= vid h`。
- **LAW-14**(預設 vault):對所有 `v ∈ vaultSetIds vs` 與所有 `i`,
  `lookupRef vs v (Ref Nothing i) == lookupRef vs v (Ref (Just v) i)`。
  *非退化*:S3(否則「忽略 `refVault` 一律全 vault 掃描」也會滿足)。
- **LAW-15**(vault 不在集合裡):對所有不屬於 `vaultSetIds vs` 的 `w`、所有 `v ∈ vaultSetIds vs`、
...
- **LAW-16**(完整且不多報):對所有 `vs` 與所有 `h`(`h` **不必**屬於 `hs`),
  ...
  *非退化*:S4——三種關聯都要在場。只有「全部解析得到」的 fixture 會讓 `[] == []` 恆真;只有懸空
  的 fixture 證明不了它會放過正常的關聯。
- **LAW-17**(`renderDanglingRef`):對所有 `d`,`renderDanglingRef d` 非空;含 `renderId (drSource d)`
  這個子字串;含 `renderRef (drTarget d)` 這個子字串;且把訊息以 `;` / `;` / `,` / `,` / `。`
  切成子句後,至少有一段(去頭尾空白)以「請」起頭(與 F008 LAW-15 對 `renderStoreError` 同一個判準)。
  且 `TargetVaultAbsent` 與 `TargetNodeMissing` 兩種成因的訊息**不相等**。
  *非退化*:樣本要含兩種 `drReason` 各至少一筆。

**退化情形**

- **LAW-18**(空集合):`openVaultSet []` 回 `Right vs`,`vaultSetIds vs == []`,且對所有 `f` / `q` /
...
- **LAW-19**(單一 vault 退化成 F006 / F007):對所有 `f` 與 `q`,`openVaultSet [h]` 之後
  ...
  *非退化*:`h` 的索引非空,且 `q` 的樣本要含有文字條件與 `sqFacets = True` 各一組;`f` 的樣本要
  含 `nfIncludeReference = False` 且 `h` 底下有 reference pack 的一組(理由同 LAW-4)。

**新增的兩個 `StoreError` 建構子**

- **LAW-20**:`renderStoreError (TooManyVaults n limit)` 非空,含 `show n` 與 `show limit` 兩個數字的
  文字,且切成子句後至少有一段以「請」起頭;`renderStoreError (VaultIdCollision v p q)` 非空,含
  `v` 的文字與 `p`、`q` 兩個路徑字串,且同樣有以「請」起頭的子句。兩則訊息互不相等。
  (判準與 F008 LAW-15 相同:以 `;` / `;` / `,` / `,` / `。` 切子句,去頭尾空白後比對開頭。)
  *非退化*:`n /= limit` 且 `p /= q`——兩個數字相同、兩個路徑相同時,只印其中一個也會通過。

## Examples

> **Fixture 前提**(EX-1–EX-12 共用,**每一條都是斷言成立的必要條件,合成 fixture 時不得省略**):
>
...
> - **`ent-00000001` 在兩個 vault 都存在**(S3)。ADR-014 明訂短 id 只在 vault 內唯一,ADR-017 第一
>   條明訂 `kind` 是運維分界不是資料模型分界,所以 asset vault 裡有主題檔是合法的
> - **id 交錯**(S2):排序後是 `A:ent-…01` < `B:ent-…01` < `B:ent-…03` < `A:ent-…05` < `A:ent-…07`
> - **兩個 vault 各自都有一個 reference pack**(在自己的 `library/reference/` 底下)與它底下的
>   asset。這是 LAW-4 / LAW-19 的 `nfIncludeReference` 那一半成立的必要條件:reference 排除子句是整段
...
>   兩邊各有一個**只有自己有**的標籤(LAW-4 / EX-17 的 `nfTags` 那一半要靠它:另一處直接寫出表名的
>   地方是 `node_tags` 子查詢,只有一邊有標籤的話對錯實作給出相同答案),外加一個**兩邊都有**的
>   標籤 `"canon"`(LAW-12 的 facet 求和要靠它:兩邊沒有共同值的話,「求和」與「取其中一個」給出
>   相同答案)。兩個要求同時成立,缺一條都會讓對應的 law 變成恆真
> - **區辨用的標籤必須放在節層,不能放在檔案層**(2026-08-26 開發者裁決,GAP-19):`design.md` 的
...
>   否則 EX-17 的「只回一筆」與 LAW-4 非退化的「這組 `nfTags` 真的篩掉了東西」都會落空(實際踩過:
>   把 `tags: [琳達, canon]` 寫在 F-A 的檔案層,`nfTags = ["琳達"]` 回三筆而不是一筆)
> - **用來測「窄命中」的查詢字串,不得出現在同一 vault 其他節點的標題或正文裡**(同一次裁決):
>   EX-1 / EX-12 用的 `sqText = Just "藥水"` 只能命中 F-A 的 `ent-00000001` 與 F-B 的 `ast-00000002`
...
| # | 輸入 | 預期輸出 | 覆蓋的邊界 |
|---|---|---|---|
| EX-1 | `searchAcross vsAB (emptySearchQuery { sqText = Just "藥水" })` | `srHits` 恰兩筆:一筆 `shVault == VaultId "vlt-aaaa0001"` 且 `metaId (shMeta) == ent-00000001`,一筆 `shVault == VaultId "vlt-bbbb0002"` 且 `metaId (shMeta) == ast-00000002`;兩筆都 `shScore > 0`、`shSnippet` 含「藥水」、`metaVault (shMeta) == shVault`;`srTotal == 2` | **契約卡驗收 1**:一次回兩種 vault、每筆 `shVault` 正確。「恰兩筆」依賴 fixture 前提的**窄命中**約束:兩個 vault 裡除了這兩個節點,沒有別的節點的標題或正文含「藥水」 |
| EX-2 | `listAcross vsAB emptyNodeFilter { nfPrefixes = [PEnt], nfLimit = 10 }`,再取 `listAcross vsAB emptyNodeFilter { nfPrefixes = [PEnt], nfOffset = 1, nfLimit = 2 }` | 第一次依序是 `[(A, ent-…01), (B, ent-…01), (B, ent-…03), (A, ent-…05), (A, ent-…07)]`;第二次是 `[(B, ent-…01), (B, ent-…03)]` | **契約卡驗收 2**:排序與分頁跨 vault。「各自排完再接」會給 `[A01, A05, A07, B01, B03]`,第二次會是 `[(A, ent-…05), (A, ent-…07)]` —— 兩種作法在這裡給出完全不同的答案 |
| EX-3 | `lookupRef vsAB (VaultId "vlt-aaaa0001") (Ref Nothing (Id "ent-00000001"))` 與 `lookupRef vsAB (VaultId "vlt-bbbb0002") (Ref Nothing (Id "ent-00000001"))` | 前者 `Just (VaultId "vlt-aaaa0001", n)` 且 `metaTitle (anyMeta n) == "琳達的藥水日記"`;後者 `Just (VaultId "vlt-bbbb0002", n')` 且 `metaTitle (anyMeta n') == "B 庫的同號節點"` | **契約卡驗收 3**:不帶 vault 的 `Ref` 以呼叫端指定的預設 vault 解析(前提:同一短 id 在兩個 vault 都存在) |
| EX-4 | `lookupRef vsAB (VaultId "vlt-aaaa0001") (Ref (Just (VaultId "vlt-bbbb0002")) (Id "ent-00000001"))` | `Just (VaultId "vlt-bbbb0002", n')`,`metaTitle (anyMeta n') == "B 庫的同號節點"` | 帶 vault 的 `Ref` 覆蓋預設 vault |
| EX-5 | `lookupRef vsAB (VaultId "vlt-aaaa0001") (Ref (Just (VaultId "vlt-cccc0003")) (Id "ent-00000001"))`,其中 `vlt-cccc0003` 不在集合裡 | `Nothing` | 目標 vault 不在集合裡(id 本身在兩個 vault 都存在,所以 `Nothing` 只可能來自 vault 路由) |
| EX-6 | `openVaultSet` 對 11 個 `vmId` 兩兩相異的把手;再對其中任意 10 個 | 前者 `Left (TooManyVaults 11 10)`;後者 `Right vs` 且 `length (vaultSetIds vs) == 10` | **契約卡驗收 4**:第 11 個 vault 回 `TooManyVaults` 並列出 10 |
| EX-7 | `openVaultSet [hA, hB, hA2]`,其中 `hA2` 是對**同一個路徑** F-A 再 `openVault` 一次(`vmId` 與 `vhRoot` 都相同、`Connection` 不同) | `Right vs`,`vaultSetIds vs == [VaultId "vlt-aaaa0001", VaultId "vlt-bbbb0002"]` | 同一路徑重複傳 → 保序去重,同 vault 只留第一個 |
| EX-8 | F-A 的 `ent-00000001` 有三筆關聯:① `uses` → `vlt-bbbb0002:ast-00000002`(存在)② `references` → `ent-0000dead`(不帶 vault,F-A 裡沒這個 id)③ `derivedFrom` → `vlt-cccc0003:ent-00000001`(該 vault 不在集合裡)。呼叫 `checkReferences vsAB hA` | 恰兩筆:`DanglingRef ent-00000001 <②的 Link 原文> (Ref (Just (VaultId "vlt-aaaa0001")) (Id "ent-0000dead")) TargetNodeMissing` 與 `DanglingRef ent-00000001 <③的 Link 原文> (Ref (Just (VaultId "vlt-cccc0003")) (Id "ent-00000001")) TargetVaultAbsent`;①不出現 | **契約卡驗收 5**:兩種懸空都找得到,而且解析得到的那一筆不會被誤報 |
| EX-9 | 對 EX-8 的兩筆各呼叫 `renderDanglingRef` | 兩則訊息都非空、互不相等;各含 `"ent-00000001"`;第一則含 `"vlt-aaaa0001:ent-0000dead"`、第二則含 `"vlt-cccc0003:ent-00000001"`;兩則都有以「請」起頭的子句 | 錯誤/問題訊息的契約 G 判準 |
| EX-10 | `openVaultSet []` 之後 `listAcross vs emptyNodeFilter`、`searchAcross vs emptySearchQuery`、`lookupRef vs (VaultId "vlt-aaaa0001") (Ref Nothing (Id "ent-00000001"))`、`checkReferences vs hA` | `[]`;`SearchResult [] 0 Nothing`;`Nothing`;`checkReferences` 對 `hA` 的**每一筆**關聯各回一筆,`drReason` 全是 `TargetVaultAbsent`(以 EX-8 的 fixture 就是三筆) | 空集合不是錯誤;空集合下「本 vault 自己」也不在集合裡 |
...
> **EX-2 的「只看 `ent-`」為什麼寫進過濾器,而不是留在散文的括號裡**(2026-08-26 開發者裁決,GAP-18):
> 契約 F 的 `NodeFilter` 本來就有 `nfPrefixes :: [IdPrefix]`,「只看某個前綴」是**過濾器的語意**,
> 不是斷言的附註。把它留在括號裡會逼測試端自己先過濾再切窗,於是 `nfOffset` / `nfLimit`
> **根本沒被驗到**——EX-2 會退化成一條只測排序、不測分頁的 example,而契約卡驗收 2 明訂要驗
...
- **F007**:`SearchQuery` / `SearchHit` / `FacetCounts` / `SearchResult` / `search` / `emptySearchQuery`
  是 `searchAcross` 的形狀與對照基準(LAW-8 / LAW-10 / LAW-12 / LAW-19);`Aapms.Store.Tokenize` 的 `routeOf` /
...
| `closeVault :: VaultHandle -> IO ()` | `store/src/Aapms/Store/Marker.hs:192` | F005 | LAW-2:`closeVaultSet` 之後仍要能單獨關 |
...
| `renderStoreError :: StoreError -> Text` | `store/src/Aapms/Store/Error.hs:91` | F005 | 本次補上 `TooManyVaults` 與 `VaultIdCollision` 兩個分支(ASM-4 / ASM-5 裁決) |
...
| `listNodes :: VaultHandle -> NodeFilter -> IO [Meta]` | `store/src/Aapms/Store/Query.hs:240` | F006 | LAW-4 / LAW-19 的對照基準 |
| `lookupNode :: VaultHandle -> Id -> IO (Maybe AnyNode)` | `store/src/Aapms/Store/Query.hs:325` | F006 | LAW-13:`lookupRef` 路由到目標 vault 之後就是它 |
| `loadLinkGraph :: VaultHandle -> IO LinkGraph` | `store/src/Aapms/Store/Query.hs:525` | F006 | LAW-16:`checkReferences` 要走過的那一份關聯清單 |
...
| `data FacetCounts = FacetCounts { fcTypes, fcVaults, fcTags, fcOwners, fcLicenses :: [(Text, Int)] }` | `store/src/Aapms/Store/Query.hs:570` | F007 | LAW-12 |
...
| `search :: VaultHandle -> SearchQuery -> IO SearchResult` | `store/src/Aapms/Store/Query.hs:591` | F007 | LAW-8 / LAW-10 / LAW-12 / LAW-19 的對照基準 |
...
| `hydrateMeta :: VaultId -> Connection -> NodeRow -> IO Meta` | `store/src/Aapms/Store/Row.hs:239` | F006 | **LAW-7 / LAW-11 的關鍵**:第一個參數決定 `metaVault`,跨 vault 時必須是**該列真正來自的 vault** |
...
| `renderRef :: Ref -> Text` | `core/src/Aapms/Core/Id.hs:181` | F001 | LAW-17 / EX-9 的訊息內容 |
...
| `anyMeta :: AnyNode -> Meta` | `core/src/Aapms/Core/AnyNode.hs:28` | F001 | EX-3 / EX-4 的斷言 |
| `data Meta = Meta { metaId :: Id, metaVault :: VaultId, … }` | `core/src/Aapms/Core/Meta.hs:123` | F001 | `listAcross` 的輸出;`metaVault` 是 LAW-7 / LAW-11 的斷言對象 |
...
- **可否與其他進行中任務平行開發**:可以,但 WAVE-7 只有這一個 feature。它是 `aapms-store` 內部依賴圖的
  新葉子(除了門面之外沒有人 import 它),不動任何既有簽名

## 不可逆決定

- **DEC-1:跨 vault 的身分是 `(VaultId, Id)` 這一對,不是 `Id`**。`listAcross` 回 `[(VaultId, Meta)]`、
...
- **DEC-2:`VaultSet` 不接管 `VaultHandle` 的生命週期**(LAW-2)。`closeVaultSet` 只放掉自己持有的資源。
...
- **DEC-3:上限是 10 個 vault,超過即失敗,不做分批查詢**(LAW-1)。
...
> 每條前面加一行「**裁決**」;ASM-1 **部分推翻**、ASM-5 **推翻**,理由記在各自的裁決行裡。
...
- **ASM-1**:design.md「模組間公開介面」寫「MultiVault → Query:`*Across` 以同一組 SQL 片段對 UNION 後
  ...
  - 選項:
    ...
    b) **MultiVault 自己複製一份 WHERE 組裝**——當下成本:零,完全不動 `Query.hs`;三個月後的
       ...
       單 vault 與跨 vault 的過濾結果會**靜默分歧**——正是本子系統已被咬過兩次(GAP-2 / GAP-17)的
       那種缺陷形態。LAW-4 是為了抓它而寫的,但 law 抓得到的是「已經分歧了」,不是「不會分歧」
  ...
  - 傾向:**a**。理由:它是三者中唯一同時滿足「NodeFilter 語意只有一份」與「ADR-017 的 ATTACH
    ...
    這一波兩次最嚴重的資料遺失(GAP-2 / GAP-17)都是「同一件事有兩個地方在做,其中一個忘了跟上」,
...
- **ASM-2**:契約 E 只列了 `VaultSet` / `openVaultSet` / `lookupRef` / `listAcross` / `searchAcross` /
  ...
  就**沒有任何公開介面觀察得到去重與上限到底怎麼作用**,LAW-1 / EX-6 / EX-7 寫不出斷言。
  ...
  - 選項:
    ...
    c) **改讓 `openVaultSet` 回 `(VaultSet, [VaultId])`**——當下成本:不新增函式;三個月後的
       ...
       同一個事實有兩個持有者,違反知識歸屬
  - 傾向:**a**。`maxAttachedVaults` 尤其必要:上限這個數字若只寫在 `openVaultSet` 內部,LAW-1 就得
    把 `10` 這個字面量抄進測試,契約一改測試不會紅、只會靜默地測錯的東西。可逆性:
    **有條件可逆**——三條都是**新增**,拿掉才是破壞性的;條件是 `service` 尚未開始使用
    (S3 才會有消費者,現在拿掉零成本)
  - 暫採:a → 影響:若裁決是 b,骨架刪掉 `vaultSetIds` / `maxAttachedVaults` 兩條簽名,LAW-1 要改寫成
    只斷言 `TooManyVaults` 的兩個 `Int` 欄位、EX-7(去重)整條刪掉

- **【裁決:接受(選項 a),已回寫契約 E】**
  `data DanglingRef = DanglingRef { drSource :: Id, drLink :: Link, drTarget :: Ref, drReason :: DanglingReason }`、
  `data DanglingReason = TargetVaultAbsent | TargetNodeMissing`。落在 LAW-16 / LAW-17 與 EX-8 / EX-9。
- **ASM-3**:契約 E 只寫 `checkReferences :: VaultSet -> VaultHandle -> IO [DanglingRef]`,
  **`DanglingRef` 的形狀沒有定義**(build-log DEC-3 已記過這件事:新的 DTO 由對應 feature 依契約卡
  ...
  - 選項:
    a) **`{ drSource, drLink, drTarget, drReason }` 四欄 + 封閉的 `DanglingReason` 兩值**
       (見「不可逆決定」DEC-4 / DEC-5)——當下成本:四個欄位;三個月後的代價:`drLink` 與 `drTarget`
  ...
  - 傾向:**a**。`drLink` 與 `drTarget` 的重疊是刻意的:診斷這種問題時「檔案裡寫了什麼」與
    ...
    加欄位可逆,刪欄位不可逆;條件同 ASM-2(S3 之前沒有消費者)
  - 暫採:a → 影響:若裁決是 b/c,骨架的 `data DanglingRef` 改欄位,LAW-16 / LAW-17 與 EX-8 / EX-9 跟著改,
    其餘 law 不受影響
...
- **【裁決:接受(選項 a 的變體)——`Error.hs` 這一輪授權給本 feature 自己改】** 結論與選項 a 相同
  ...
  落在 LAW-1 / LAW-20 與 EX-6 / EX-14 / EX-15。
- **ASM-4**:`TooManyVaults` 依契約 G 必須是 `StoreError` 的**新建構子**(「`StoreError` 是
  `aapms-store` 的唯一錯誤型別,由各 feature 擴充,不得另立平行型別再橋接」),但
  `store/src/Aapms/Store/Error.hs` **不在本次的骨架檔案清單裡**。這與 F008 ASM-2 是同一個情況——
  ...
  - 選項:
    ...
    b) **本 feature 自己去改 `Error.hs`**——當下成本:零;三個月後的代價:違反委派的檔案清單
       (WAVE-7 只有一個 feature、沒有平行寫入的風險,但規則是規則,而且 `Error.hs` 是全套件共用檔)
    c) **另立 `MultiVaultError` 再橋接**——當下成本:零;三個月後的代價:**已被開發者推翻過一次**
       (F008 ASM-2),`service` 會看到兩種錯誤形狀、兩套 `render*`
  - 傾向:**a**。這一條沒有真正的取捨,只是「誰動手」;c 已有前例判死。可逆性:**可逆**
  - 暫採:a。骨架**不 import** `TooManyVaults`,所以現在編得過;LAW-1 / EX-6 在建構子落地前寫不出來
...
- **【裁決:推翻(提報時傾向 a「一律保序去重」)→ 依成因分成兩種**】 開發者指出提報**沒有把兩種
  ...
  資料問題咬過兩次(GAP-2 / GAP-17),兩次都不是測試抓到的。
...
- **ASM-5**:契約沒說 `openVaultSet` 收到**兩個 `vmId` 相同的把手**(同一個 vault 被 `openVault` 兩次)
  ...
  - 選項:
    a) **保序去重、同 `vmId` 只留第一個,上限以去重後的數量計**(LAW-1 / EX-7)——當下成本:一次
  ...
  - 傾向:**a**。c 直接排除(重複結果不會被任何現有斷言擋下);b 與 a 的差別只在「誰去重」,
    而 a 讓 `vaultSetIds` 成為唯一真相。可逆性:**可逆**(改的是 `openVaultSet` 的一行與 LAW-1 的措辭)
  - 暫採:a → 影響:若裁決是 b,LAW-1 的第二個子句改成「有重複 `vmId` 時回 `Left <新建構子>`」、
    EX-7 改成期望 `Left`,並要在 `Error.hs` 再加一個建構子(併進 ASM-4 一起處理)

- **【裁決:接受(選項 a)】** 同一個值跨 vault **求和**。落在 LAW-12 子句 2 與 EX-12。
- **ASM-6**:契約 F 的 `FacetCounts` 是為單一 vault 定的(F007 LAW-16:`fcVaults` 恰有一筆)。跨 vault 時
  ...
  - 層級自答:出現在邊界上?**會**(`searchAcross` 的回傳內容);改錯驚動其他模組?**要**
    (`service` 的側欄直接顯示它)
  - 選項:
    a) **同一個值跨 vault 求和**(LAW-12 子句 2):`type = "character"` 在 A 有 3 筆、B 有 2 筆 →
  ...
  - 傾向:**a**。b 被 ADR-014「不要把結構編進字串」的同一個理由擋掉;c 讓 facet 在預設路徑上消失。
    可逆性:**可逆**(改的是合併函式,型別不動)
  - 暫採:a → 影響:若裁決是 c,LAW-12 的子句 2 與 3 刪掉、EX-12 只驗 `fcVaults`
...
- **落地時發現、值得記下的一點**:授權原文是「匯出 `whereOf`」,但**照字面只匯出是不夠的**——
  ...
  兩處都做了與 `baseFrom` **同形**的前綴一般化,`""` 時輸出逐字不變。LAW-4 的非退化條件與 EX-16 / EX-17
  就是為了讓這件事在測試裡被抓到而寫的
...
結構約束,依 F007 GAP-3 與 F008 GAP-12 的裁決先例——文字掃描分不出語法結構,不寫成 law:
...
- **「`VaultSet` 只開讀取,任何寫入函式不接受它」**(契約卡驗收 6):這是**型別層級**的事實
  ...
  F007 GAP-3 / F008 GAP-12 兩度驗不準的作法。`/arch-audit` 檢查:`MultiVault.hs` 沒有 import
