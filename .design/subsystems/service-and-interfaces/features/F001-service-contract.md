...
created: 2026-08-16
updated: 2026-08-19
depends-on: [entity-graph-core/F002, entity-graph-core/F003, entity-graph-core/F004, entity-graph-core/F005]
related-adr: [ADR-002, ADR-003, ADR-005, ADR-006, ADR-008]
related-feature: []
---
...
ADR-006 的核心主張:**業務邏輯只存在於 `storyflow-service`,它是唯一的契約定義處**。
CLI(F002)、servant server(F003)、未來的 MCP adapter 全部是它的薄包裝。這一層
存在的價值不是「多一層」,而是**讓三種介面的行為由型別強制一致**——邏輯只有一份,不可能
悄悄長歪。

entity-graph-core/F005 完成後,`storyflow-store` 有了完整的讀寫能力,但它們是**落地操作**,不是業務操作。
兩者的差距是這一層要補的:
...
2. `service` 的測試不經過 CLI 也不經過 HTTP——用 `temporary` 建臨時 Vault,直接 hspec
   跑完整業務流程(ADR-006 的正面影響之一)
3. `service` 不 import 任何與終端輸出、HTTP、`optparse-applicative` 有關的東西;
   `storyflow-service.cabal` 的 `build-depends` 就是這條界線的證明
4. 樂觀鎖在每一個會改動既有實體的操作上都是**必填參數**,不是選配(ADR-006 的負面影響第三條:
   兩種模式下都必須生效)
5. 型別註冊表在執行期找得到——不是只有從原始碼目錄跑才行

明確**不做**的:conflict(P4)、workshop(P5)、LLM。它們在 ADR-006 的操作清單裡,但屬於
後續階段;本文檔 只做 P2 那一段。跨 Vault 的讀寫也不做,見下。

## 相依性

`depends-on: [entity-graph-core/F002, entity-graph-core/F003, entity-graph-core/F004, entity-graph-core/F005]`。前三者已 done,**entity-graph-core/F005 尚未
開工**,是本文檔 的唯一阻塞來源。

- **entity-graph-core/F005**:本文檔 的每一個寫入操作都直接包裝它的函式(`createEntityFile`、`addFragment`、
  `deleteEntity`、`addNode`、`removeNode`、`listLevels`、擴充後的 `writeEntityMeta` 等)。
  這些函式**目前不存在於程式碼中**,介面表裡對應的簽名是依 entity-graph-core/F005 的介面約定寫的,不是
  從原始碼讀出的——實作時若 entity-graph-core/F005 的簽名有偏差,本文檔 要跟著修
- **entity-graph-core/F004**:`Vault` 定位、`openVaultIndex`、既有的查詢函式、`StoreError`
- **entity-graph-core/F003**:`MetaOverride` 是 `writeEntityMeta` 的參數型別,service 的 patch 要組出它;
  `MdWarning` 隨寫入結果回傳
- **entity-graph-core/F002**:全部核心型別與 `checkEntity` / `TypeRegistry`

entity-graph-core/F001 建立的 `cabal.project` 要加一行 `service/`,但那是骨架的延伸、不是介面相依,
因此不列入 `depends-on`。

**可否平行開發**:不能與 entity-graph-core/F005 平行——本文檔 的實作面幾乎全部是對它的包裝。
F002(CLI)與 F003(server)都必須等本文檔,但**這兩支彼此可以平行**:
它們各自包裝同一組 `ServiceM` 函式,互不相干。

## 對應的 Level 2 契約

實作 `design.md`「對外契約」的內嵌那一組:`Env`、`ServiceM`、`openEnv`、`runService`、
23 個業務操作,以及 `errorCode` / `renderServiceError` 與 View、請求型別。對應資料流管線的
中段(自 Env 取相依 → 業務驗證 → 委派 store 落地 → 組 View)。CLI 與 REST 的形狀不在本規格內。
...
**沒有把時鐘放進 `Env`**:entity-graph-core/F004 的 `writeEntityMeta` 已經自己 `getCurrentTime`,
entity-graph-core/F005 的建立函式同理。把時鐘注入 service 卻不注入 store,等於只有一半可控,不如統一。
...
`Maybe Text` 是 `--vault <名稱>`,`FilePath` 是 cwd——兩者原樣轉給 `resolveVault`,ADR-008
...
`types/registry/*.toml` 隨程式碼版控(system.md),從原始碼目錄跑測試時相對路徑找得到,
...
| `LinkNotAllowed` | 警告,照寫 | ADR-005 明說自訂關聯合法,引擎當純標註儲存 |
...
**寫得進去、查得出來**——那是 entity-graph-core/F004 已經做完的事,本文檔 不動它。
...
`Maybe Int`,因為 ADR-006 明列「樂觀鎖在兩種模式下都必須生效,不能只在 server 端做」——
給一個「不帶就跳過檢查」的逃生口,CLI 一定會用它,然後遠端模式的並發保護就只剩一半。
CLI 的作法是**先讀再寫**(F002 的事),不是繞過。
...
`StoreFailed` 原樣包住 `StoreError` 而不逐個翻譯:entity-graph-core/F004 的 `renderStoreError` 每一則
...
`LevelTreeInvalid` 是**讀取**時才可能出現的:entity-graph-core/F005 的 `addNode` / `removeNode` 在寫檔前
就驗過樹,但作者可以直接編輯 Level 檔把樹改壞。`getLevel` 因此要能回報這件事,而不是崩潰。

## 使用到的既有串接介面

> entity-graph-core/F005 的介面**尚未實作**,下表中來源文檔 為 entity-graph-core/F005 的各列是依該 spec 的
> 「新增的介面」約定填寫,不是從原始碼讀出的簽名。

| 介面(含完整簽名) | 來源檔案 | 來源文檔 | 用途 |
|---|---|---|---|
| `data Meta = Meta { metaId :: Id, metaVault :: Text, metaType :: Text, metaTitle :: Text, metaSummary :: Text, metaTags :: [Text], metaStatus :: Status, metaTimeline :: Timeline, metaAliases :: [Text], metaLinks :: [Link], metaSource :: Source, metaRevision :: Int, metaCreated :: Day, metaUpdated :: Day }` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `listEntities` / `listLevels` 的回傳元素 |
| `data Entity = Entity { entMeta :: Meta, entBody :: Text }` | `core/src/StoryFlow/Core/Entity.hs` | entity-graph-core/F002 | `EntityView` 內層;`checkEntity` 的輸入 |
| `data Level = Level { lvlMeta :: Meta, lvlRoot :: Id }` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `LevelView` 內層 |
| `data Node = Node { nodMeta :: Meta, nodLevel :: Id, nodParent :: Maybe Id, nodOrder :: Int, nodKind :: NodeKind, nodEntities :: [Ref] }` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `buildTree` 的輸入 |
| `data NodeTree = NodeTree { ntNode :: Node, ntChildren :: [NodeTree] }` | `core/src/StoryFlow/Core/Tree.hs` | entity-graph-core/F002 | `LevelView` 回樹而不是扁平清單 |
| `buildTree :: Level -> [Node] -> Either [TreeError] NodeTree` | `core/src/StoryFlow/Core/Tree.hs` | entity-graph-core/F002 | `getLevel` 組樹;失敗回 `LevelTreeInvalid` |
| `data TreeError = MultipleRoots [Id] \| NoRoot \| OrphanNode Id Id \| Cycle [Id] \| DuplicateOrder Id Int [Id] \| DuplicateNodeId Id \| RootMismatch Id Id` | `core/src/StoryFlow/Core/Tree.hs` | entity-graph-core/F002 | `LevelTreeInvalid` 的酬載 |
| `data Link = Link { linkKind :: LinkKind, linkTarget :: Ref, linkNote :: Maybe Text }` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `addLink` 的參數 |
| `data Ref = Ref { refVault :: Maybe Text, refId :: Id }` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 跨 Vault 判定與 `DanglingLinkTarget` |
| `localRef :: Id -> Ref` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `linksOf` 的反向查詢參數 |
| `checkEntity :: TypeRegistry -> Entity -> [EntityWarning]` | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | 寫入前的型別驗證,依警告種類分流 |
| `data EntityWarning = MissingRequiredField Text Text \| LinkNotAllowed Text Text \| UnknownEntityType Text` | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | 分流的判斷依據與 `ValidationFailed` 的酬載 |
| `data EntityTypeSpec`(entity-graph-core/F005 擴充後含 `etsDir` / `etsOwnerType`) | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | `listEntityTypes` 的回傳;CLI 的 `--type` 選項來源 |
| `lookupType :: Text -> TypeRegistry -> Maybe EntityTypeSpec` | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | 建立時確認型別存在,不存在回 `UnknownType` |
| `listTypes :: TypeRegistry -> [EntityTypeSpec]` | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | `listEntityTypes` |
| `loadRegistry :: FilePath -> IO (Either [LoadError] TypeRegistry)` | `types/src/StoryFlow/Types/Loader.hs` | entity-graph-core/F002 | `openEnv` 載入註冊表;失敗即 `RegistryLoadFailed` |
| `data LoadError = TomlParseError FilePath Text \| MissingField FilePath Text \| BadFieldType FilePath Text Text \| UnknownKey FilePath Text \| RegistryDirMissing FilePath \| RegistryInvalid RegistryError`(`renderLoadError :: LoadError -> Text`) | `types/src/StoryFlow/Types/Loader.hs` | entity-graph-core/F002 | `RegistryLoadFailed` 的酬載與訊息。**注意它已有一個 `RegistryInvalid` 建構子**,`ServiceError` 因此不可同名 |
| `data MetaOverride`(13 個 `Maybe` 欄位) | `md/src/StoryFlow/Md/Inherit.hs` | entity-graph-core/F003 | `EntityPatch` 轉成它交給 `writeEntityMeta` |
| `emptyOverride :: MetaOverride` | `md/src/StoryFlow/Md/Inherit.hs` | entity-graph-core/F003 | patch 的起點 |
| `data MdWarning = MissingSummary Id \| CustomLinkKind Id Text \| EmptyBody Id`(`renderMdWarning :: MdWarning -> Text`) | `md/src/StoryFlow/Md/Error.hs` | entity-graph-core/F003 | 併進 `evWarnings` |
| `data Vault = Vault { vaultName :: Text, vaultRoot :: FilePath, vaultCfg :: VaultConfig }` | `store/src/StoryFlow/Store/Vault.hs` | entity-graph-core/F004 | `Env` 的欄位;`VaultView` 的來源 |
| `resolveVault :: Maybe Text -> FilePath -> IO (Either StoreError Vault)` | `store/src/StoryFlow/Store/Vault.hs` | entity-graph-core/F004 | `openEnv` 的第一步,ADR-008 的規則不重寫 |
| `initVault :: FilePath -> Text -> IO (Either StoreError Vault)` | `store/src/StoryFlow/Store/Vault.hs` | entity-graph-core/F004 | `createVault` |
| `loadVaultRegistry :: IO (Either StoreError (Map Text FilePath))` | `store/src/StoryFlow/Store/Vault.hs` | entity-graph-core/F004 | `listVaults`:全域註冊表的名稱 → 路徑 |
| `registryPath :: IO FilePath` | `store/src/StoryFlow/Store/Vault.hs` | entity-graph-core/F004 | `RegistryUnavailable` 的訊息要說出去哪裡找過 |
| `openVaultIndex :: Vault -> IO (Either StoreError (Connection, [IndexIssue]))` | `store/src/StoryFlow/Store/Index.hs` | entity-graph-core/F004 | `openEnv` 開索引並順手補過時檔案 |
| `closeIndex :: Connection -> IO ()` | `store/src/StoryFlow/Store/Schema.hs` | entity-graph-core/F004 | `closeEnv` |
| `rebuildIndex :: Connection -> Vault -> IO (Either StoreError [IndexIssue])` | `store/src/StoryFlow/Store/Index.hs` | entity-graph-core/F004 | `reindex` |
| `refreshStale :: Connection -> Vault -> IO (Either StoreError [IndexIssue])` | `store/src/StoryFlow/Store/Index.hs` | entity-graph-core/F004 | `refreshIndex` |
| `data IndexIssue = IndexIssue FilePath [MdError] [MdWarning]`(`issueHasError`, `renderIndexIssue`) | `store/src/StoryFlow/Store/Index.hs` | entity-graph-core/F004 | `IndexReport` 的酬載 |
| `data EntityFilter = EntityFilter { efType :: Maybe Text, efStatus :: Maybe Status, efTag :: Maybe Text, efLimit :: Maybe Int }` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | 直接沿用,不重造過濾條件 |
| `emptyFilter :: EntityFilter` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | 預設過濾條件 |
| `lookupEntity :: Connection -> Id -> IO (Maybe Entity)` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `getEntity`;`addLink` 的目標存在性檢查 |
| `listEntities :: Connection -> EntityFilter -> IO [Meta]` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `listEntities` |
| `lookupLevel :: Connection -> Id -> IO (Maybe (Level, [Node]))` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `getLevel`,取出後餵 `buildTree` |
| `linksFrom :: Connection -> Id -> IO [Link]` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `linksOf` 的正向 |
| `linksTo :: Connection -> Ref -> IO [(Id, Link)]` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `linksOf` 的反向 |
| `searchEntities :: Connection -> Text -> EntityFilter -> IO [(Meta, Text)]` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `searchEntity`;`Text` 是 snippet |
| `data StoreError = VaultNotFound Text \| VaultConfigInvalid FilePath Text \| VaultAlreadyExists FilePath \| EntityNotFound Id \| StaleRevision Id Int Int \| IdCollision IdPrefix \| FileReadFailed FilePath Text \| FileWriteFailed FilePath Text \| IndexUpdateFailed FilePath Text \| ParseFailed FilePath [MdError] \| SqliteError Text`(entity-graph-core/F005 再加九個、刪 `FrontmatterWriteUnsupported`;`renderStoreError :: StoreError -> Text`) | `store/src/StoryFlow/Store/Error.hs` | entity-graph-core/F004 | `StoreFailed` 包住它,訊息委派不重寫 |
| `data WriteResult = WriteResult { wrNewRevision :: Int, wrPath :: FilePath }` | `store/src/StoryFlow/Store/Write.hs` | entity-graph-core/F004 | 寫入後拿新 revision 組 View |
| `writeEntityMeta :: Connection -> Vault -> Id -> Int -> (MetaOverride -> MetaOverride) -> IO (Either StoreError WriteResult)`(entity-graph-core/F005 擴充為支援主體) | `store/src/StoryFlow/Store/Write.hs` | entity-graph-core/F004 | `updateEntity` |
| `createEntityFile :: Connection -> Vault -> TypeRegistry -> NewEntity -> IO (Either StoreError CreateResult)` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | `createEntity` |
| `createLevelFile :: Connection -> Vault -> NewLevel -> IO (Either StoreError CreateResult)` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | `createLevel` |
| `addFragment :: Connection -> Vault -> Id -> Int -> NewFragment -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | `addFragment` |
| `deleteEntity :: Connection -> Vault -> Id -> Int -> DeleteMode -> IO (Either StoreError DeleteResult)` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | `deleteEntity` |
| `deleteLevel :: Connection -> Vault -> Id -> Int -> DeleteMode -> IO (Either StoreError DeleteResult)` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | `deleteLevel` |
| `data NewEntity` / `NewFragment` / `NewLevel` / `NewNode` / `CreateResult` / `DeleteResult` / `DeleteMode` | `store/src/StoryFlow/Store/Create.hs` | entity-graph-core/F005 | 請求型別轉成它們;`Bool` 的 force 轉成 `DeleteMode` |
| `writeEntityBody :: Connection -> Vault -> Id -> Int -> Text -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Write.hs` | entity-graph-core/F005 | `setEntityBody` |
| `addEntityLink :: Connection -> Vault -> Id -> Int -> Link -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Write.hs` | entity-graph-core/F005 | `addLink` |
| `removeEntityLink :: Connection -> Vault -> Id -> Int -> LinkKind -> Ref -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Write.hs` | entity-graph-core/F005 | `removeLink` |
| `addNode :: Connection -> Vault -> Id -> Int -> NewNode -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Node.hs` | entity-graph-core/F005 | `addNode` |
| `removeNode :: Connection -> Vault -> Id -> Int -> DeleteMode -> IO (Either StoreError WriteResult)` | `store/src/StoryFlow/Store/Node.hs` | entity-graph-core/F005 | `removeNode` |
| `listLevels :: Connection -> EntityFilter -> IO [Meta]` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F005 | `listLevels` |
| `lookupDir :: Text -> TypeRegistry -> Maybe Text` | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F005 | 建立前確認型別有宣告目錄,否則訊息比 `RegistryDirUnknown` 更早給出 |
...
`StoryFlow.Service.Json`——F003 的 servant API 型別直接吃它們,不再定義第二套編碼。
...
| T17 | `service/test/.../EndToEndSpec.hs` → `純 service 從零建出琳達與教室` | 臨時 Vault → `createVault` → 建主體與兩個片段、建 Level 與六個 Node、掛上 `involves` 關聯 → `getLevel` 的樹形狀與 system.md 的圖相符 → `reindex` 後全部查詢結果不變 |

## 實作備註

寫這份規格時 entity-graph-core/F005 還沒開工,介面表裡來源文檔 為 entity-graph-core/F005 的每一列都是依約定填的。
實作時逐個對照原始碼,以下是與規格不同的地方,一律**以程式碼為準**。

1. **`addFragment` / `addNode` 回 `CreateResult` 而不是 `WriteResult`**(entity-graph-core/F005 實作時就
...
2. **`epTitle` 在落地層原本沒有路可走**,補了兩個函式(已回寫 entity-graph-core/F003 / entity-graph-core/F005):
   ...
   system.md 選 `updateFrontmatter :: (Meta -> Meta) -> ...` 的理由正是「改標題是檔案層
...
6. **`createVault` 會把 Vault 登記進全域註冊表**,並因此在 `store` 補了
   ...
   (只**追加一行**,不重寫整份檔案,保住 ADR-008 要的「可手寫」)。原本沒有任何人負責寫
   `vaults.toml`,ADR-008 的第一段規則(`--vault <名稱>` 查全域註冊表)就永遠命不中。
...
13. **已知限制,留給 F002**:`IndexReport.irIssues` 是已 render 的 `[Text]`,錯誤與品質
