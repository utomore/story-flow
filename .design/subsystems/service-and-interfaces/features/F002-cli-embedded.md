...
created: 2026-08-16
updated: 2026-08-19
depends-on: [entity-graph-core/F002, entity-graph-core/F004, F001]
related-adr: [ADR-005, ADR-006, ADR-008, ADR-009]
related-feature: []
---
...
ADR-006 的雙模式決策裡,本文檔 只做**內嵌**那一半——直接呼叫 `storyflow-service` 的
`ServiceM` 函式,不需要 daemon、啟動即用、堆疊追蹤完整。

`--remote` 那一半刻意留給 F003:它要走 `servant-client`,而 servant 的 API 型別在
F003 才定義。硬要在本文檔 先做,就得先發明一份 API 型別再讓 F003 去對齊它——
契約會有兩個定義處,正是 ADR-006 要避免的事。system.md 的階段表也把
「CLI `--remote`」排在 P3。
...
1. 只用 `story-flow` 指令,從 `vault init` 開始建出 system.md 範例的琳達
...
- **F001**(`storyflow-service`):唯一的業務入口。本文檔 的每個子指令都是對某個
  `ServiceM` 函式的包裝。(實作時 F001 已 `done`,阻塞解除)
- **entity-graph-core/F004**:`EntityFilter` 的四個欄位直接對應到 `--type` / `--status` / `--tag` /
  `--limit` 四個選項,CLI 自己組出它
- **entity-graph-core/F002**:`parseId` / `parseStatus` / `parseLinkKind` / `parseNodeKind` / `parseRef`
  是引數解析的驗證函式;`Meta` / `Entity` / `NodeTree` 是輸出渲染的對象

**沒有相依 entity-graph-core/F003 與 entity-graph-core/F005**:CLI 不 import `storyflow-md` 也不 import
`storyflow-store`。`MdWarning` 在 `EntityView` 裡已經被 service render 成 `Text`,CLI 拿到的
是字串;落地層的一切都在 `service` 後面。這不是巧合,是驗收標準 3 要守的界線。

**可否平行開發**:與 F003 **可以平行**——兩者各自包裝同一組 `ServiceM` 函式,互不相干。
唯一的接觸點是 F003 要回頭在本文檔 交付的 CLI 上加 `--remote`,那是 F003 的工作,
不構成本文檔 對它的相依。兩者都必須等 F001。

## 對應的 Level 2 契約

實作 `design.md`「對外契約」的 CLI 那一組(指令形狀、`--json` 統一信封、exit code 約定)
與「模組間公開介面」的 `storyflow-cli` → `storyflow-service`(`Backend` 的 `Embedded`
建構子),對應資料流管線兩端的「CLI 參數解碼」與「渲染」。不新增業務操作——23 個操作全部
沿用 `ServiceM` 既有定義。
...
關聯(ADR-005 明說自訂關聯合法,引擎當純標註儲存)。CLI 能做的只有**提示**——
...
service 的每個修改操作都要 expected revision(F001 刻意設成必填)。CLI 的填法:
...
先讀再寫之間有毫秒級窗口,與 entity-graph-core/F004 已明確接受的競態(重讀與 rename 之間)是同一種,
...
(entity-graph-core/F005 的 `addNode` / `removeNode` 就是這麼定的),不是節點自己的。
...
- `entity list` / `search`:對齊的表格,欄位 `id | type | status | title | summary`;
  `search` 多一欄 snippet
- `level show`:**樹狀圖**,形狀與 system.md 的場景樹圖一致
...
樹的繪製吃 `LevelView` 的 `lvTree`(`NodeTree`),不自己從扁平清單重建——F001 特意回樹
就是為了這件事。
...
- `--vault <名稱>`:原樣傳給 `openEnv`,ADR-008 的解析規則不在 CLI 重寫
- `--json`:如上
- 沒有 `--verbose` / `--quiet`:目前沒有需要分級的輸出;真需要時再加

`vault init` 與 `vault list` 在 `Env` 存在之前就要能跑,所以它們走 F001 的
...
> 來源文檔 為 F001 的各列,原先是依該 spec 的「新增的介面」約定填寫的。實作時
> `storyflow-service` 已經存在,逐條對照過原始碼——有出入的四條記在「實作備註」1,
> 本表其餘各列與 `service/src/` 相符。

| 介面(含完整簽名) | 來源檔案 | 來源文檔 | 用途 |
|---|---|---|---|
| `data Meta = Meta { metaId :: Id, metaVault :: Text, metaType :: Text, metaTitle :: Text, metaSummary :: Text, metaTags :: [Text], metaStatus :: Status, metaTimeline :: Timeline, metaAliases :: [Text], metaLinks :: [Link], metaSource :: Source, metaRevision :: Int, metaCreated :: Day, metaUpdated :: Day }` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `entity list` 的表格欄位;`--revision` 未給時取 `metaRevision` |
| `data Entity = Entity { entMeta :: Meta, entBody :: Text }` | `core/src/StoryFlow/Core/Entity.hs` | entity-graph-core/F002 | `entity show` 的渲染對象 |
| `data NodeTree = NodeTree { ntNode :: Node, ntChildren :: [NodeTree] }` | `core/src/StoryFlow/Core/Tree.hs` | entity-graph-core/F002 | `level show` 的樹狀輸出,直接遞迴它 |
| `data Node = Node { nodMeta :: Meta, nodLevel :: Id, nodParent :: Maybe Id, nodOrder :: Int, nodKind :: NodeKind, nodEntities :: [Ref] }` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | 樹的每一行要印 id / kind / summary |
| `parseId :: Text -> Either IdError (IdPrefix, Id)` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 定址第一步:引數是不是 id 格式 |
| `idPrefix :: Id -> IdPrefix` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `currentRevision` 決定去問 `getEntity` 還是 `getLevel` |
| `renderId :: Id -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | 所有輸出裡的 id |
| `parseRef :: Text -> Either IdError Ref` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `--target` 支援 `<vault>:<id>` 寫法 |
| `renderRef :: Ref -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `link list` 的輸出 |
| `parseStatus :: Text -> Either MetaError Status` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `--status` 的驗證 |
| `renderStatus :: Status -> Text` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | 表格的 status 欄 |
| `parseSource :: Text -> Either MetaError Source` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `--source` 的驗證 |
| `parseLinkKind :: Text -> LinkKind` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `--kind` 的轉換。**它是全函式,不會失敗**——自訂關聯一律合法(ADR-005),所以 CLI 沒有「驗證 kind」這個步驟 |
| `isCoreKind :: LinkKind -> Bool` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | 判斷 `--kind` 是否落在引擎認得的詞彙內,決定要不要提示 |
| `suggestCoreKind :: Text -> Maybe LinkKind` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | 非核心關聯時提示最接近的核心關聯(`Just` 才印) |
| `coreLinkKinds :: [LinkKind]` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `link add --kind` 的 `--help` 列出核心關聯詞彙 |
| `renderLinkKind :: LinkKind -> Text` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `link list` 的輸出 |
| `parseNodeKind :: Text -> Either LevelError NodeKind` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `--kind`(Node)的驗證 |
| `renderNodeKind :: NodeKind -> Text` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `level show` 每一行的 kind |
| `allNodeKinds :: [NodeKind]` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `node add --kind` 的 `--help` 列出全部合法值 |
| `data Timeline = Timeline { tlLabel :: Maybe Text, tlOrder :: Maybe Int }` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `--timeline` 與 `--order` 兩個選項組成它 |
| `data Link = Link { linkKind :: LinkKind, linkTarget :: Ref, linkNote :: Maybe Text }` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `--link <kind>:<target>[:<note>]` 解析後的產物 |
| `data EntityTypeSpec`(entity-graph-core/F005 擴充後含 `etsDir` / `etsOwnerType`) | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | `type list` 的輸出:key、名稱、必填欄位、允許的關聯 |
| `ToJSON` / `FromJSON` 實例(`Id` / `Ref` / `Meta` / `Entity` / `Level` / `Node` / `Link` / `Status` / `Source` / `Timeline` / `NodeKind` / `LinkKind`) | `core/src/StoryFlow/Core/Json.hs` | entity-graph-core/F002 | `--json` 的 `data` 欄位序列化 |
| `data EntityFilter = EntityFilter { efType :: Maybe Text, efStatus :: Maybe Status, efTag :: Maybe Text, efLimit :: Maybe Int }` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | `--type` / `--status` / `--tag` / `--limit` 四個選項直接組成它 |
| `emptyFilter :: EntityFilter` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | 沒給任何過濾選項時的預設值 |
| `withEnv :: Maybe Text -> FilePath -> (Env -> IO a) -> IO (Either ServiceError a)` | `service/src/StoryFlow/Service/Monad.hs` | F001 | 除 `vault init` / `vault list` 外每個子指令的外框 |
| `runService :: Env -> ServiceM a -> IO (Either ServiceError a)` | `service/src/StoryFlow/Service/Monad.hs` | F001 | 在 `withEnv` 內執行業務操作 |
| `openEnv :: Maybe Text -> FilePath -> IO (Either ServiceError (Env, [IndexIssue]))` | `service/src/StoryFlow/Service/Monad.hs` | F001 | 需要拿 `[IndexIssue]` 印警告時直接用它而非 `withEnv` |
| `data ServiceError`(`renderServiceError :: ServiceError -> Text`) | `service/src/StoryFlow/Service/Error.hs` | F001 | `message` 欄位與人類可讀模式的錯誤輸出 |
| `errorCode :: ServiceError -> Text` | `service/src/StoryFlow/Service/Error.hs` | F001 | 信封的 `code`。**定義在 service 而非 CLI**:server 與未來的 MCP 要用同一套代碼 |
| `createVault :: FilePath -> Text -> IO (Either ServiceError VaultView)` | `service/src/StoryFlow/Service.hs` | F001 | `vault init` |
| `listVaults :: IO (Either ServiceError [VaultView])` | `service/src/StoryFlow/Service.hs` | F001 | `vault list` |
| `vaultInfo :: ServiceM VaultView` | `service/src/StoryFlow/Service.hs` | F001 | `vault info` |
| `reindex :: ServiceM IndexReport` / `refreshIndex :: ServiceM IndexReport` | `service/src/StoryFlow/Service.hs` | F001 | `index rebuild` / `index refresh` |
| `listEntityTypes :: ServiceM [EntityTypeSpec]` | `service/src/StoryFlow/Service.hs` | F001 | `type list` |
| `createEntity :: NewEntityReq -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `entity new` |
| `addFragment :: Id -> NewFragmentReq -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `entity add` |
| `getEntity :: Id -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `entity show`;先讀再寫的「先讀」 |
| `listEntities :: EntityFilter -> ServiceM [Meta]` | `service/src/StoryFlow/Service.hs` | F001 | `entity list`;標題定址的候選來源 |
| `searchEntity :: Text -> EntityFilter -> ServiceM [SearchHit]` | `service/src/StoryFlow/Service.hs` | F001 | `search` |
| `updateEntity :: Id -> Int -> EntityPatch -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `entity set` |
| `setEntityBody :: Id -> Int -> Text -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `entity set-body` |
| `deleteEntity :: Id -> Int -> Bool -> ServiceM DeleteReport` | `service/src/StoryFlow/Service.hs` | F001 | `entity rm`,`Bool` 來自 `--force` |
| `addLink :: Id -> Int -> Link -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `link add` |
| `removeLink :: Id -> Int -> LinkKind -> Ref -> ServiceM EntityView` | `service/src/StoryFlow/Service.hs` | F001 | `link rm` |
| `linksOf :: Id -> ServiceM LinkReport` | `service/src/StoryFlow/Service.hs` | F001 | `link list`,正反向一次印完 |
| `createLevel :: NewLevelReq -> ServiceM LevelView` | `service/src/StoryFlow/Service.hs` | F001 | `level new` |
| `getLevel :: Id -> ServiceM LevelView` | `service/src/StoryFlow/Service.hs` | F001 | `level show`;node 操作的「先讀」 |
| `listLevels :: EntityFilter -> ServiceM [Meta]` | `service/src/StoryFlow/Service.hs` | F001 | `level list`;Level 的標題定址 |
| `deleteLevel :: Id -> Int -> Bool -> ServiceM DeleteReport` | `service/src/StoryFlow/Service.hs` | F001 | `level rm` |
| `addNode :: Id -> Id -> Int -> NewNodeReq -> ServiceM LevelView`(Level、父 Node、expected) | `service/src/StoryFlow/Service.hs` | F001 | `node add`。多出來的 Level 參數見實作備註 1 |
| `removeNode :: Id -> Id -> Int -> Bool -> ServiceM LevelView` | `service/src/StoryFlow/Service.hs` | F001 | `node rm` |
| `evId` / `evRevision` / `lvId` / `lvRevision` | `service/src/StoryFlow/Service/Types.hs` | F001 | 先讀再寫時從 View 取 id 與 revision,不自己往下鑽三層 |
| `renderIndexIssue :: IndexIssue -> Text` | `store/src/StoryFlow/Store/Index.hs`(由 service 重新匯出) | entity-graph-core/F004 | 每個指令開頭印到 stderr 的索引警告 |
| `data EntityView` / `LevelView` / `VaultView` / `SearchHit` / `LinkReport` / `IndexReport` / `DeleteReport` | `service/src/StoryFlow/Service/Types.hs` | F001 | 兩種輸出模式的渲染對象 |
| `data NewEntityReq` / `NewFragmentReq` / `NewLevelReq` / `NewNodeReq` / `EntityPatch`(`emptyPatch`) | `service/src/StoryFlow/Service/Types.hs` | F001 | 選項解析後組出來的請求 |
| `ToJSON` 實例(上述 View 與請求型別) | `service/src/StoryFlow/Service/Json.hs` | F001 | `--json` 的 `data`,CLI 不重新編碼 |
...
renderLevelTree:: LevelView -> Text     -- ASCII 樹,形狀與 system.md 的圖一致
...
- [x] T5: `renderLevelTree`:ASCII 樹,遞迴 `NodeTree`,分支字元與 system.md 的圖一致  `dep: T4`
...
| T5 | `cli/test/.../TreeSpec.hs` → `level show 的樹形狀與架構文件一致` | 用 system.md 教室場景的六個 Node 建樹,斷言輸出逐行等於預期字串(含 `└─` / `├─` / `│` 的位置) |
...
| T15 | `cli/test/.../EndToEndSpec.hs` → `純 CLI 從零建出琳達與教室` | 只用 `runCli` 依序下 `vault init` → `entity new` → `entity add` ×2 → `link add` → `level new` → `node add` ×5,最後 `level show` 的輸出與 system.md 的樹圖一致 |

## 實作備註

### 1. 規格寫在 `service` 實作之前,四條簽名對不上

本文檔 的「使用到的既有串接介面」是照 F001 的約定填的,實作時逐條對照原始碼,
四條與實際不同,一律**以原始碼為準**:
...
這是對已 `done` 的 entity-graph-core/F002 做的追加,純新增、不改既有行為。
...
- **`entity new --link` 不驗目標存在**:service 的 `requireTargetExists` 只掛在 `addLink`,
  ...
  行為不一致。現況以測試釘住(`EntityNewSpec`),要改的話該改 F001
...
| 驗收標準 | 結果 |
|---|---|
| 1. 純 CLI 建出琳達與教室,樹形狀與 system.md 一致 | ✅ `EndToEndSpec`:`vault init` → `entity new` → `entity add` ×2 → `link add` → `level new` → `node add` ×5 → `level show` 七行,分支字元逐項相符 |
