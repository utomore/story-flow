...
created: 2026-08-16
updated: 2026-08-19
depends-on: [entity-graph-core/F002, entity-graph-core/F004, entity-graph-core/F005, F001, F002]
related-adr: [ADR-002, ADR-003, ADR-006, ADR-008]
related-feature: []
---

# F003: servant REST API 與遠端模式

## 功能概述

P3 的完成標準是**「claude code 只靠 API 文件就能建/查片段與關聯」**。這需要三件東西一起到位:
一組覆蓋 `service` 全部操作的 REST API、一份從型別自動推導的 OpenAPI 文件、以及 ADR-006
承諾的 CLI `--remote`。三者共用同一份 servant API 型別——這正是 ADR-006 選 servant 的理由:
...
3. `storyflow-cli` 的 `--remote <url>` —— F002 刻意留下的另一半
...
誰也不必知道對方存在——system.md 架構圖裡 server 與 cli 平行的關係因此得以維持。
這是本文檔 對 system.md 的唯一結構性新增。

驗收標準:

1. servant API 覆蓋 F001 `ServiceM` 的**每一個**操作,沒有只有 CLI 做得到的事
...
- **F001**:API 的每一條路由都對應一個 `ServiceM` 函式;請求與 View 型別、`ServiceError`、
  `errorCode` 全部來自它。**尚未開工**,是主要阻塞來源
- **F002**:本文檔 要在它交付的 CLI 上長出 `--remote`——`GlobalOpts` 加欄位、
  `runCli` 的分派多一條路徑、`Command` 的每個建構子多一個遠端執行路徑。沒有那份 CLI
  就沒有東西可以改
- **entity-graph-core/F005**:狀態碼對照表**逐個列舉** `StoreError` 的建構子,其中 `ReferencedBy` /
  ...
  `ServiceError` 裡看到它們
- **entity-graph-core/F004**:`EntityFilter` 是查詢路由的 query parameter 來源;`StoreError` 的其餘建構子
- **entity-graph-core/F002**:核心型別與它們的 aeson 實例;OpenAPI 要為同一批型別補 `ToSchema`

**沒有相依 entity-graph-core/F003**:與 F002 同理,server 在 `service` 之後,碰不到 md。

**可否平行開發**:不能與 F002 平行——`--remote` 是長在它上面的。與 F001 更不能。
本文檔 是 0005 → 0006 → 0007 → 0008 這條鏈的最後一環。

若要縮短關鍵路徑,可行的切法是把本文檔 的第 1 部分(`storyflow-api` 型別)提前到
F002 完成前開工——它只依賴 F001 的型別。但 `--remote` 那一段仍然要等。

## 對應的 Level 2 契約

實作 `design.md`「對外契約」的 REST 那一組(14 條路徑 / 23 個 operation、必填 `revision`、
錯誤 body 形狀、`storyFlowOpenApi`),以及「模組間公開介面」的 server → service 與
cli → api 兩條。對應資料流管線的 REST 半邊,加上遠端 CLI 從 `Backend` 分出去再回到同一條路
的那一跳。業務語意完全沿用 `ServiceM`,本規格不新增任何業務判斷。
...
選配。理由與 F001 相同:ADR-006 明列樂觀鎖在兩種模式下都要生效,而遠端模式恰恰是
...
`GET /vaults` 與 `POST /vaults` 在**沒有目前 Vault 的情況下**也要能跑(它們對應 F001
...
`IndexUpdateFailed` 是唯一需要解釋的:**檔案已經寫成功了**,只有索引沒跟上(ADR-002:
...
`code` 來自 F001 的 `errorCode`,與 CLI `--json` 用的**是同一個函式**。這是三種介面
錯誤語彙一致的唯一保證。
...
- **綁非 loopback 時強制要求 token**:沒設就拒絕啟動並說明原因,不是印個警告了事。
  ADR-006 只要求「明確加旗標並顯示警告」,本文檔 收得更緊——警告會被忽略,而
...
回的都是 F001 的 View 型別:內嵌路徑直接拿到,遠端路徑由 `servant-client` 依同一份
型別解碼出來。

**遠端模式的「先讀再寫」**:F002 的邏輯原樣適用,只是 `getEntity` 換成 HTTP GET。
...
> 來源文檔 為 F001 / F002 的各列,原先是依那兩份文檔 的「新增的介面」約定填寫的。
> 實作時兩個套件都已經存在,逐條對照過原始碼——有出入的記在「實作備註」,
> 其中最重要的是 `parseId` 回 `(IdPrefix, Id)`、`addNode` / `removeNode` 多一個 Level 參數、
> 以及 `GlobalOpts` / `CliError` / `resolveEntity` 的實際形狀(F002 已經與規格不同)。

| 介面(含完整簽名) | 來源檔案 | 來源文檔 | 用途 |
|---|---|---|---|
| `data Meta = Meta { metaId :: Id, metaVault :: Text, metaType :: Text, metaTitle :: Text, metaSummary :: Text, metaTags :: [Text], metaStatus :: Status, metaTimeline :: Timeline, metaAliases :: [Text], metaLinks :: [Link], metaSource :: Source, metaRevision :: Int, metaCreated :: Day, metaUpdated :: Day }` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | 清單路由的回應元素;要補 `ToSchema` |
| `data Entity = Entity { entMeta :: Meta, entBody :: Text }` | `core/src/StoryFlow/Core/Entity.hs` | entity-graph-core/F002 | `EntityView` 內層;要補 `ToSchema` |
| `data Level = Level { lvlMeta :: Meta, lvlRoot :: Id }` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | `LevelView` 內層;要補 `ToSchema` |
| `data Node = Node { nodMeta :: Meta, nodLevel :: Id, nodParent :: Maybe Id, nodOrder :: Int, nodKind :: NodeKind, nodEntities :: [Ref] }` | `core/src/StoryFlow/Core/Level.hs` | entity-graph-core/F002 | 同上 |
| `data NodeTree = NodeTree { ntNode :: Node, ntChildren :: [NodeTree] }` | `core/src/StoryFlow/Core/Tree.hs` | entity-graph-core/F002 | `LevelView` 的樹;`ToSchema` 是遞迴 schema,要顯式命名避免無限展開 |
| `data Link = Link { linkKind :: LinkKind, linkTarget :: Ref, linkNote :: Maybe Text }` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `POST /entities/:id/links` 的 body |
| `data Ref = Ref { refVault :: Maybe Text, refId :: Id }` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `DELETE .../links` 的 `target` query param |
| `parseId :: Text -> Either IdError Id` / `renderId :: Id -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `Id` 的 `FromHttpApiData` / `ToHttpApiData`(capture 段) |
| `parseRef :: Text -> Either IdError Ref` / `renderRef :: Ref -> Text` | `core/src/StoryFlow/Core/Id.hs` | entity-graph-core/F002 | `Ref` 的 `FromHttpApiData` / `ToHttpApiData` |
| `parseStatus :: Text -> Either MetaError Status` / `renderStatus :: Status -> Text` | `core/src/StoryFlow/Core/Meta.hs` | entity-graph-core/F002 | `?status=` query param 的編解碼 |
| `parseLinkKind :: Text -> LinkKind` / `renderLinkKind :: LinkKind -> Text` | `core/src/StoryFlow/Core/Link.hs` | entity-graph-core/F002 | `?kind=` query param;**全函式,不會失敗** |
| `data EntityTypeSpec`(entity-graph-core/F005 擴充後含 `etsDir` / `etsOwnerType`) | `core/src/StoryFlow/Core/Registry.hs` | entity-graph-core/F002 | `GET /types` 的回應 |
| `ToJSON` / `FromJSON` 實例(`Id` / `Ref` / `Meta` / `Entity` / `Level` / `Node` / `Link` / `Status` / `Source` / `Timeline` / `NodeKind` / `LinkKind`) | `core/src/StoryFlow/Core/Json.hs` | entity-graph-core/F002 | 全部 body 的編解碼;`ToSchema` 必須與它們一致 |
| `data EntityFilter = EntityFilter { efType :: Maybe Text, efStatus :: Maybe Status, efTag :: Maybe Text, efLimit :: Maybe Int }` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | 四個 query parameter 組成它 |
| `emptyFilter :: EntityFilter` | `store/src/StoryFlow/Store/Query.hs` | entity-graph-core/F004 | 沒帶 query parameter 時的預設 |
| `data StoreError = VaultNotFound Text \| VaultConfigInvalid FilePath Text \| VaultAlreadyExists FilePath \| EntityNotFound Id \| StaleRevision Id Int Int \| IdCollision IdPrefix \| FileReadFailed FilePath Text \| FileWriteFailed FilePath Text \| IndexUpdateFailed FilePath Text \| ParseFailed FilePath [MdError] \| SqliteError Text`(`renderStoreError :: StoreError -> Text`) | `store/src/StoryFlow/Store/Error.hs` | entity-graph-core/F004 | 狀態碼對照表要逐個建構子分派 |
| `StoreError` 的新建構子:`ReferencedBy Id [(Id, Link)]` / `NotAFileMain Id` / `NotAFragment Id` / `NodeDepthExceeded Id Int` / `CannotRemoveRootNode Id` / `LinkNotFound Id LinkKind Ref` / `FileAlreadyExists FilePath` / `TreeInvalid FilePath [TreeError]` / `RegistryDirUnknown Text` | `store/src/StoryFlow/Store/Error.hs` | entity-graph-core/F005 | 同上;它們各自對應到 409 / 400 / 500 |
| `openEnv :: Maybe Text -> FilePath -> IO (Either ServiceError (Env, [IndexIssue]))` | `service/src/StoryFlow/Service/Monad.hs` | F001 | 啟動時建立 `MVar Env`;失敗時仍讓 `/vaults` 可用 |
| `closeEnv :: Env -> IO ()` | `service/src/StoryFlow/Service/Monad.hs` | F001 | 關機時釋放連線 |
| `runService :: Env -> ServiceM a -> IO (Either ServiceError a)` | `service/src/StoryFlow/Service/Monad.hs` | F001 | `run1` 輔助函式的核心 |
| `data ServiceError`(`renderServiceError :: ServiceError -> Text`) | `service/src/StoryFlow/Service/Error.hs` | F001 | 錯誤 body 的 `message` 與狀態碼對照 |
| `errorCode :: ServiceError -> Text` | `service/src/StoryFlow/Service/Error.hs` | F001 | 錯誤 body 的 `code`,**與 CLI 用同一個函式** |
| F001 的全部業務函式(`createVault` / `listVaults` / `vaultInfo` / `reindex` / `refreshIndex` / `listEntityTypes` / `createEntity` / `addFragment` / `getEntity` / `listEntities` / `searchEntity` / `updateEntity` / `setEntityBody` / `deleteEntity` / `addLink` / `removeLink` / `linksOf` / `createLevel` / `getLevel` / `listLevels` / `deleteLevel` / `addNode` / `removeNode`) | `service/src/StoryFlow/Service.hs` | F001 | 每條路由各對應一個,handler 不做業務判斷 |
| `data EntityView` / `LevelView` / `VaultView` / `SearchHit` / `LinkReport` / `IndexReport` / `DeleteReport` | `service/src/StoryFlow/Service/Types.hs` | F001 | 回應型別;要補 `ToSchema` |
| `data NewEntityReq` / `NewFragmentReq` / `NewLevelReq` / `NewNodeReq` / `EntityPatch`(`emptyPatch`) | `service/src/StoryFlow/Service/Types.hs` | F001 | 請求 body 型別;要補 `ToSchema` |
| `ToJSON` / `FromJSON` 實例(上述 View 與請求型別) | `service/src/StoryFlow/Service/Json.hs` | F001 | body 編解碼;`servant-client` 解碼回同一批型別 |
| `runCli :: [String] -> IO ExitCode` | `cli/src/StoryFlow/Cli.hs` | F002 | **要擴充**:分派出遠端路徑 |
| `data GlobalOpts = GlobalOpts { goVault :: Maybe Text, goJson :: Bool }` | `cli/src/StoryFlow/Cli/Options.hs` | F002 | **要擴充**:加 `goRemote :: Maybe Text` |
| `data Command`(23 個建構子) | `cli/src/StoryFlow/Cli/Options.hs` | F002 | 每個建構子多一條遠端執行路徑 |
| `data Selector = SelById Id \| SelByTitle Text` | `cli/src/StoryFlow/Cli/Options.hs` | F002 | 標題定址在遠端模式改用 HTTP 清單查詢 |
| `resolveEntity :: Selector -> ServiceM (Either ResolveError Id)`(同 `resolveLevel` / `resolveNode`) | `cli/src/StoryFlow/Cli/Resolve.hs` | F002 | **要擴充**:改成對 `Backend` 分派而非只吃 `ServiceM` |
| `currentRevision :: Id -> ServiceM Int` | `cli/src/StoryFlow/Cli/Resolve.hs` | F002 | **要擴充**:同上,遠端模式走 HTTP GET |
| `data Envelope a = Ok a \| Err Text Text`(`ToJSON` 實例) | `cli/src/StoryFlow/Cli/Render.hs` | F002 | 兩種模式共用,連線錯誤也裝進它 |
| `renderEntity :: EntityView -> Text` / `renderMetaTable :: [Meta] -> Text` / `renderSearch :: [SearchHit] -> Text` / `renderLevelTree :: LevelView -> Text` / `renderLinks :: LinkReport -> Text` | `cli/src/StoryFlow/Cli/Render.hs` | F002 | **兩條路徑共用同一個渲染器**,這是驗收標準 4 的達成方式 |
...
| T4 | `api/test/.../ApiSpec.hs` → `路由涵蓋 service 的全部操作且 revision 必填` | 列出 `StoryFlowAPI` 的全部端點,斷言數量與 F001 的操作清單相符;每個寫入端點的 `revision` 缺席時型別層即拒絕(以 `servant-client` 呼叫時無法省略,由編譯期證明) |
...
規格 T11 把 `serve` 寫成 CLI 的子指令,但那與 system.md 直接衝突:讓 `storyflow-api`
...
依賴關係因此與 system.md 的架構圖逐條相符:`storyflow-cli` → `api` + `service`(無
...
`removeNode` 都要 Level 的 id(F002 實作備註 1 記錄過同一件事),而 REST 路徑上只有節點。
...
`RemoteError` 之後會形成循環 import)。F002 的 131 條測試在重構後全數通過,是這次改動
的回歸保護。
...
| 5. 預設綁 loopback;綁非 loopback 必須設 token 且印警告 | ✅ 收得比 ADR-006 更緊——沒 token 直接**拒絕啟動**;`ServeOptsSpec` 驗 `0.0.0.0` / 區網位址 / 空字串 token,並確認 warp 萬用字元不算 loopback |

測試:`cabal test all` 全綠。本文檔 新增 `storyflow-api-test` 51 條、`storyflow-server-test`
