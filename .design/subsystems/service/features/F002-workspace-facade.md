...
`ServiceM` 動作(十六個操作裡的十五個;`workspaceSetup` 是先於環境的頂層 IO,見 ASM-2),並把下層
的報告型別投影成**線上格式**。實作那些操作的模組是 design.md「內部模組劃分」的 **Machine**
一個;六個 View 型別依同一張表住 **Types**(ASM-1),`Machine.hs` 原地 re-export。
...
**2026-08-30 WAVE-2 閘門的八條裁決已全部套進本文檔**(四條待確認假設的裁決結果見文末「待確認假設」,
條目保留為決策紀錄)。其中三條改變了本 feature 的形狀:

1. **`workspaceSetup` 移出 `ServiceM`**(ASM-2):新契約是
   `workspaceSetup :: Maybe Text -> FilePath -> IO (Either ServiceError SetupView)`,與 `openEnv`
   同層。它是十六個操作裡**唯一**不經 `Env` 的一個。
2. **六個 View 型別住 `Aapms.Service.Types`**(ASM-1),`Machine.hs` 在匯出清單裡**原地 re-export**
   ——消費端的 import 路徑不變。
3. **`vaultInit` 回 `(VaultView, AdoptNotice)`**(ASM-3);**`vaultInfo` 直接對參數指定的 vault
   取 handle**(ASM-4,`Machine → Monad` 新增 `handleFor`),不再經 `withRead`。

**驗收標準 7 的 `UnknownType Text` 由本 feature 交付**(WAVE-2 閘門阻塞項 1 授權):`Types.hs` 本波
...
**`graph-core/F009` 在 WAVE-2 閘門後移出 `depends-on`**:ASM-4 裁決讓 `vaultInfo` 改用單 vault 的
...
design.md 契約 C 已於 2026-08-30 WAVE-2 閘門由編排者回寫;下表的「原文」欄是**回寫後**的版本。

| design.md 契約 C 原文 | 本 feature |
|---|---|
| `workspaceSetup :: Maybe Text -> FilePath -> IO (Either ServiceError SetupView)` | 交付,簽名逐字(ASM-2 裁決後的新契約;唯一不在 `ServiceM` 裡的操作) |
...
| `vaultInit :: FilePath -> VaultKind -> Text -> InitMode -> ServiceM (VaultView, AdoptNotice)` | 交付,簽名逐字(ASM-3 裁決後的新契約;`AdoptNotice` 進 re-export 清單) |
| `vaultAdd :: FilePath -> ServiceM VaultView` | 交付,簽名逐字 |
| `vaultList :: ServiceM [VaultView]` | 交付,簽名逐字 |
| `vaultInfo :: Text -> ServiceM VaultInfoView` | 交付,簽名逐字;目標是**參數指到的 vault**(ASM-4) |
...
| `listTypes :: ServiceM [TypeDecl]` | 交付,簽名逐字(與 `Aapms.Core.Registry.listTypes` 撞名,見 SELF-2) |
...
六個 View 型別**宣告在 `Aapms.Service.Types`**(ASM-1 裁決),`Machine.hs` 在匯出清單裡原地
...
(`ToolOrigin`、`IndexIssue`、`AdoptNotice`——後者由 ASM-3 裁決帶進來),並**排除** `RegistrySource`
...
| `UnknownType Text` | **本 feature 交付**(WAVE-2 閘門阻塞項 1 授權):建構子 + `errorCode` + `renderServiceError` 三處,規格見「數據」 |

### 模組間公開介面(design.md 表裡本 feature 用到的列)

| design.md 原文 | 本 feature |
|---|---|
| `Read / Write / Machine → Scope`:`withRead` | **本 feature 不使用**。ASM-4 裁決後 `vaultInfo` 改走 `handleFor`,Machine 沒有第二個需要範圍解析的操作;這一列留給 F003 起的讀取操作 |
...
| `Machine → Monad`:`handleFor`(2026-08-30 WAVE-2 閘門 ASM-4 新增的一列) | 使用(`vaultInfo` 唯一的用處):對 `lookupSelector` 解出來的那一個 vault 取 handle |
| `Machine → aapms-workspace`:生命週期那一組;`Env` 的中樞快照在寫入後必須重新載入 | 使用;「寫入後重新載入」的執行點是 `reloadHub`,適用範圍逐條釘在 LAW-14 / LAW-13 |
| `Machine → aapms-store`:**查詢組,只讀**(2026-08-30 WAVE-2 閘門補列的一列) | 使用(`vaultInfo` 的 `viCounts`:`listNodes` + `NodeFilter`)。**不再需要 `MultiVault`**——ASM-4 之後目標只有一個 vault,不必組 `VaultSet` |
| `Scope → Monad`:`indexIssuesFor :: VaultId -> ServiceM [IndexIssue]` | **使用(呼叫方是 Machine,不是 Scope)**。這是 WAVE-1 閘門 ASM-3 加這個介面的**原始理由**(build-log:「`vaultInfo` 的 `viIssues` 欄(F002)要得到它」),但它被寫在 `Scope → Monad` 那一列。建議編排者把它併進新增的 `Machine → Monad` 列(ASM-4 已把 `handleFor` 放在那裡,兩者是同一次呼叫的前後腳) |
...
2. **`setupHub` 只在中樞位置上工作,而 `openEnv` 已經要求中樞載得起來**:`loadHub` 對
   ...
   **必定已存在**,留在 `ServiceM` 裡的 `workspaceSetup` 會讓 `svHubCreated` 恒為 `False`。
   → ASM-2,**裁決:移出 `ServiceM`**,新契約與 `openEnv` 同層
...
3. **`withRead` 用的是 `Env` 的 selector,不是 `vaultInfo` 的參數**
   ...
   `vault info B`(B 不在 A 的 `refs*` 裡)拿不到 B 的索引。→ ASM-4,**裁決:`vaultInfo` 改用
...
4. **graph-core 沒有任何「數節點」的介面**:`store/src/Aapms/Store/` 底下沒有 `count*`,能拿到
   ...
   (`store/src/Aapms/Store/MultiVault.hs:230`)。`viCounts` 因此是「查回來再數」;ASM-4 裁決之後
...
7. **`HubSource` 與 `RegistrySource` 各有一個 `FromEnv` 建構子**
   ...
   → WAVE-2 閘門裁決:**根治的作法是上游改建構子名**,但那動的是 `aapms-workspace` /
...
ASM-4 裁決之後 `Machine` **不再 import `Scope`**:唯一用過 `withRead` 的 `vaultInfo` 改走
...
四組:三組對應 design.md「本機」管線的三段,外加 ASM-2 裁決之後自成一格的 `workspaceSetup`。
...
要讀的那個檔 —— 重載必然回 `HubNotFound`(LAW-13)。`workspaceSetup` 更不在:它連 `Env` 都沒有。

`vaultInfo` 是唯一要開索引的(ASM-4 裁決後改走 `handleFor`):
...
WAVE-2 閘門的裁決是**根治:上游改建構子名**(例如 `RegistrySource` 的 `FromEnv` 改成
...
|---|---|---|---|
| `newtype ServiceM a`(不透明) | `service/src/Aapms/Service/Monad.hs:138` | `F001` | 十六個操作裡**十五個**的回傳(`workspaceSetup` 是頂層 IO,見 ASM-2) |
...
| `handleFor :: VaultRef -> ServiceM VaultHandle` | `service/src/Aapms/Service/Monad.hs:276` | `F001` | `vaultInfo`(ASM-4:對參數指到的 vault 取 handle);開啟失敗已由它包成 `StoreFailed` |
...
| `newtype AdoptNotice = AdoptNotice { anLegacyMarkers :: [FilePath] }` | `workspace/src/Aapms/Workspace/Types.hs:241` | `workspace/F001` | `vaultInit` 的第三個回傳值(ASM-3) |
...
| `emptyNodeFilter :: NodeFilter`(`nfLimit = 1000`、`nfIncludeReference = False`) | `store/src/Aapms/Store/Query.hs:119` | `graph-core/F006` | 同上;**兩個預設值都要改**(LAW-21) |
| `listNodes :: VaultHandle -> NodeFilter -> IO [Meta]` | `store/src/Aapms/Store/Query.hs:240` | `graph-core/F006` | `viCounts` 的唯一資料來源(ASM-4 之後目標只有一個 vault,不再需要 `MultiVault` 的 `listAcross`) |

## 新增的介面

### `service/src/Aapms/Service/Types.hs`(ASM-1 之後六個 View 型別的家;既有檔案的擴充)
...
### `ServiceError` 新增的建構子(**本 feature 交付**;WAVE-2 閘門阻塞項 1 授權)
...
|---|---|---|---|
| `UnknownType Text` | `unknown_type` | 本層自撰(**不委派**:下面沒有對應的錯誤原件)。繁中,說出下一步——訊息**必須含酬載那個鍵的字串**,並告訴使用者去型別註冊表目錄補宣告、或用 `type list` 看有哪些 | `showType`(LAW-24 / EX-23);F004 起的寫入路徑同用 |
...
兩個分支因此在骨架階段就寫完,對應的 LAW-27 / EX-27 依 `spec-roles.md` 交付判準第二列(骨架自身
承載的事實)**從第一天就綠**。
...
- **LAW-1**(逐列對應):對所有中樞內容,`vaultList` 的長度等於 `hubVaults hub` 的長度,第 `i` 筆的
  `vvId` / `vvName` / `vvKind` / `vvPath` 逐欄等於 `hubVaults hub !! i` 的
  `veId` / `veName` / `veKind` / `vePath`,**順序相同**。中樞沒有任何 vault 時回 `Right []`。
- **LAW-2**(`vvRegistered` 恒真):對所有中樞內容,`vaultList` 的每一筆 `vvRegistered == True`
  ——它逐列來自中樞,「在中樞裡」是它的建構前提。
- **LAW-3**(`vvReachable` 的判準):對所有中樞內容,
...
- **LAW-4**(原樣轉出):對所有中樞內容,`vaultCheck` 的結果與 `checkVaults hub`(同一份中樞快照)
  **逐項相同、順序相同**。本層不過濾、不排序、不翻譯,也不合併。

### `workspaceDoctor`

- **LAW-5**(六欄的來源):對所有中樞內容與所有起點,`workspaceDoctor` 回的 `DoctorView` 滿足
...
- **LAW-6**(`dvVaults` 含 `vaultList` 全部):對所有中樞內容,`dvVaults` 的**前 `n` 筆**
  (`n` = `length (hubVaults hub)`)逐欄等於 `vaultList` 的結果;其後至多一筆,且那一筆的
  `vvRegistered == False`。
- **LAW-7**(未註冊那一筆的存在條件):對所有起點 `cwd`,`dvVaults` 含 `vvRegistered == False` 的那一筆
...
- **LAW-8**(`[llm]` 內容不外洩):對所有 `[llm]` 段內容 `s`(視為一組字串),`show doctorView` 不含
  `s` 的任何鍵或值作為子字串;`dvLlmConfigured` 只反映該段**存不存在**。
- **LAW-9**(唯讀):對所有中樞內容與所有起點,`workspaceDoctor` 與 `vaultCheck` 執行前後,中樞目錄
  與每個 vault 的 `.aapms/` 底下**所有檔案的存在性與位元組內容都相同**。

### `workspaceTools`

- **LAW-10**(單筆、無失敗通道):對所有中樞內容,`workspaceTools` 恒回 `Right`,清單長度恰為 `1`,
...
- **LAW-11**(`SetupView` 是逐欄投影,而且兩個參數不影響結果):對所有中樞狀態、所有 selector `sel`
  ...
  **它不開 `Env`**:中樞不存在時仍跑得起來(這正是 ASM-2 把它移出 `ServiceM` 的理由),而且對同一個
  中樞位置,不同的 `sel` / `cwd` 得到**逐欄相同**的結果 —— 中樞位置只由 `hubLocation` 決定。
- **LAW-12**(`PurgeView` 是逐欄投影):對所有 `scope`,`workspacePurge scope` 成功時回的 `PurgeView`
  的三欄逐欄等於**同一次** `purge loc hub scope` 回的 `PurgeReport` 的三欄;
  `scope == PurgeHubOnly` 時 `pvVaultIndexesRemoved == []`。
- **LAW-13**(`purge` 不重載中樞快照):對所有 `scope`,`workspacePurge scope` 成功之後,同一個 `Env`
...
- **LAW-14**(寫後同一個 `Env` 看得到):對 `vaultInit` / `vaultAdd` / `vaultForget` /
...
- **LAW-15**(`vaultInit` / `vaultAdd` 回的那一筆):兩者成功時回的 `VaultView`(`vaultInit` 取
...
- **LAW-16**(`vaultForget` 回的那一筆):成功時回的 `VaultView` 的前四欄逐欄等於**被移除的**那一列
  `VaultEntry`,`vvRegistered == False`(它已經不在中樞了),`vvReachable` 依 LAW-3 的同一判準計算
  (`vault forget` 不刪 marker,所以正常情況下仍為 `True`)。
- **LAW-17**(失敗即原樣包、什麼都不動):對五個寫中樞的操作與 `vaultInfo`,下層回 `Left e` 時本層
...
- **LAW-18**(逐列對應):對所有中樞內容,`projectList` 的長度等於 `hubProjects hub` 的長度,第 `i` 筆
  的 `pvId` / `pvName` / `pvPath` 逐欄等於 `hubProjects hub !! i` 的 `peId` / `peName` / `pePath`,
  順序相同;`pvReachable == True` **當且僅當** `pePath` 是一個既存目錄。
- **LAW-19**(回的那一筆):`projectRegister` 成功時回的 `ProjectView` 前三欄逐欄等於下層回的
  `ProjectEntry`;`projectForget` 成功時回的是**被移除的**那一列。兩者的 `pvReachable` 依 LAW-18 的
  同一判準計算。

### `vaultInfo`

- **LAW-20**(`viVault` 與 selector):對所有 selector `s`,`lookupSelector hub s` 回 `Right e` 時
...
- **LAW-21**(`viCounts`):對**中樞裡的任一 vault** 與**任一 `--vault` selector**(含 `Nothing`),
  ...
  `viCounts == []`。
  **`Env` 的 `--vault` 不影響本操作**(ASM-4 裁決):目標是參數解出來的那一個 vault,`vaultInfo`
  直接對它取 handle,所以 `--vault A` 之下問 B 仍算得出 B 的節點數(EX-28),**不會**靜默回空。
- **LAW-22**(`viIssues`):對中樞裡的任一 vault,`viIssues` 等於該 vault 在本次執行期間**第一次**
...
- **LAW-23**(`listTypes` 逐項轉出):對所有註冊表,`listTypes` 的結果與
  `Aapms.Core.Registry.listTypes reg`(`reg` = `askRegistry`)**逐項相同、順序相同**。本層不過濾、
  不重排、不改寫任何欄位。
- **LAW-24**(`showType` 的兩條路):對所有 `k :: TypeKey`,
...
- **LAW-25**(位置由 workspace 算、本層只判存在):對所有 `h :: Sha256`,
...
- **LAW-26**(`service/src/` 的程式碼行不得出現本機路徑字面):契約卡「明確不做」第二條
  ...
  **掃描範圍與正規化**:與 F001 的 LAW-23 / LAW-25 **逐字相同**(`service/src/` 底下**遞迴**取得的每一個
  ...
  **斷言**:違規行的清單為**空**。判準本身寫成對「(檔名, 檔案全文)」的純函數,`service/src/` 的
  實況(EX-25)與一份合成文字(EX-26)餵的是同一個判準 —— **沒有 EX-26 這條 law 就可能是空洞的**
  (掃描器寫壞時正向斷言照樣綠),與 F001 的 LAW-23 / LAW-25 是同一個理由。
...
- **LAW-27**(本層自撰的那一則錯誤):對所有 `t :: Text`,
  `errorCode (UnknownType t) == "unknown_type"`(只看建構子、不看酬載,與 F001 的 LAW-20 一致);
  ...
  的錯誤原件。
  既有四個建構子的 `errorCode` 與 `renderServiceError` **一個字都沒變**(F001 的 LAW-19–LAW-22 與
  EX-21–EX-23 仍全數成立,本條不得與它們衝突)。

`L-`:六個 View 型別的 `deriving stock (Show, Eq)` 子句**無獨立 law**,理由具體:它們是骨架原文
承載的事實,寫成 property 只會得到「`show x` 不是空字串」這種恆真斷言;而它們**存在**這件事由
LAW-8(要 `show` 得出整份報告才驗得到「不含 `[llm]` 的值」)與其餘每一條逐欄比較的 law(要 `Eq`)
...
|---|---|---|---|---|
| EX-1 | 完整佈局,`runService env vaultList` | 兩筆,`vvId` 依序 `[VA, VB]`;`vvName` `["story", "assets"]`;`vvKind` `[StoryVault, AssetVault]`;兩筆的 `vvRegistered` 與 `vvReachable` 皆 `True` | 正常路徑 | LAW-1, LAW-2, LAW-3 |
| EX-2 | 中樞多一列 `VC` 指向不存在的路徑後 `vaultList` 與 `vaultCheck` | `vaultList` 三筆;`VC` 那筆 `vvReachable == False`、其餘兩筆 `True`;`vaultCheck` 恰含一則 `VaultPathMissing` 且它的 `veId` 是 `VC` | 不可達的一列 + 驗收標準 1 | LAW-1, LAW-3, LAW-4 |
| EX-3 | 把 `<tmp>/va/.aapms/config.toml` 的 id 改成別的值後 `vaultList` 與 `vaultCheck` | `vaultCheck` 恰含一則 `VaultIdDrift`;而 `vaultList` 裡 `VA` 那筆的 `vvReachable` 仍為 **`True`** | id 漂移**不是**不可達 | LAW-3, LAW-4 |
| EX-4 | 中樞的 `[[vaults]]` 清空後 `vaultList` | `Right []` | 空中樞 | LAW-1 |
| EX-5 | 從中樞刪掉 `VA` 那一列(`<tmp>/va/.aapms/` 保留),`openEnv Nothing <tmp>/va` 後 `workspaceDoctor` | `dvVaults` 有兩筆:第一筆是 `VB`(`vvRegistered == True`),第二筆 `vvRegistered == False`、`vvId == VA`、`vvPath` 是 `<tmp>/va` 的正規化路徑、`vvReachable == True` | 未註冊的 vault + 驗收標準 2 | LAW-6, LAW-7 |
| EX-6 | 完整佈局,`openEnv Nothing <tmp>/outside` 後 `workspaceDoctor` | `dvVaults` 逐欄等於 `vaultList` 的兩筆,**沒有**任何 `vvRegistered == False` 的項目 | 起點不在任何 vault 底下 | LAW-6, LAW-7 |
| EX-7 | 中樞加一段 `[llm]`,其中 `api_key = "SENTINEL-7f3b9c"`,`workspaceDoctor` | `dvLlmConfigured == True`;`show` 整個 `DoctorView` 的結果**不含** `"SENTINEL"`、也不含 `"api_key"` | 金鑰不進診斷輸出 + 驗收標準 3 | LAW-5, LAW-8 |
| EX-8 | 無 `[llm]` 段時 `workspaceDoctor` | `dvLlmConfigured == False` | `[llm]` 缺席 | LAW-5 |
...
| EX-23 | `showType (TypeKey "no-such-type-xyz")` | `Left (UnknownType "no-such-type-xyz")` | 未命中 + 驗收標準 7 | LAW-24 |
| EX-24 | 對一個不在快取裡的 `h` 呼叫 `thumbPath h`;接著手動建出 `thumbCachePath loc h` 這個檔(含中間目錄)再呼叫一次 | 第一次 `Right Nothing`;第二次 `Right (Just p)` 且 `p` 逐字等於 `thumbCachePath loc h`、且 `p` 讀得出剛寫進去的位元組;兩次前後 `<tmp>/hub/cache/` 底下除了測試自己建的那個檔以外沒有任何變化 | 快取命中與未命中 + 驗收標準 8 | LAW-25 |
| EX-25 | 對 `service/src/` 的**實況**(骨架階段是 `Types.hs` / `Monad.hs` / `Scope.hs` / `Machine.hs` 四個檔)跑 LAW-26 的判準 | 違規清單**為空**。`Machine.hs` 的 haddock 提到 `.aapms\/` 的那些**註解行**全部被正規化丟掉,**不計入** | 路徑防線的正向路徑;順帶驗「判準不把註解誤判成越界」 | LAW-26 |
| EX-26 | 一份**合成**的模組文字(只存在於測試裡,不寫進 repo):檔名 `Aapms/Service/Machine.hs`,內容取 `Machine.hs` 現況再插入一行 `  let p = root </> ".aapms" </> "index.db"`,以及一行 `-- 別自己拼 ".aapms" 底下的路徑` | 違規清單**恰好一條**,指向插入的那一行(帶檔名與行號);那行**註解**不被算成違規 | 判準非空洞(掃描器壞掉時 EX-25 也會綠)+ 註解豁免的負向確認 | LAW-26 |
| EX-27 | `errorCode (UnknownType "no-such-type-xyz")` 與 `renderServiceError (UnknownType "no-such-type-xyz")` | `code` 逐字等於 `"unknown_type"`;訊息非空、**含 `"no-such-type-xyz"`**、且**含 `"type list"`**(說出下一步的其中一條) | 新建構子的 code 與訊息(骨架承載,預期綠) | LAW-27 |

## 依賴方向

- **依賴誰**:同套件的 `Aapms.Service.Monad` 與 `Aapms.Service.Types`(**不再有 `Scope`**,見 ASM-4);
...
- **新增的依賴邊**(逐條,一條都不能漏;分兩個模組):
  ...
  - `Aapms.Service.Machine` → `Aapms.Store.Schema` / `Aapms.Store.Query`
    ...
    直接餵給 `listNodes`,型別可以由推論得出)—— design.md「模組間公開介面」已於 WAVE-2 閘門補上
    `Machine → aapms-store`(**查詢組,只讀**)這一列,本組落在它裡面。`Aapms.Store.MultiVault`
    **不再需要**(ASM-4 之後目標只有一個 vault)
  ...
  - **`Aapms.Service.Machine` → `Aapms.Service.Scope` 這條邊不存在**(原本因 `withRead` 而有,
    ASM-4 裁決後 `vaultInfo` 改走 `handleFor`);`Machine` 對 `Monad` 多用一個既有介面 `handleFor`,
    design.md 已補 `Machine → Monad` 那一列
  - **`Aapms.Service.Types` 新增的四條**(ASM-1 把六個 View 型別搬進來的後果):
  ...
  - **套件層級無新增**:四個下層套件在 F001 就已全部進 `build-depends`;`directory` 已由編排者
    依 DEC-1 加進 library 的 `build-depends`(`projectList` 的 `pvReachable` 與 `thumbPath` /
...
`Aapms.Service.Machine` 已由編排者依 DEC-1 加進 `exposed-modules`,`directory` 也已進
...
`ServiceM`(F001 的 LAW-23 會在原始碼文字上抓到,而且它根本不需要 `Env`);兩個參數在本層用不到,
填本體時要用底線前綴命名(`_sel` / `_cwd`),否則 `-Wall` 的 `-Wunused-matches` 會出警告。

**三條靜態 law 對 impl 的約束**:F001 的 **LAW-23**(本檔的程式碼行一個字都不准提到 `Monad` 模組的
執行入口)、F001 的 **LAW-25**(不得寫 `instance` 宣告、不得 standalone deriving、不得動 `ServiceM`
的 deriving 子句)、本 feature 的 **LAW-26**(不得出現 `".aapms"` / `"config.toml"` / `"index.db"` /
`"cache"` / `"thumbs"` 五個帶引號的字串字面)。三條都掃整個 `service/src/`(含本波動到的
`Types.hs`),填本體的時候踩到就紅。六個 View 型別搬進 `Types.hs` 之後仍**不違反 LAW-25**:
它們走的是資料宣告上的 `deriving stock` 子句,不是 `instance` 宣告,也沒有一行 `deriving` 含
`Monad`。

## 待確認假設

四條,全部是契約層級,**全部已於 2026-08-30 的 WAVE-2 spec 批准閘門裁決**。條目保留為決策紀錄:
下面每一條的「選項 / 傾向 / 可逆性」是當時呈報給開發者的材料,末尾的「已裁決」欄是閘門的結果與
本次修訂實際做的事。**spec 與骨架現在照裁決結果寫,不再是「暫採」。**

- **ASM-1**:契約 C 的六個 View 型別(`SetupView` / `PurgeView` / `VaultView` / `VaultInfoView` /
  ...
  - 暫採:**a**(六個型別宣告寫在 `Machine.hs`)
    ...
    **一個字都不用改**
  - **已裁決(2026-08-30 WAVE-2 閘門):採 b**。design.md「內部模組劃分」的 Types 列已寫明
...
- **ASM-2**:契約 C 的 `workspaceSetup :: ServiceM SetupView` 在「中樞還不存在」這個它唯一有意義的
  ...
  - 暫採:**a**(骨架照契約原文寫 `workspaceSetup :: ServiceM SetupView`)
    → 影響:若裁決 b,`Machine.hs` 改一個簽名、LAW-11 的敘述從「`workspaceSetup` 成功時」改成
    「`workspaceSetup` 回 `Right` 時」、EX-11 的第一次呼叫改成在**中樞不存在**的佈局上跑並斷言
    `svHubCreated == True`;其餘十五個操作不受影響
  - **已裁決(2026-08-30 WAVE-2 閘門):採 b**,契約 C 的新原文是
...
- **ASM-3**:`vaultInit` 丟掉了 `AdoptNotice`。`initVault` 回三個值,第三個是「掃到的
  ...
  - 暫採:**a**(骨架照契約原文寫 `vaultInit :: FilePath -> VaultKind -> Text -> InitMode -> ServiceM VaultView`)
    → 影響:若裁決 b,`Machine.hs` 改一個簽名、匯出清單加 `AdoptNotice (..)`、LAW-15 加一句
    「第二個分量逐欄等於 `initVault` 回的 `AdoptNotice`」、EX-13 的預期輸出多一個分量
  - **已裁決(2026-08-30 WAVE-2 閘門):採 b**,契約 C 的新原文是
...
- **ASM-4**:`vaultInfo` 用的是 `Env` 的 selector 解出的**讀取範圍**,不是它自己那個參數指到的 vault。
  ...
  - 暫採:**a**(骨架不動;LAW-21 明文寫出「目標 vault 不在讀取範圍內時 `viCounts == []`」)
    → 影響:若裁決 b,`Machine.hs` 的匯出與簽名**都不動**(只動本體與 import),LAW-21 / LAW-22 刪掉
...
|---|---|---|
| LAW-1–LAW-3, EX-1–EX-4 | `vaultList` / `VaultView` | 紅 |
| LAW-4, EX-2, EX-3 | `vaultCheck` | 紅 |
...
| LAW-25, EX-24 | `thumbPath` | 紅 |
| **LAW-26, EX-25, EX-26** | `service/src/` 的**原始碼文字**(五個帶引號的路徑字面) | **綠** |
| **LAW-27, EX-27** | `errorCode` / `renderServiceError` 對 `UnknownType` 的兩個分支 | **綠** |

**除了 LAW-26 / EX-25 / EX-26 與 LAW-27 / EX-27 之外全部預期紅**(EX-23 現在編得過了:`UnknownType` 已由本
...
- **綠的那五條**:LAW-26 / EX-25 / EX-26 是對**原始碼文字**的斷言,LAW-27 / EX-27 是對骨架階段就寫完的兩個
  分支(`errorCode` / `renderServiceError` 的 `UnknownType`)的斷言——兩組都適用
  `spec-roles.md`「qa 的交付判準」第二列(骨架自身就承載的事實):它們從第一天就綠,而且
  **應該**綠。impl 把 `undefined` 換掉之後仍須維持綠(LAW-26 守的是 F003–F008,不是本波;LAW-27 守的
  是「本層自撰的訊息不被改成委派」)。**不得**因為它們綠就退回或刪掉。反過來,EX-26 是 LAW-26 的
...
`DoctorView` → LAW-5–LAW-8、`ProjectView` → LAW-18);`UnknownType` 由 LAW-24(誰產生它)與 LAW-27(它的 code
與訊息)兩面覆蓋;`deriving stock (Show, Eq)` 走 `L-` 的理由段;LAW-17 與 LAW-26 是**跨介面**的兩條
(失敗路徑與原始碼文字),不對應單一列。

**F001 的兩條靜態 law 也覆蓋本波動到的兩個檔**:LAW-23(`service/src/` 底下除 `Monad.hs` 外不得提到
執行入口——`workspaceSetup` 變成頂層 IO 之後這一條更要緊)與 LAW-25(不得有 `instance` 宣告 /
...
標準,等於沒有(對照 `contract-readiness.md` 的 ASM-9 可測性、`graph-core` 的 GAP-5,同一個根)。
