---
id: service-spec-gaps
type: spec-gaps
title: service-spec-gaps
...
- 模糊點:**X10 的建構在裝了 7-Zip 的機器上不成立**。它假設「`[tools]` 未設 + `PATH` 清空」
...
- 卡住的項目:X10(`test_workspace_tools_not_found_example`)。
  **L10 本身沒有卡住** —— 它上面那條 property test(`prop_workspace_tools_matches_detect`,
...
- 狀態:resolved(2026-08-30 W2 仲裁,開發者裁決):選 (a) —— X10 改成斷言「三層都試過」(`tsSearched` 非空),**不再斷言具體的 `NotFound`**。理由是它在裝與沒裝 7-Zip 的機器上都可重現,CI 與開發機行為一致;`NotFound` 這個值仍由 L10 的 property test 間接覆蓋(機器沒裝時那條自然比對到它)。(b) 被否決:那個覆寫點很可能就是測試後門
...
- 模糊點:**X19b 期待的 `viIssues` 非空是不可達的**。它要求對一個已建好索引的 vault 呼叫
  ...
  依 L22 自己的定義 `viIssues` 就該是 `[]`。
- 卡住的項目:X19b 的 `viIssues` 斷言(X19b 的其餘部分 —— 「`vaultInfo` 不受 `--vault` 範圍拘束」
  那一半 —— 是綠的,A4 的裁決有被驗到)。
- 需要 spec 回答什麼:X19b 對 `viIssues` 的預期是否應改為「與 X18 同一判準:逐項等於同一個
  `Env` 的 `indexIssuesFor`,**不強制非空**」?
- 狀態:resolved(2026-08-30 W2 仲裁,開發者裁決):**是**,改成與 X18 同一判準、不強制非空。L22 的法條文字不動(它是對的);X19b 驗 A4 裁決的那一半原樣保留
