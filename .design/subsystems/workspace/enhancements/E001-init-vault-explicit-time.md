      ...
      ...
              ...
              case initR of
                Left err -> do
                  ...
                  pure (Left (VaultInitFailed dir' err))        -- L44 的分支
                Right m -> case find ((== vmId m) . veId) (hubVaults hub) of
                  Just existing -> do
                    removePathForcibly (markerDir dir')
                    pure (Left (VaultIdCollision (vmId m) (vePath existing) dir'))  -- L18/L19 的分支
```

**痛點一:撞號分支永遠測不到(spec-gap G4,`status: open`)。**
...
`Marker.hs:146`),但 `initVault` 沒跟上,所以 G4 沒有因為 E002 完成而結案。

**痛點二:`VaultInitFailed` 的驗收路徑已經打通了,但沒有人去收(spec-gap G5 的 workspace 側尾巴)。**
G5 標成 `resolved`,裁決是「修 graph-core,讓 `initVaultAt` 不逸出 `IOException`」,已由
...
**L44 / X41 現在不改一行實作就測得起來**,只差沒有人把 `LifecycleSpec.hs:485` 的 `pendingWith`
換成斷言。

**痛點三:`Lifecycle.hs` 的逐字 import 斷言會擋住這次改動。**
F004 的 **L42(b)** 要求 `Aapms.Store.Marker` 的 import 行**逐字是**
...
`initVaultAtWith` 就必須同步修訂這條 law 的字串——這與 spec-gap **G2 / G3 是同一類問題**
...
就是上面痛點一與痛點二的那五條驗收(L18 / L19 / X18 / X19、L44 / X41)。
...
3. 修訂 F004 的 **L42(b)** 逐字 import 字串,加入 `initVaultAtWith`
4. 把 F004 被卡住的五條驗收轉成正式斷言:**L18 / L19 / X18 / X19**(G4)與 **L44 / X41**(G5 尾巴)
...
| `VaultInitFailed` 建構子的斷言數 | 0 | **≥ 2**(`initVault` 一條走 R5、`initVaultWith` 一條走 L7) |
| `workspace/spec-gaps.md` 的未結條目 | 1(G4) | **0** |
| `service` 受影響的程式碼行數 | — | **0** |
...
| F004 的 **L42(b)** 逐字字串 | **修訂**(改既有 spec 的條文) | 同上一列的字串 | L42(b) 守的三件事(見 F004 原文①②③)**一條都不放寬**,只多一個名字 | `LifecycleSpec.hs:978-984` 的期望值 | — |
...
- **R3(成功時身分逐欄來自 marker)**:成功時回傳的 `VaultEntry` 滿足 `veId == vmId m`、
  `veName == vmName m`、`veKind == vmKind m`、`vePath == dir'`,其中 `m` 是 `dir'` 的 marker
- **R4(`initVault` 仍然每次取當下時間)**:對同一個 `name`、兩個各自為空且互不相同的目錄,
  連續兩次 `initVault` 產生**不同**的 `veId`。這條防止實作把時間改成常數來湊 L1
- **R5(`initVault` 建檔失敗 → `VaultInitFailed`,且不留半成品)**:呼叫 **`initVault`**
  ...
  **不得**回 `MarkerUnreadable`。
  〔**這是 F004 的 L44 / X41 原文**,主詞就是 `initVault`。現況程式碼已經正確,本條是**回歸 law**:
  從第一天就該綠,收掉 `LifecycleSpec.hs:485` 那條 `pendingWith`——G5 的 workspace 側尾巴。
  `initVaultWith` 的同一個行為由 **L7** 管,兩者分開,不要合寫成一條〕
- **R6(L42 的另外五條不放寬)**:`Lifecycle.hs` 的 import 行仍滿足 F004 的 L42 (a)(c)(d)(e)(f)
...
- **L1(決定性)**:對任意 `kind`、任意去空白後非空的 `name`、任意 `t`,與任意兩個各自為空且
  互不相同的目錄 `d1` / `d2`,兩次 `initVaultWith … t` 成功時回傳的 `veId` **相同**
- **L2(id 的來源逐字可算)**:`initVaultWith loc hub d kind name mode t` 成功時,
  `veId == VaultId (renderId (newId PVlt (T.strip name) t 0))`。`newId` 是 graph-core 契約 B 的
  公開純函式,呼叫端算得出期望值,**不需要讀 graph-core 的實作**——G4 要的就是這一條
- **L3(薄包裝等價)**:`initVault loc hub d kind name mode` 的結果,除了 `veId` 之外,與
  `initVaultWith loc hub d kind name mode t`(任意 `t`)逐欄相同:`veName` / `veKind` / `vePath`
  一致、`AdoptNotice` 一致、落地的檔案集合一致、中樞新增的列數一致
- **L4(撞號的三個值)**:若中樞 `hubVaults hub` 中某一列 `e` 的 `veId` 等於
  ...
  〔F004 的 L18,本次才第一次可斷言〕
- **L5(撞號要回滾)**:承 L4,`markerDir dir'` 在呼叫後**不存在**,且 `dir'` 底下其餘檔案逐位元組
  相同、中樞檔案位元組不變。因此**對同一個目錄改用 `initVault` 立刻重跑一次,不會撞到
  `VaultAlreadyInitialized`**。〔F004 的 L19 + L20,本次才第一次可斷言〕
- **L6(import 行的新逐字字串)**:`Lifecycle.hs` 的 `Aapms.Store.Marker` import 行**必須逐字是**
  `import Aapms.Store.Marker (VaultMarker (vmId, vmKind, vmName), indexDbPath, initVaultAt, initVaultAtWith, markerDir, readMarker)`
  ——比對前先去除行尾 `\r`(F004 L42 的 CRLF 規則沿用)。放寬成 `VaultMarker (..)`、
  或多列 `vmRefs` / `openVault` / `closeVault` / `VaultHandle` / `configPath` 任何一個名字都要紅
- **L7(`initVaultWith` 建檔失敗 → `VaultInitFailed`,且不留半成品)**:`initVaultAtWith` 回
  `Left err` 時,`initVaultWith` 回 `Left (VaultInitFailed dir' err)`——`err` 是**原件**;
  且 `markerDir dir'` 在呼叫後**不存在**、`dir'` 底下其餘檔案逐位元組相同、中樞檔案位元組不變;
  **不得**回 `MarkerUnreadable`。
  〔**與 R5 是同一個行為的兩個入口**:R5 打 `initVault`(現況、綠),本條打 `initVaultWith`
  (新增、骨架階段紅)。兩條都要,因為兩個入口在 impl 之後才會共用同一段本體——**在那之前
  沒有任何東西保證它們一致**,而 L3「薄包裝等價」只寫了成功情況,接不住失敗路徑〕

## Examples

| # | 輸入 | 預期輸出 | 覆蓋的邊界 |
|---|---|---|---|
| E1 | 空目錄 `d`、`AssetVault`、`"alchbees-assets"`、`FreshVault`,呼叫 `initVault` | `Right (hub', entry, AdoptNotice [])`;`entry` 四欄如 R3;中樞多一列 | R3(**= 現況**) |
| E2 | 名稱 `"   "`(去空白後為空),呼叫 `initVault` | `Left (InvalidName "   ")`——帶**原始**字串;中樞檔案位元組不變 | R2(**= 現況**) |
| E3 | 兩個空目錄 `d1` ≠ `d2`,同樣 `StoryVault` / `"liftgame"` / `FreshVault` / 同一個 `t`,各呼叫一次 `initVaultWith` | 兩次都 `Right`,且 `veId` **相同** | L1(撞號可重現) |
| E4 | 空目錄 `d`、`StoryVault`、`"liftgame"`、`FreshVault`、`t`,呼叫 `initVaultWith` | `veId == VaultId (renderId (newId PVlt "liftgame" t 0))` | L2(期望值可獨立算出) |
| E5 | 先算出 `i = VaultId (renderId (newId PVlt "liftgame" t 0))`,把 `VaultEntry i "old" StoryVault O` 放進中樞;對空目錄 `V` 呼叫 `initVaultWith … "liftgame" … t` | `Left (VaultIdCollision i O V')`——第二個是 `O`、第三個是 `V'`;`renderWorkspaceError` 的輸出同時含這兩個路徑 | L4(= F004 X18) |
| E6 | 承 E5,檢查 `V` 與中樞 | `V/.aapms` **不存在**(已回滾),`V` 底下其餘檔案逐位元組不變,中樞檔案位元組不變;接著對 `V` 呼叫 `initVault` 得到 `Right` | L5(= F004 X19) |
| E7 | `blocker` 是一個**一般檔案**,對 `blocker/sub` 呼叫 **`initVault`**(`FreshVault`) | `Left (VaultInitFailed V' err)`,`err` 與直接呼叫 `initVaultAt V' kind name'` 得到的 `Left` **逐欄相同**;`V/.aapms` 不存在;中樞檔案位元組不變 | R5(**= 現況**;= F004 X41,G5 尾巴) |
| E8 | 對 E1 產生的 `d` 再呼叫一次 `initVault`(換一個名字、`AdoptExisting`) | `Left (VaultAlreadyInitialized d')`;`d/.aapms/config.toml` 逐位元組不變 | R2(**= 現況**) |
| E9 | 同一個 `name`、兩個空目錄,連續兩次 `initVault` | 兩次 `veId` **不同** | R4(薄包裝仍取當下時間) |
| E10 | 同 E7 的建構(`blocker` 是一般檔案),改對 `blocker/sub` 呼叫 **`initVaultWith … t`**(`FreshVault`) | `Left (VaultInitFailed V' err)`,`err` 與直接呼叫 `initVaultAtWith V' kind name' t` 得到的 `Left` **逐欄相同**;`V/.aapms` 不存在;中樞檔案位元組不變 | L7(新入口的同一條錯誤路徑) |

**Laws ↔ Examples 自洽對照**:E1←R3、E2←R2、E3←L1、E4←L2、E5←L4、E6←L5、**E7←R5**、E8←R2、
E9←R4、**E10←L7**。E7 與 E10 是**同一個建構、兩個入口**:E7 打 `initVault`(現況程式碼,綠)、
E10 打 `initVaultWith`(骨架 `undefined`,紅);兩者的預期輸出形狀相同但**期望值各自從自己那條
law 推出**,不互相引用——`initVault` 的 `err` 對照 `initVaultAt`,`initVaultWith` 的對照
`initVaultAtWith`。
E5 的預期輸出由 L2(算得出 `i`)與 L4(撞號的三個值)共同推出,兩條一致;E6 的「重跑得到
`Right`」由 L5 的回滾推出,與 R2 的 `VaultAlreadyInitialized` 不衝突——因為回滾後 `.aapms/`
已經不在。L3 由 E1 與 E3/E4 對照覆蓋(同一組輸入、兩個入口),L6 由 import 行斷言直接覆蓋。

## 遷移約束

- **`initVault` 的簽名不得改動**(R1)。`service/src/Aapms/Service/Machine.hs:258` 依賴它不變,
  而 `service` 已經通過階段一閘門驗收
- **不得移除 `initVault`**:它是 `service` 與既有測試 fixture 的唯一入口;`initVaultWith` 是**增設**,
  不是取代
- **L42(b) 的修訂必須與 import 行同一輪落地**:spec 條文、`LifecycleSpec.hs` 的期望值、
  `Lifecycle.hs` 的 import 行三處不同步就會紅。這是 G2 / G3 的教訓
- 本文檔完成後,`workspace/spec-gaps.md` 的 **G4 應回填 `resolved`**;G5 的「workspace 這一側還有
  一步」也隨 R5 收掉
...
- **`F004`**:本文檔優化的就是 F004 交付的 `initVault`,並**修訂 F004 的 L42(b) 條文**、
  把它的 L18 / L19 / L44 從 `pendingWith` 轉正。同子系統,直接寫 id
...
- **`graph-core/F001`**:`newId` / `renderId`。**這條不是形式相依**——L2 / L4 的期望值就是
...
| `initVaultAt :: FilePath -> VaultKind -> Text -> IO (Either StoreError VaultMarker)` | `store/src/Aapms/Store/Marker.hs:177` | `graph-core/F005`、`graph-core/E002` | 現況 `:182` 的呼叫;改完後**不再被 `Lifecycle` 呼叫**,但仍留在 import 白名單(L6)與 `Marker` 的匯出清單 |
| `newId :: IdPrefix -> Text -> UTCTime -> Int -> Id` | `core/src/Aapms/Core/Id.hs:103` | `graph-core/F001` | L2 / L4 / E4 / E5 的期望值由它算出(**qa 用得到,不必讀 graph-core 實作**) |
| `renderId :: Id -> Text` | `core/src/Aapms/Core/Id.hs:123` | `graph-core/F001` | 同上,把 `Id` 轉成 `VaultId` 的字串 |
| `data VaultMarker = VaultMarker`,欄位 `vmId` / `vmKind` / `vmName` | `store/src/Aapms/Store/Marker.hs:59` | `graph-core/F005` | 成功時把 marker 的三欄搬進 `VaultEntry`(R3) |
| `markerDir :: FilePath -> FilePath` | `store/src/Aapms/Store/Marker.hs:48` | `graph-core/F005` | 前置檢查與回滾的目標路徑(R5 / L5) |
...
| `hubVaults :: Hub -> [VaultEntry]`(`Hub` 的 record 欄位,`Hub` 不外露建構子) | 定義 `workspace/src/Aapms/Workspace/Types.hs:92`;`Lifecycle` 經 `Aapms.Workspace.Hub` 的 re-export 取用(`Hub.hs:19`) | `F001` | 撞號比對的候選清單(L4) |
...
| 這條打到的東西 | 骨架狀態 | 預期 |
|---|---|---|
| L1 / L2 / L3 / L4 / L5 / **L7**(全部經 `initVaultWith`) | `undefined` | **紅** |
| L6(import 行的新逐字字串) | `Lifecycle.hs:47` 仍是舊字串(骨架用不到 `initVaultAtWith`,先放進去會有 `-Wall` 的 redundant import 警告) | **紅** |
| R1–R4、R6(`initVault` 與其餘 import 行,現況程式碼) | 未經任何未實作標記 | **綠** |
| **R5**(**`initVault`** 的 `VaultInitFailed` 路徑,現況程式碼已正確) | 未經任何未實作標記 | **綠**——這條是 G5 尾巴,它從第一天就該綠 |

**R5 與 L7 不可合寫**(2026-08-30 `/spec-build` 閘門擋下的原因):兩條講的是同一個行為,但**打的是
