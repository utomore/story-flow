...
> `resolveWrite` 上管的是 `wsRead` 那一段(`refs` 展開)。這條區分是本 spec 的 LAW-12 / LAW-13;
> 其中「id 漂移該回哪個 `WorkspaceError`」由 **2026-08-29 WAVE-3 閘門裁決**(ASM-1 選 c),契約 F 新增
...
- `WriteTargetIdDrift` 是 **2026-08-29 WAVE-3 閘門裁決新增**(本 spec 的 ASM-1 選 c)。三個值依序是
  ...
  **宣告與訊息由 F001 的 impl 加進 `Types.hs`,不屬本 feature 的寫入白名單**;本 feature 只是
  它的產生點(`resolveWrite` 的 id 守門,見 LAW-13(b))。
...
5. **`markerDir` / `readMarker` / `configPath` / `indexDbPath` 在本 feature 一次都用不到**:
   ...
   `Aapms.Store.Marker` 只需要三個欄位存取子,一個函式都不要——這由 LAW-25(b) 逐字守住。
6. **`maxAttachedVaults = 10` 與 `TooManyVaults Int Int` 住在 graph-core**
   ...
   feature 對結果清單的長度**沒有任何上限檢查**——LAW-24 把這一點寫成可驗證的條文(11 個 vault
   仍回 `Right`)。
...
「graph-core 那一側沒有別人在用」,workspace 這一側則由 LAW-25(d) 的 import 行守住。

### 「正規化」在本 spec 全篇的定義

> **正規化 = `System.Directory.canonicalizePath`**(WAVE-2 閘門裁決,design.md 契約 C 的 `vrPath`
> 值域欄已釘死)。
                      ...
                      Right r
                        | vmId (vrMarker r) /= veId e
                            → Left (WriteTargetIdDrift (veId e) (vrPath r) (vmId (vrMarker r)))
                              -- id 漂移:註冊表記的 id、路徑、marker 實際的 id(WAVE-3 閘門)
                        | otherwise → target = r
...
前三條逐字落實 design.md **契約 C 的「展開的走訪規則」**段(2026-08-29 WAVE-3 閘門裁決新增),
...
「決定寫入目標」——失敗是硬錯誤。F002 拆這兩個函式的整個理由就是這個(F002 的 ASM-1)。所以
...
那一格該回哪個 `WorkspaceError`,由 **2026-08-29 WAVE-3 閘門裁決**(本 spec 的 ASM-1 選 c):契約 F
...
翻譯」)且會叫使用者去修一個沒壞的 marker。完整的成本比較留在 ASM-1 備查。

`wsIssues` 因此**只**裝 `expandRefs` 產生的降級紀錄,**恒不含任何描述 `wsTarget` 的
`ScopeIssue`**(LAW-13(c))——這是「寫入目標決定不了就硬失敗」在型別以外的另一半保證。
...
| `WriteTargetIdDrift VaultId FilePath VaultId`(註冊表記的 id、路徑、marker 實際的 id) | `workspace/src/Aapms/Workspace/Types.hs`(由 **F001 的 impl** 補上,WAVE-3 閘門新增) | design.md 契約 F(2026-08-29 WAVE-3) | `resolveWrite` 的 id 守門(LAW-13(b))**唯一**產生它;本 feature **不宣告、不寫它的訊息** |
...
| `readMarker :: FilePath -> IO (Either StoreError VaultMarker)`、`markerDir`、`configPath`(vault 的) | `store/src/Aapms/Store/Marker.hs:46-93` | graph-core F005 | **本 feature 一個都不 import**(LAW-25(b));列出是因為測試要拿 `readMarker` 造預期值、拿 `configPath` 算 `VaultMarkerMissing` 帶的路徑 |
| `maxAttachedVaults :: Int`(= 10)、`TooManyVaults Int Int` | `store/src/Aapms/Store/MultiVault.hs:126-127`、`store/src/Aapms/Store/Error.hs:76` | graph-core F009 | **本 feature 明確不做**;列出是為了 LAW-24 指得出「這件事屬於誰」 |
...
任何型別(沿用 WAVE-1 / WAVE-2 立下的慣例)。兩個私有 helper(`walkAll` / `expandRefs` 的實作)
...
用來驗 LAW-19。`Hub` 是不透明型別,測試造它走 `mkHub vs ps llm tools txt`(F001 的唯一建構入口)。

## Laws

### 三個函式共用

- **LAW-1(保序去重,以 `vmId`)**:對任意輸入,`rsVaults` / `wsRead` / `psRuns` 三個清單裡任兩個
  元素的 `vmId (vrMarker …)` 互不相同;而且清單等於「走訪序列」以 `vmId` 為鍵、**保留首次
  出現位置**的去重結果(`nubOn vmId`),不是排序、不是取最後一次。
- **LAW-2(marker 是真相)**:三個清單裡每一個 `VaultRef r`,`vrMarker r` 逐欄等於直接對 `vrPath r`
  呼叫 `readMarker` 的結果。把中樞那一列的 `veName` / `veKind` 換成與 marker **不同**的任何值
  後重跑,三個函式的結果**逐欄不變**(中樞是快取,marker 才是真相)。
- **LAW-3(不可達不中止)**:某個候選 vault 路徑不見 / marker 壞 / id 漂移時,`resolveRead` 與
  `resolvePipeline` **恒回 `Right`**;該 vault 的 `vmId` 不出現在結果清單裡,而
  `readVaultRef` 對它回的那一則 `ScopeIssue` **逐欄相同地**出現在對應的 `*Issues` 裡。
  `resolveWrite` 在**非寫入目標**的候選上同樣如此(寫入目標見 LAW-12 / LAW-13)。
- **LAW-4(三個函式都不動檔案系統)**:對任意輸入,呼叫前後 `T` 底下的整棵目錄樹(檔案清單 +
  逐位元組內容)相同;特別是**不會建立** `.aapms/`、不會建立或開啟任何 `index.db`、不會修補
  壞掉的 marker、不會寫回中樞。
- **LAW-5(`*Issues` 的來源受限)**:`RefVaultNotRegistered` **只**由 `refs` 展開產生——
  `resolveRead hub Nothing` 與 `resolvePipeline hub k Nothing` 的 `*Issues` 恒不含它(那兩條路
  不展開 `refs`),即使中樞裡某個 vault 的 `refs` 指向未註冊的 id。

### `resolveRead`

- **LAW-6(無 selector = 全部已註冊,與當前目錄無關)**:`resolveRead hub Nothing` 的 `rsVaults`
...
- **LAW-7(有 selector = `{X} ∪ refs*(X)`,BFS 順序;design.md 契約 C「展開的走訪規則」第二條)**:`resolveRead hub (Just s)` 在
...
- **LAW-10(`wsTarget` 恒不來自 `refs` 展開)**:對任意輸入,若 `resolveWrite hub sel start == Right ws`,
  ...
  `wsTarget` 逐欄不變。
- **LAW-11(`wsRead` = `{目標} ∪ refs*(目標)`,目標排第一)**:`head (map (vmId . vrMarker) (wsRead ws))
  == vmId (vrMarker (wsTarget ws))`;其餘元素與 `resolveRead` 對同一個種子的展開結果**逐欄相同**
  (同一個 BFS、同一套保序去重)。`wsRead` 恒非空。
- **LAW-12(沒有 selector 且探測不到 → `NoWriteTarget`,帶正規化後的起點)**:`sel == Nothing` 且
  ...
  這個字串**。
- **LAW-13(寫入目標的失敗一律是 `WorkspaceError`,不是 `ScopeIssue`)**:
...
- **LAW-17(有 selector 且 kind 相符 = 恰好一個,不展開 `refs`)**:`psRuns` 的長度恰好 1,
  其 `vmId` 等於 selector 命中那一列的 `veId`;`psIssues == []`。把該 vault 的 `refs` 改成
  任何值,`psRuns` 逐欄不變(管線每次執行都寫自己的索引,`refs` 展開進來的一律唯讀)。
- **LAW-18(有 selector 且 kind 不符 → `VaultKindMismatch`,三個值)**:回
  ...
  `renderVaultKind got`。
- **LAW-19(有 selector 但該 vault 不可達 → `Right` + issue,不是 `VaultKindMismatch`;design.md
...
- **LAW-20(遞移閉包、對環安全、恒終止;design.md 契約 C 性質 3 + 「展開的走訪規則」第二條)**:
...
- **LAW-21(未註冊的 `refs` 目標降級為警告;ADR-017 2026-08-29 補充 + design.md 契約 C「展開的
  ...
  **BFS 序最早**的那一個來源。
- **LAW-22(不可達的節點不展開它自己的 `refs`,三種不可達一視同仁;design.md 契約 C「展開的走訪
...
- **LAW-23(展開進來的一律唯讀)**:`resolveWrite` 的 `wsRead` 裡除第一個元素外,沒有任何一個
  的 `vmId` 會等於 `vmId (vrMarker (wsTarget ws))`(LAW-1 的去重保證),而且對任意 `refs` 圖,
  `wsTarget` 都不是「只能經由 `refs` 走到」的那些節點之一;`psRuns` 恒不含任何只經由 `refs`
  走到的 vault。

### 明確不做

- **LAW-24(不判 ATTACH 上限)**:中樞含 11 個以上讀得到 marker 的 vault 時,`resolveRead hub Nothing`
...
- **LAW-25(以 import 行驗證;**比對前先去除行尾 `\r`**)**:專案的 `core.autocrlf = true` 讓 `.hs`
  在乾淨 checkout 上是 CRLF,逐字比對前必須先把行尾的 `\r` 去掉(WAVE-1 的 LAW-17(d) 與 WAVE-2 的 LAW-18
  ...
  - (b) **若**有對 `Aapms.Store.Marker` 的 import 行,它**必須逐字是**
    ...
    **這條 law 守的是「Scope 只讀 marker 的三個欄位、自己不讀 marker」**:清單放行的是三個
    ...
    import 警告;impl 填本體時才會出現這一行。與 F002 的 LAW-18(b) 同一個處置。)
  ...
  - (d) **完全不得** import `Aapms.Store.Atomic`(不寫任何檔案)、`Aapms.Store`(門面)、
    ...
    (ATTACH 上限屬 graph-core,見 LAW-24)、`Aapms.Store.Index` / `Query` / `Write` / `Create` /
  ...
  - (f) **若**有對 `System.Directory` 的 import 行,它**必須逐字是**
    ...
    (那是 `readVaultRef` 的 `VaultPathMissing`)、**不建立也不刪除任何東西**(LAW-4)、
    **不看當前目錄**(LAW-6 的「與當前目錄無關」——`getCurrentDirectory` 一旦出現就是違反)。
...
>
> - **預期綠**:**LAW-25 的六條子斷言 (a)–(f) 全部**。它們驗的是骨架原文自身就承載的事實
...
| # | 輸入 | 預期 | 覆蓋的 law |
|---|---|---|---|
| EX-1 | `resolveRead h Nothing` | `Right rs`;`map (vmId . vrMarker) (rsVaults rs) == [vlt-aaaa1111, vlt-bbbb2222, vlt-cccc3333, vlt-dddd4444]`(中樞順序);`rsIssues rs == [VaultMarkerBroken mEntry err, VaultPathMissing pEntry (T/gone 的正規化), VaultIdDrift zEntry (VaultId "vlt-99998888")]` | LAW-3, LAW-6 |
| EX-2 | 把當前目錄切到 `T/a/deep`,再切到 `T` 外的另一個暫存目錄,各跑一次 `resolveRead h Nothing` | 兩次結果**逐欄相同**,且與 EX-1 相同 | LAW-6 |
| EX-3 | `resolveRead h (Just "a")` | `Right rs`;`map (vmId . vrMarker) (rsVaults rs) == [vlt-aaaa1111, vlt-bbbb2222, vlt-cccc3333]`(BFS,`A → B → C → A` 成環仍終止且不重複);`rsIssues rs == []` | LAW-7, LAW-20 |
| EX-4 | `resolveRead h (Just "vlt-aaaa1111")`(改用 id) | 與 EX-3 **逐欄相同** | LAW-7 |
| EX-5 | `resolveRead h (Just "d")`(`refs == []`) | `Right (ReadScope [D 的 ref] [])`——恰好一個 | LAW-7 |
| EX-6 | `resolveRead h (Just "nope")` | `Left (VaultSelectorNotFound "nope")`,與 `lookupSelector h "nope"` 回的逐欄相同 | LAW-8 |
| EX-7 | 中樞多加兩列同名 `veName == "dup"`;`resolveRead h' (Just "dup")` | `Left (VaultSelectorAmbiguous "dup" [兩列])`,原樣透傳 | LAW-8 |
| EX-8 | 把 A 的 marker `refs` 改成 `["vlt-bbbb2222", "vlt-ffff0000"]`(後者中樞查不到);`resolveRead h (Just "a")` | `rsVaults` 仍是 `[A, B, C]`;`rsIssues` 含 `RefVaultNotRegistered (VaultId "vlt-aaaa1111") (VaultId "vlt-ffff0000")` | LAW-21 |
| EX-9 | `resolveRead h (Just "m")`(種子的 marker 壞) | `Right (ReadScope [] [VaultMarkerBroken mEntry err])`——仍是 `Right`,空清單 | LAW-3, LAW-9 |
| EX-10 | `resolveRead h (Just "p")`(種子路徑不存在) | `Right (ReadScope [] [VaultPathMissing pEntry (T/gone 的正規化)])` | LAW-9 |
| EX-11 | `resolveRead h (Just "z")`(種子 id 漂移) | `Right (ReadScope [] [VaultIdDrift zEntry (VaultId "vlt-99998888")])`,且**不含任何 `RefVaultNotRegistered`** | LAW-9 |
| EX-12 | 造一個自環 `F`(`refs = ["vlt-ffff1111"]` 指自己);`resolveRead h'' (Just "f")` | `rsVaults` 恰好一個 F,終止 | LAW-20 |
| EX-13 | EX-8 的中樞;`resolveRead h Nothing` | `rsIssues` **不含**任何 `RefVaultNotRegistered`(無 selector 不展開 `refs`) | LAW-5 |
| EX-14 | 中樞加第二列 `veId == vlt-dddd4444`、`vePath == T/d`(重複 id,以 `mkHub` 直接造);`resolveRead h''' Nothing` | `rsVaults` 裡 `vlt-dddd4444` **只出現一次**,位置是**第一列**的位置 | LAW-1 |
| EX-15 | 把 A 的中樞那一列的 `veName` 改成 `"stale"`、`veKind` 改成 `StoryVault`(marker 不動);`resolveRead h (Just "vlt-aaaa1111")` | 結果與 EX-4 **逐欄相同**(`vrMarker` 全部來自檔案) | LAW-2 |
| EX-16 | `resolveWrite h Nothing (T/a/deep/deeper)` | `Right ws`;`vmId (vrMarker (wsTarget ws)) == vlt-aaaa1111`;`map (vmId . vrMarker) (wsRead ws) == [vlt-aaaa1111, vlt-bbbb2222, vlt-cccc3333]`;`wsIssues ws == []` | LAW-10, LAW-11 |
| EX-17 | `resolveWrite h Nothing T2`(`T2` 是一個一路到根都沒有 `.aapms/` 的暫存目錄) | `Left (NoWriteTarget T2')`,`T2'` 是 `canonicalizePath T2`;`renderWorkspaceError` 的輸出含 `T2'` | LAW-12 |
| EX-18 | `resolveWrite h Nothing (T/e/x)`(E 未註冊) | `Right ws`;`vrEntry (wsTarget ws) == Nothing`;`wsRead ws` 恰好一個 E | LAW-14 |
| EX-19 | `resolveWrite h (Just "z") (T/a)`(selector 指到 id 漂移的 Z) | `Left (WriteTargetIdDrift (VaultId "vlt-77776666") (T/z 的正規化) (VaultId "vlt-99998888"))`;`renderWorkspaceError` 的輸出同時含 `"vlt-77776666"`、該路徑、`"vlt-99998888"` 三個值 | LAW-13(b) |
| EX-20 | `resolveWrite h (Just "p") (T/a)`(中樞那列路徑不存在) | `Left (MarkerUnreadable p' (VaultMarkerMissing (Aapms.Store.Marker.configPath p')))`,`p'` 是 `T/gone` 的正規化 | LAW-13(a) |
| EX-21 | `resolveWrite h (Just "m") (T/a)`(marker 壞) | `Left (MarkerUnreadable m' err)`,`err` 與直接呼叫 `readMarker m'` 的結果**逐欄相同** | LAW-13(a) |
| EX-22 | `resolveWrite h Nothing (T/m/deep)`(探測命中 M,但 marker 壞) | `Left (MarkerUnreadable m' err)`——探測命中之後 marker 壞仍是**硬失敗** | LAW-13(a) |
| EX-23 | `resolveWrite h (Just "b") (T/a/deep)`(起點在 A 裡,selector 指 B) | `vmId (vrMarker (wsTarget ws)) == vlt-bbbb2222`;`map (vmId . vrMarker) (wsRead ws) == [vlt-bbbb2222, vlt-cccc3333, vlt-aaaa1111]`——A 只以**唯讀**身分經 `refs` 進來,`wsTarget` 仍是 B | LAW-10, LAW-15, LAW-23 |
| EX-24 | EX-23 的同一組輸入,起點改成 `T2`(不在任何 vault 內) | 與 EX-23 **逐欄相同** | LAW-15 |
| EX-25 | EX-8 的 marker;`resolveWrite h (Just "a") (T/a)` | `wsIssues ws` 含 `RefVaultNotRegistered (VaultId "vlt-aaaa1111") (VaultId "vlt-ffff0000")`,且**不含**任何以 `wsTarget` 為對象的 issue | LAW-13(c), LAW-21 |
| EX-26 | `resolvePipeline h AssetVault Nothing` | `Right ps`;`map (vmId . vrMarker) (psRuns ps) == [vlt-aaaa1111, vlt-cccc3333, vlt-dddd4444]`(B 是 story,**靜默排除**);`psIssues ps` 與 EX-1 的 `rsIssues` **逐欄相同** | LAW-16 |
| EX-27 | `resolvePipeline h StoryVault Nothing` | `psRuns` 恰好 `[B]` | LAW-16 |
| EX-28 | `resolvePipeline h AssetVault (Just "a")` | `Right (PipelineScope [A 的 ref] [])`——**恰好一個**,不含 B / C(`refs` 不展開) | LAW-17 |
| EX-29 | `resolvePipeline h AssetVault (Just "b")` | `Left (VaultKindMismatch (VaultId "vlt-bbbb2222") AssetVault StoryVault)`;`renderWorkspaceError` 的輸出同時含 `"vlt-bbbb2222"`、`"asset"`、`"story"` | LAW-18 |
| EX-30 | `resolvePipeline h AssetVault (Just "m")` | `Right (PipelineScope [] [VaultMarkerBroken mEntry err])`——**不是** `VaultKindMismatch` | LAW-19 |
| EX-31 | `resolvePipeline h AssetVault (Just "nope")` | `Left (VaultSelectorNotFound "nope")` | LAW-8 |
| EX-32 | 造 11 個 kind 都是 `AssetVault` 的 vault 並全部註冊;`resolveRead h4 Nothing` 與 `resolvePipeline h4 AssetVault Nothing` | 兩者都回 `Right`,清單長度都是 11,**沒有任何錯誤**(ATTACH 上限屬 graph-core) | LAW-24 |
| EX-33 | EX-1 / EX-16 / EX-26 三次呼叫的前後,對 `T` 遞迴列出全部檔案與內容 | 三次前後完全相同——沒有新增 `.aapms/`、沒有 `index.db`、壞掉的 M 沒有被修補 | LAW-4 |
| EX-34 | 菱形 `refs`:`A → [B, C]`、`B → [D]`、`C → [D]`;`resolveRead h5 (Just "a")` | `map (vmId . vrMarker) (rsVaults rs) == [A, B, C, D]`——BFS 順序,D 只出現一次 | LAW-1, LAW-7, LAW-20 |
| EX-35 | `refs` 指向 M(marker 壞):`A → [M]`;`resolveRead h6 (Just "a")` | `rsVaults == [A]`;`rsIssues` 含 `VaultMarkerBroken mEntry err`;把 M 的 `refs` 改成任何值結果不變(不可達的節點不展開) | LAW-22 |
| EX-36 | `refs` 指向 **Z**(marker 讀得到,但 id 漂移):`A → [vlt-77776666]`,而 Z 的 marker `refs = ["vlt-dddd4444"]`;`resolveRead h7 (Just "a")` | `rsVaults == [A]`——Z 自己不進結果集,**D 也不會被展開進來**;`rsIssues` 含 `VaultIdDrift zEntry (VaultId "vlt-99998888")`。把 Z 的 `refs` 改成任何值結果逐欄不變:marker 讀得到不代表它說的話可信 | LAW-22 |
...
- **誰會依賴它**:`aapms-service`(S3,`Env` 要靠三個裁決函式決定開哪些 vault),目前尚未存在。
...
**編排者**補上(DEC-2:`.cabal` 由編排者單線維護,本 feature 不得修改)。`cabal build
...
**WAVE-3 閘門裁決後骨架一個字未改**:`WriteTargetIdDrift` 只出現在 `resolveWrite` 的**本體**裡,
而本體是 `undefined`;三條簽名與匯出清單與交件當下逐字相同。

## TodoList

- [ ] STEP-1: 私有 `walkAll`:依 `hubVaults` 順序對每一列跑 `readVaultRef e (vePath e)`,成功的進
  結果、失敗的進 issues;以 `vmId` 保序去重 `dep: -`
- [ ] STEP-2: 私有 `expandRefs`:BFS 佇列帶 `(來源 VaultId, 目標 VaultId)`,`Data.Set` 的 visited
  以「走到它時用的 `VaultId`」為鍵(種子先放它的 `vmId`);中樞查不到 → `RefVaultNotRegistered`,
  `readVaultRef` 失敗 → 進 issues 且**不展開**;以 `vmId` 保序去重 `dep: -`
- [ ] STEP-3: `resolveRead`:`Nothing` → `walkAll`;`Just s` → `lookupSelector`(失敗原樣透傳)→
  `readVaultRef` 種子(失敗 → `Right` + 空清單 + 一則 issue)→ `expandRefs` `dep: T1, T2`
- [ ] STEP-4: `resolveWrite` 的**寫入目標**:`Nothing` → `detectVault`,沒命中就
...
- [ ] STEP-5: `resolveWrite` 的 `wsRead`:對 `wsTarget` 跑 `expandRefs`,目標排第一;`wsIssues`
  只收 `expandRefs` 的產出 `dep: T2, T4`
- [ ] STEP-6: `resolvePipeline`:`Nothing` → `walkAll` 後過濾 `vmKind == k`(不符不產生 issue);
...
| Todo | Law / Example | 測試 |
|------|---------------|------|
| STEP-1 | LAW-1, LAW-6 / EX-1, EX-2, EX-14 | `test_resolve_read_all_registered_in_hub_order`、`test_resolve_read_independent_of_cwd`、`test_scope_dedupes_by_vmid_keeping_first` |
| STEP-2 | LAW-20, LAW-21, LAW-22 / EX-8, EX-12, EX-34, EX-35, EX-36 | `test_refs_expansion_is_transitive_and_cycle_safe`、`test_refs_self_loop_terminates`、`test_refs_diamond_bfs_order`、`test_refs_unregistered_target_degrades`、`test_unreachable_node_refs_not_expanded`、`test_id_drift_node_refs_not_expanded`(EX-36,WAVE-3 閘門把「id 漂移也不展開」升格為契約後補) |
| STEP-3 | LAW-2, LAW-3, LAW-5, LAW-7, LAW-8, LAW-9 / EX-3–EX-5, EX-6, EX-7, EX-9–EX-11, EX-13, EX-15 | `test_resolve_read_selector_is_closure`、`test_resolve_read_selector_by_id_equals_by_name`、`test_resolve_read_selector_not_found_passthrough`、`test_resolve_read_selector_ambiguous_passthrough`、`test_resolve_read_seed_unreachable_still_right`、`test_resolve_read_no_selector_never_expands_refs`、`test_resolve_read_marker_is_truth` |
| STEP-4 | LAW-12, LAW-13(a), LAW-13(b), LAW-14, LAW-15 / EX-17–EX-24 | `test_resolve_write_no_target_carries_canonical_start`、`test_resolve_write_target_marker_unreadable_is_hard_error`、`test_resolve_write_id_drift_is_hard_error`(斷言 `WriteTargetIdDrift` 三個值,WAVE-3 閘門)、`test_resolve_write_unregistered_target_allowed`、`test_resolve_write_selector_ignores_start` |
| STEP-5 | LAW-10, LAW-11, LAW-13(c), LAW-23 / EX-16, EX-23, EX-25 | `test_resolve_write_target_never_from_refs`、`test_resolve_write_read_starts_with_target`、`test_resolve_write_issues_never_describe_target` |
| STEP-6 | LAW-16, LAW-17, LAW-18, LAW-19 / EX-26–EX-31 | `test_resolve_pipeline_filters_by_kind`、`test_resolve_pipeline_kind_mismatch_is_silent_without_selector`、`test_resolve_pipeline_selector_is_single_run`、`test_resolve_pipeline_kind_mismatch_carries_three_values`、`test_resolve_pipeline_unreachable_is_right` |
| (全部) | LAW-4, LAW-24 / EX-32, EX-33 | `test_scope_touches_no_files`、`test_scope_has_no_attach_limit` |
| (全部) | LAW-25 (a)–(f) | `test_scope_no_downstream_or_location_imports`(a)、`test_scope_marker_import_is_three_fields_only`(b,條件式逐字比對 `import Aapms.Store.Marker (VaultMarker (vmId, vmKind, vmRefs))`)、`test_scope_schema_import_is_type_only`(c,逐字比對 `import Aapms.Store.Schema (VaultKind)`,**骨架階段就實質比對**)、`test_scope_never_imports_store_internals`(d)、`test_scope_no_process_import`(e)、`test_scope_directory_import_is_canonicalize_only`(f)。**六條都只掃 import 行,比對前先去除行尾 `\r`** |

## 待確認假設

> **本段已全數裁決,保留備查。** ASM-1 於 **2026-08-29 WAVE-3 閘門裁決成選項 c**(新增
> `WriteTargetIdDrift`),與本 spec 的暫採 a **不同**;正文的 LAW-13(b)、EX-19、`resolveWrite` 的
> 資料流與收斂表都已改成 c。選項比較原文原樣留著,是為了讓後續要動這個建構子的人看得到 a / b
> 為什麼被否決。

- ASM-1: `resolveWrite` 的**寫入目標**上出現 `ScopeIssue` 時(路徑不見 / marker 壞 / **id 漂移**),
  ...
  **原件**,零成本、無爭議(見「實作方式」的收斂表)。**剩下的是 id 漂移**——`readVaultRefAt`
  ...
  - 選項:
    ...
    c) **新增建構子,例如 `WriteTargetIdDrift VaultId FilePath VaultId`(中樞記的 id、目標
       ...
       `renderWorkspaceError` +1 則訊息),而那**打破 DEC-2「`Types.hs` 在 WAVE-1 一次寫齊,WAVE-2 之後
       沒人再碰它」的併發前提**——WAVE-4 的三個 feature 平行寫作正是建立在這條上;此外 F001 的
       驗收 law「`renderWorkspaceError` 對**每一個**建構子都回一段非空的繁中訊息」的測試要
       跟著改,等於 WAVE-1 要多跑一輪 qa,`aapms-workspace.cabal` 雖不用動但 F001 文檔要回寫。
       三個月後代價:**最小**——訊息說得出三個值與正確的下一步(「中樞說 `vlt-X` 在這裡,
       但這裡的 marker 是 `vlt-Y`;請執行 `aapms doctor` 或 `vault add` 更新中樞」),
       `service` 的 `errorCode` 表也多一格明確的碼。**注意:WAVE-3 是單 feature 波次,此刻沒有
       任何併發寫入 `Types.hs` 的風險——DEC-2 的前提要到 WAVE-4 才真正生效,所以「現在改」比
       「WAVE-4 之後再改」便宜一個數量級**
    d) **沿用 `HubMalformed`(中樞與現實不一致,叫使用者去修中樞)**——當下成本:語意上最貼切
       ...
       它」,以及 LAW-25(a) 為什麼要把 `Aapms.Workspace.Location` 一起禁掉
  - 傾向:**c**,但**暫採 a**。理由分兩層。**設計上 c 是對的**:a 與 b 都是把一則會誤導使用者
    ...
    (+1 建構子 +1 則訊息)、本 spec 的 LAW-13(b) 一條、EX-19 一格、`Scope.hs` 一行、F001 的
    `renderWorkspaceError` 測試一處——**沒有任何 law 需要重寫,骨架一個字不用改**。
    我依賴的前提有兩個,都經查證:(i) WAVE-3 是單 feature 波次,`build-log.md` 的排程表只列
  ...
  - 暫採:a(`Left (NoWriteTarget (vrPath r))`)→ 影響:若裁決成 c,改 `Types.hs`
    (+`WriteTargetIdDrift VaultId FilePath VaultId` 與它的繁中訊息)、本 spec 的 LAW-13(b)、
    EX-19、1-to-1 表裡 `test_resolve_write_id_drift_is_hard_error` 那一格的預期值,以及 F001
    文檔裡「`renderWorkspaceError` 的全部建構子」那條 law 的計數;若裁決成 b,只改 LAW-13(b)
    與 EX-19,但要在 spec 明寫「這裡的 `StoreError` 是本層合成的,不是 graph-core 的原件」
    ——那會是一條與 design.md 契約 F 原文衝突的例外,建議一併回寫 design.md
  - **裁決(2026-08-29 WAVE-3 閘門):選 c**。`design.md` 契約 F 已新增
    `WriteTargetIdDrift VaultId FilePath VaultId`(註冊表記的 id、路徑、marker 實際的 id),
    並寫明它與 `ScopeIssue.VaultIdDrift` 是同一件事的兩種身分、訊息要說出
    `vault check` / `syncHub` / 重新 `vault add`。開發者採納了「WAVE-3 是單 feature 波次、此刻
    沒有併發對象,現在改比 WAVE-4 之後便宜一個數量級」這條論證。**宣告與訊息由 F001 的 impl 加進
    `Types.hs`、F001 的 qa 補那一格斷言;本 feature 不碰那個檔**,只在 `resolveWrite` 的 id
    守門產生它。本 spec 實際改動:LAW-13(b)、EX-19、`resolveWrite` 資料流、收斂表、TodoList STEP-4、
    1-to-1 表 STEP-4 那一格——**law 總數不變(25 條)、EX-19 只換預期值,骨架一個字不改**

## 實作備註

### 2026-08-29 WAVE-3 閘門裁決(兩條)

**裁決一(本 spec 的 ASM-1)——選 c,契約 F 新增 `WriteTargetIdDrift VaultId FilePath VaultId`。**
`resolveWrite` 的 selector 路徑在「註冊表記的 `veId` 與目標 marker 的 `vmId` 不符」時回它,
三個值依序是註冊表記的 id、目標路徑、marker 裡實際的 id。被否決的 a(`NoWriteTarget`)與
b(`MarkerUnreadable` + 自造 `StoreError`)都會說謊,理由見 ASM-1 原文與 design.md 契約 F 的
新增段。**型別宣告與繁中訊息由 F001 的 impl 加進 `Types.hs`,不在本 feature 的寫入白名單**;
本 feature 是它唯一的產生點。改動落在 LAW-13(b)、EX-19、`resolveWrite` 的資料流與收斂表、
TodoList STEP-4、1-to-1 表 STEP-4 那一格,**骨架 `Scope.hs` 一個字未改**(簽名相同,新建構子只出現在
`undefined` 的本體裡)。
...
**完全一致**,所以 law 的**條數與判準**(LAW-7 / LAW-19 / LAW-20 / LAW-21 / LAW-22)都沒變;本次只把它們在
「三個函式的資料流」與相關 law 裡的**出處**從「自裁」改成 design.md 契約 C 的引用、把「三種
不可達一視同仁(含 id 漂移)」與「BFS 種子排第一」從散文提升成 law 條文裡的明文,並在
「不可逆決定」表標註閘門日期。**唯一新增的是 EX-36**:裁決一把「id 漂移的節點也不展開它的
`refs`」明確化之後,原本只有 EX-35(marker 壞)這一路被覆蓋,「marker 讀得到但不採信」那一路
沒有 example——那正是這條契約最容易被實作反過來寫的一格。Examples 因此由 35 條增為 **36 條**。

**編排者的升級篩結果**:本 spec 交件時列的九條自裁(SELF-1、SELF-2、SELF-4、SELF-5、SELF-6、SELF-7、SELF-8、SELF-9、SELF-10)
全部被判為**契約層級**——它們的「層級自答」欄自己寫著「邊界:會」,依 `boundary-rules.md` 就
不該進自裁清單。其中 SELF-2 / SELF-4 / SELF-5 / SELF-9 即裁決二的四條,已升格進 design.md 契約 C;
SELF-1(`NoWriteTarget` 帶正規化後的起點)、SELF-6(無 selector 時 kind 不符靜默排除)、
SELF-7(`resolvePipeline` 兩條路都不展開 `refs`)、SELF-8(`VaultKindMismatch` 取 `vmId` 不取 `veId`)、
SELF-10(以 `vmId` 保序去重、保留首次出現位置)沿用既有裁決,條文不動。本 spec 正文自此**不再
