...
**2026-08-29 WAVE-4 閘門裁決(本 spec 的 ASM-1,裁決為選項 a)**:契約 E 增列
...
- **不建立、不修改、不刪除任何檔案或目錄**,也不改任何檔案的權限(LAW-11、LAW-15(d))。
- **不啟動任何外部行程**(LAW-11、LAW-15(b))。
- **不讀中樞檔案**:`ToolsConfig` 是參數,由呼叫端(`service`)以 `hubTools` 取出後交進來;
  本模組不碰 `config.toml`、不碰任何 vault(LAW-15(c))。
...
-- 2026-08-29 WAVE-4 閘門裁決(本 spec 的 ASM-1)新增;detectSevenZip 的原簽名一個字不動
...
前兩個型別**已由 F001 宣告完畢**(`Types.hs:259-280`),本 feature 一個字都不改;後兩項是 WAVE-4
閘門補進契約 E 的,定義住 `Aapms.Workspace.Tools`。契約 E 的欄位值域(design.md 的表)逐條
落成 Laws:

| 欄位 | 契約原文的值域 | 落在哪一條 law |
|---|---|---|
| `tsName` | 固定字串,如 `"7-Zip"` | LAW-13 |
| `tsPath` | 絕對路徑;`Nothing` ⟺ `tsOrigin == NotFound` | LAW-5、LAW-8 |
| `tsOrigin` | 這個路徑**哪裡來的** | LAW-6 |
| `tsSearched` | 依序、去重;`NotFound` 時**必為非空** | LAW-1、LAW-4、LAW-9 |
...
1. **`getPermissions` 的 `executable` 在 Windows 上看副檔名,而且對目錄回 `False`。**
   ...
   `executable` 就是 x 位元。
   → 「可執行」的跨平台判準因此可以**委給 `directory`**,不必在 workspace 裡寫任何平台分支(ASM-2)。
2. **`getPermissions` 對不存在的路徑會拋 `isDoesNotExistError`**(實測:
   `getPermissions "…/nope.exe"` → `Left … does not exist`)。契約 E **沒有失敗通道**,所以判準
   必須先 `doesFileExist` 再 `getPermissions`,順序不可調換(LAW-7、LAW-12)。
3. **`findExecutable` 在 Windows 上呼叫 Win32 的 `SearchPath`**(haddock 原文:「may search other
   ...
   環境變數(ASM-3)。`findExecutablesInDirectories` 的 haddock 明說它「does not use `SearchPath`」
...
5. **`exeExtension` 由 `directory-1.3.10.0` 的 `System.Directory` 匯出,不是 `filepath` 的。**
   ...
   副檔名不會補第二個點);一條式子兩個平台都對,law 因此寫得出跨平台的版本(LAW-10)。

   > **2026-08-29 WAVE-4 更正(spec-gaps GAP-3)**:本條原文寫的是「`exeExtension` 在 Windows 是
   ...
   > 獨立查證後修正,連帶要改 LAW-15(d) 與 LAW-15(f) 的白名單歸屬。
...
6. **`normalise` / `makeAbsolute` 在 Windows 會把 `/` 換成 `\`**(實測
   ...
   ——**正斜線**。任何一種正規化都會讓驗收標準第 1 條的「`tsPath` 等於它」變成不成立(ASM-5)。
7. **`loadHub` 已經擋下非絕對的 `seven_zip`**(`workspace/src/Aapms/Workspace/Hub.hs:183-191`):
   ...
   → 「`tcSevenZip` 是絕對路徑」這個事實**已經有人守**,本模組不重複驗一次(ASM-5 的前提,已查證)。
8. **本機的 `PATH` 有 45 個目錄**(實測),`PATHEXT` 是
   ...
   展開整份 `PATHEXT` 是 45 × 2 × 12 = **1080 條**(ASM-3、ASM-4)。
...
而且 Windows 的 ACL 判定需要新增套件依賴(選項見 ASM-2)。
...
detectSevenZipIn plan cfg
  → 組出三層的候選檔案路徑,依序串起來:
      tier1 = maybe [] (: []) (tcSevenZip cfg)                             -- 覆寫
      tier2 = [ d </> (n <.> exeExtension)                                 -- PATH
              | n <- ["7z", "7zz"], d <- tspPathDirs plan ]                --   名稱外層、目錄內層
      tier3 = tspCandidates plan                                           -- 內建候選清單
      probes = 保序去重 (tier1 ++ tier2 ++ tier3)
  → 依序對 probes 的每一項 p 問「合格?」
       第一個合格的 p → tsPath    = Just p
                        tsOrigin  = p 在去重後**最早**出現的那一層(tier1→FromToolsConfig /
                                    tier2→FromPath / tier3→FromCandidate)
...
- **命中即停**是驗收標準第 1 條的全部內容:覆寫合格時 `probes` 的探測在第一項就結束,
  ...
  LAW-2 用「把 plan 的兩個清單換成任何值,結果逐欄不變」來驗這件事,不依賴任何內部觀察)。
...
| `exeExtension :: String` | **`directory` 的 `System.Directory`**(**不是** `filepath`——2026-08-29 WAVE-4 更正,見「相依性查證」事實 5 與 spec-gaps GAP-3) | - | Windows `".exe"` / POSIX `""`,**值含前導點**;第二層的副檔名 |
...
**三條都在契約 E 內**(後兩條由 2026-08-29 WAVE-4 閘門補進契約,見 ASM-1 的裁決欄)。
...
| `data ToolSearchPlan = ToolSearchPlan { tspPathDirs :: [FilePath], tspCandidates :: [FilePath] }` | 三層裡「來自環境」的那兩層的來源。WAVE-4 閘門納入契約 E | `workspace/src/Aapms/Workspace/Tools.hs:49-57` |
| `detectSevenZipIn :: ToolSearchPlan -> ToolsConfig -> IO ToolStatus` | 把那兩層的來源變成**明碼參數**:給定兩份清單,依序探測。**確定性**——同樣的 plan、同樣的檔案系統就是同樣的結果。WAVE-4 閘門納入契約 E | `workspace/src/Aapms/Workspace/Tools.hs:81` |

**這不是測試後門,是「把隱含參數變明碼、原函式變薄包裝」這個既有模式的第三次套用**:
先例是 graph-core 的 `allocateId`(2026-08-25 GAP-8 裁決),同一波的 F005 也依同一個判準把
`allocateProjectId` 提進契約。三者的共同形狀是——原簽名一字不動、新入口只描述**環境事實的
來源**(時間 / 亂數種子 / `PATH` 與候選清單),不洩漏任何內部演算法,而且兩個入口的一致性
由一條 law 釘住(本 feature 是 LAW-14)。
...
| `ToolSearchPlan` | **新增**(WAVE-4 閘門納入契約 E;定義住 `Aapms.Workspace.Tools`,**不進已凍結的 `Types.hs`**) | `{ tspPathDirs :: [FilePath], tspCandidates :: [FilePath] }` | 「這一次探測要掃哪些目錄、要試哪些固定路徑」 |

### `tsName` 的值

**逐字 `"7-Zip"`**,取自 design.md 契約 E 欄位表的 `tsName` 那一列(「固定字串,如 `"7-Zip"`」)。
不論結果如何都是這個字串,`NotFound` 時也一樣(LAW-13)。
...
- **LAW-1(`tsSearched` 是 `probes` 的前綴,命中的那一項在最後)**:對任意 `plan`、任意 `cfg`,
  令 `ps = probes plan cfg`。若 `tsPath == Just p`,則:(a) `p` 是 `ps` 中**第一個**滿足
  `qualifies` 的元素;(b) `tsSearched` 逐項等於 `ps` 從頭到 `p`(含 `p`)的前綴;
  (c) `last tsSearched == p`。
- **LAW-2(覆寫合格時後兩層完全不參與)**:若 `tcSevenZip cfg == Just p` 且 `qualifies p`,則
  ...
  **且與 `plan` 無關**——把 `tspPathDirs` 與 `tspCandidates` 換成任意兩份清單(含空清單、含
  裡面全是合格檔案的清單),結果逐欄不變。
- **LAW-3(覆寫不合格時不中止)**:若 `tcSevenZip cfg == Just p` 而 `not (qualifies p)`,則
...
- **LAW-7(合格 = 存在且可執行,依此順序)**:一個路徑 `p` 被算成命中,當且僅當
...
- **LAW-8(逐字,不做任何正規化)**:`tsPath` 與 `tsSearched` 的每一項都與它的來源**逐字相同**:
  ...
  絕對化、symlink 不會被解開。
- **LAW-9(依序、去重,且去重跨層)**:`tsSearched` 沒有重複元素;若同一個路徑在多層出現,它只在
  **最早**出現的位置留下一項,`tsOrigin` 依那一次所屬的層決定(LAW-6)。`tsSearched` 的順序恒為
  `probes` 的順序,不重排、不排序。
- **LAW-10(第二層的展開式,兩個平台同一條)**:給定 `tspPathDirs plan == [d1, …, dn]`,`probes` 的
  第二段逐項等於
...
- **LAW-11(不執行、不動檔案系統)**:對任意 `plan`、`cfg`,呼叫前後 `tsSearched` 涉及的每一個
...
- **LAW-12(沒有失敗通道)**:對任意 `plan`、任意 `cfg`、任意檔案系統狀態,`detectSevenZipIn` 都
...
- **LAW-13(`tsName` 恒為 `"7-Zip"`)**:對任意 `plan`、`cfg`,`tsName == "7-Zip"`,`NotFound` 時
  也一樣。
- **LAW-14(兩個入口一致)**:令 `dirs = maybe [] splitSearchPath` 施用於 `lookupEnv "PATH"` 的結果、
  `cands` 是「數據」段逐字列出的那七條。對任意 `cfg`,`detectSevenZip cfg` 逐欄等於
  `detectSevenZipIn (ToolSearchPlan dirs cands) cfg`。

### 依賴方向與職責界線

- **LAW-15(以 import 行驗證;**比對前先去除行尾 `\r`**)**:專案的 `.gitattributes` 讓 `.hs` 在
  ...
  - (d) **若**有 `import System.Directory` 的行,它的匯入清單必須是
    ...
    或被放寬成無清單的 `import System.Directory`,這條就紅——LAW-8(不正規化)、LAW-11(不動檔案
    系統)與 ASM-3(不用 `SearchPath`)因此守得住,而不是只寫在 Haddock 裡。
    **`exeExtension` 在這一格,不在 (f)**:它由 `directory` 匯出,不是 `filepath`
    (2026-08-29 WAVE-4 更正,spec-gaps GAP-3;見「相依性查證」事實 5)。
  ...
  - (e) **若**有 `import System.Environment` 的行,它的匯入清單必須是 `{lookupEnv}` 的
    **子集合**——`getEnv` 在變數未設時拋例外(違反 LAW-12),`setEnv` / `getArgs` /
    `getExecutablePath` 都不屬於本 feature。
  - (f) **若**有 `import System.FilePath` 的行,它的匯入清單必須是
    `{(</>), (<.>), splitSearchPath}` 的**子集合**——**`exeExtension` 已移出本格**
    (它由 `directory` 匯出,歸 (d);2026-08-29 WAVE-4 更正,spec-gaps GAP-3)。
    impl 交件時的實際 import 行逐字是:
    `import System.FilePath (splitSearchPath, (<.>), (</>))`

  **(d) 與 (f) 的判準仍是「子集合」,不是逐字比對**(未隨本次更正改變):GAP-3 這條 gap 正好是
  「白名單漏了一個實際需要的符號」——逐字比對會讓同一類漏列**直接變成紅燈而不是可通過的子集**,
  而 F002 的 GAP-2 是同一種傷(逐字白名單漏了 `vmId`,害 law 沒有任何合法實作滿足得了)。
...
>
> - **預期綠**:**LAW-15 的六條子斷言 (a)–(f) 全部**。它們驗的是骨架原文自身就承載的事實
...
| # | 輸入 | 預期 | 覆蓋的 law |
|---|---|---|---|
| EX-1 | `E1 = T` 下的合格檔 `seven <.> exeExtension`;`D` 下有合格的 `7z <.> exeExtension`;`C` 是合格的候選檔。`detectSevenZipIn (plan [D] [C]) (ToolsConfig (Just E1))` | `ToolStatus "7-Zip" (Just E1) FromToolsConfig [E1]` | LAW-1, LAW-2, LAW-5, LAW-6(a), LAW-13 |
| EX-2 | 同 EX-1,但 plan 換成 `plan [] []` | 與 EX-1 **逐欄相同** | LAW-2 |
| EX-3 | `P = T </> "nope.exe"`(**不建立**);`D` 下有合格的 `7z <.> exeExtension`(記為 `S`)。`detectSevenZipIn (plan [D] []) (ToolsConfig (Just P))` | `ToolStatus "7-Zip" (Just S) FromPath [P, S]`——覆寫不合格不中止,`P` 仍在 `tsSearched` | LAW-1, LAW-3, LAW-6(b), LAW-7(a) |
| EX-4 | `D1` 空、`D2` 下有合格的 `7z <.> exeExtension`(記為 `S2`)。`detectSevenZipIn (plan [D1, D2] []) (ToolsConfig Nothing)` | `tsPath == Just S2`、`tsOrigin == FromPath`、`tsSearched == [D1 </> "7z…", D2 </> "7z…"]` | LAW-1, LAW-6(b), LAW-10 |
| EX-5 | `D1` 下有合格的 `7zz <.> exeExtension`、`D2` 下有合格的 `7z <.> exeExtension`。`detectSevenZipIn (plan [D1, D2] []) (ToolsConfig Nothing)` | 命中的是 **`D2` 的 `7z`**(名稱外層):`tsSearched == [D1 </> "7z…", D2 </> "7z…"]`,`D1` 的 `7zz` **沒有**進 `tsSearched` | LAW-1, LAW-10 |
| EX-6 | `D` 下沒有任何合格檔;候選清單 `[C1, C2]`,`C1` 不存在、`C2` 是合格檔。`detectSevenZipIn (plan [D] [C1, C2]) (ToolsConfig Nothing)` | `tsOrigin == FromCandidate`、`tsPath == Just C2`、`tsSearched == [D </> "7z…", D </> "7zz…", C1, C2]` | LAW-1, LAW-6(c), LAW-10 |
| EX-7 | `D` 下沒有任何合格檔;候選清單 `[C1]`,`C1` 不存在。`detectSevenZipIn (plan [D] [C1]) (ToolsConfig Nothing)` | `ToolStatus "7-Zip" Nothing NotFound [D </> "7z…", D </> "7zz…", C1]`——**非空**,而且**沒有拋例外** | LAW-4, LAW-5, LAW-12, LAW-13 |
| EX-8 | `B = T </> ("bogus" <.> exeExtension)` 是一個**目錄**;`detectSevenZipIn (plan [] [B]) (ToolsConfig (Just B))` | `tsPath == Nothing`、`tsOrigin == NotFound`、`tsSearched == [B]`(去重後只有一項) | LAW-7(c), LAW-9 |
| EX-9 | `N = T </> "seven.txt"`(存在,未設執行權限);`detectSevenZipIn (plan [] [N]) (ToolsConfig (Just N))` | `tsPath == Nothing`、`tsOrigin == NotFound`、`tsSearched == [N]` | LAW-7(b), LAW-9 |
| EX-10 | `C1` 不存在;`detectSevenZipIn (plan [] [C1, C1]) (ToolsConfig (Just C1))` | `tsSearched == [C1]`——三層都提到同一個路徑,只留一項 | LAW-9 |
| EX-11 | `X = T </> "gone"`(不存在的目錄);`detectSevenZipIn (plan [X] [T </> "also-gone.exe"]) (ToolsConfig Nothing)` | 回 `ToolStatus`,不拋例外;`tsSearched == [X </> "7z…", X </> "7zz…", T </> "also-gone.exe"]`;呼叫後 `X` **仍然不存在** | LAW-11(a), LAW-12 |
| EX-12 | `R = D </> ("7z" <.> exeExtension)` 是一個「被執行就會建立 `D </> "RAN"`」的合格檔;`detectSevenZipIn (plan [D] []) (ToolsConfig Nothing)` | `tsOrigin == FromPath`、`tsPath == Just R`,而 `D </> "RAN"` **不存在**;呼叫前後對 `T` 遞迴列出的檔案清單與內容完全相同 | LAW-11 |
| EX-13 | `detectSevenZipIn (plan [] ["relative/7z.exe"]) (ToolsConfig (Just "sub dir/7z.exe"))` | `tsSearched == ["sub dir/7z.exe", "relative/7z.exe"]`——**逐字**,正斜線沒有變成反斜線、沒有被絕對化 | LAW-8 |
| EX-14 | `detectSevenZipIn (plan [] ["C:\\Program Files\\7-Zip\\7z.exe"]) (ToolsConfig Nothing)` | `tsSearched` 的那一項逐字等於 `"C:\\Program Files\\7-Zip\\7z.exe"`,**不論平台** | LAW-8 |
| EX-15 | `cfg = ToolsConfig Nothing`;取 `dirs = maybe [] splitSearchPath` 對 `lookupEnv "PATH"` 的結果、`cands` = 「數據」段的七條;比較 `detectSevenZip cfg` 與 `detectSevenZipIn (ToolSearchPlan dirs cands) cfg` | 兩者**逐欄相同**(不需要改動任何環境變數) | LAW-14 |
| EX-16 | `E = T` 下的合格檔;`detectSevenZip (ToolsConfig (Just E))` | `ToolStatus "7-Zip" (Just E) FromToolsConfig [E]`——真實入口在覆寫合格時同樣不看 `PATH` 與候選清單 | LAW-2, LAW-14 |
| EX-17 | 讀 `workspace/src/Aapms/Workspace/Tools.hs` 的 import 行(**先去除行尾 `\r`**) | 滿足 LAW-15 的 (a)–(f) 六條 | LAW-15 |
...
- **新增的依賴邊**(一條都不能漏):
  ...
  - **不新增** `Aapms.Workspace.Tools → Aapms.Workspace.Hub`:**本模組不 import `Hub`,
    ...
    履行它的是呼叫端(`service`),不是本模組的 import 行。**2026-08-29 WAVE-4 閘門裁決:該列
    不改**(要改是 `service` 設計時的事;現在動會讓 WAVE-4 的三份 spec 對不上同一張表)。
    LAW-15(a) 仍放行這條邊,只是骨架不建立它。
...
- [ ] STEP-1: 私有判準 `qualifies :: FilePath -> IO Bool` = `doesFileExist` 為真時再問
  `executable <$> getPermissions`,否則直接 `False`(不呼叫 `getPermissions`,避免它對不存在的
  路徑拋例外) `dep: -`
- [ ] STEP-2: 私有常數:內建候選清單(七條,逐字)與 PATH 查詢名稱 `["7z", "7zz"]`;兩者都**不匯出**
  `dep: -`
- [ ] STEP-3: 三層候選路徑的組出與**保序去重**:`tier1 ++ tier2 ++ tier3`,`tier2` 名稱外層、目錄內層、副檔名
  只用 `exeExtension` `dep: T2`
- [ ] STEP-4: `detectSevenZipIn`:依序問 `qualifies`,命中即停;算出 `tsPath` / `tsOrigin`(依命中項
  所屬的層)/ `tsSearched`(前綴)/ `tsName`;全不中回 `NotFound` + 完整清單 `dep: T1, T3`
- [ ] STEP-5: `detectSevenZip`:`lookupEnv "PATH"` → `maybe [] splitSearchPath` → 組 `ToolSearchPlan`
...
| Todo | Law / Example | 測試 |
|------|---------------|------|
| STEP-1 | LAW-7 / EX-8, EX-9 | `test_qualifies_rejects_directory`、`test_qualifies_rejects_non_executable_file`、`test_qualifies_missing_path_does_not_throw` |
| STEP-2 | LAW-10(名稱與副檔名的部分)/ EX-14 | `test_candidate_list_is_verbatim_and_platform_independent`(斷言「數據」段那七條逐字進 `tsSearched`) |
| STEP-3 | LAW-9, LAW-10 / EX-5, EX-6, EX-10, EX-13 | `test_probe_order_names_outer_dirs_inner`、`test_probe_order_layers`、`test_searched_is_deduped_across_layers`、`test_paths_are_verbatim` |
| STEP-4 | LAW-1, LAW-2, LAW-3, LAW-4, LAW-5, LAW-6, LAW-8, LAW-11, LAW-12, LAW-13 / EX-1–EX-4, EX-7, EX-11, EX-12 | `test_config_hit_short_circuits`、`test_config_hit_ignores_plan`、`test_config_miss_continues`、`test_path_layer_hit`、`test_not_found_is_not_a_failure`、`test_missing_dirs_do_not_throw`、`test_never_executes_the_tool`、`test_path_and_origin_are_consistent` |
| STEP-5 | LAW-14 / EX-15, EX-16 | `test_real_entry_matches_injected_plan`、`test_real_entry_config_override` |
| (全部) | LAW-15 (a)–(f) | `test_tools_no_sibling_module_imports`(a)、`test_tools_never_imports_process`(b)、`test_tools_never_imports_store_or_core`(c)、`test_tools_directory_import_is_read_only`(d)、`test_tools_environment_import_is_lookup_only`(e)、`test_tools_filepath_import_is_whitelisted`(f)。**六條都只掃 import 行,比對前先去除行尾 `\r`** |

## 待確認假設

> **2026-08-29 WAVE-4 閘門:五條全部裁完。** ASM-1 裁決為選項 a(納入契約 E);ASM-2–ASM-5 由編排者**降級**
> ——不上閘門、暫採即定案。每條的「裁決」欄記在該條末尾,**選項與代價的原文一律保留**,供事後
> 抽查與日後重議時對照。

- ASM-1: 在契約 E 的 `detectSevenZip` **之外**新增一個注入接縫
  ...
  - 選項:
    a) **新增 `ToolSearchPlan` + `detectSevenZipIn`(本 spec 採用)**——當下成本:模組匯出多兩個
       ...
       (`service` 若直接呼叫 `detectSevenZipIn` 自帶清單,清單的唯一真相就分岔了)——LAW-14 把
  ...
  - 暫採:a(`detectSevenZip` 契約原簽名 + `ToolSearchPlan (..)` + `detectSevenZipIn`)→ 影響:
    若裁決成 b,骨架刪掉 `ToolSearchPlan` 與 `detectSevenZipIn`(匯出清單只剩一個),LAW-1–LAW-12 的
    主詞全部從 `detectSevenZipIn plan cfg` 改寫成 `detectSevenZip cfg`、LAW-14 整條刪掉,
    EX-1–EX-14 全部要改成「以真實環境呼叫」的形式,而 EX-7(`NotFound`)與 EX-6(`FromCandidate`)
    **必須整條刪掉並在 spec 明寫「驗收標準第 3 條無法機械驗證」**;若裁決成 c,再加一條
    「測試必須序列執行且復原 `PATH`」的限制,EX-7 仍然要刪
  - **裁決(2026-08-29 WAVE-4 閘門):選 a,注入接縫進契約。** `design.md` 契約 E 已增列
    `ToolSearchPlan` 與 `detectSevenZipIn`,`detectSevenZip` 的原簽名一個字不動、變成薄包裝。
    採納理由即本條的傾向:這台機器上 `C:\Program Files\7-Zip\7z.exe` 實際存在、候選清單又是
    寫死常數,驗收標準第 3 條(`NotFound`)與 `FromCandidate` 這個 `ToolOrigin` 值**永遠觸發
    不到**——那條驗收標準會變成沒人驗得了的話。
    **這不是測試後門**:它與 graph-core `allocateId` 的 2026-08-25 **GAP-8 裁決**是同一個模式
    (把隱含參數變明碼、原函式變薄包裝),同一波的 **F005 也依同一個判準把 `allocateProjectId`
    提進契約**。三者的共同形狀是:原簽名一字不動、新入口只描述**環境事實的來源**、兩個入口的
    一致性由一條 law 釘住(本 feature 是 LAW-14)。
...
- ASM-2: 「存在且可執行」的判準定成
  ...
  - 暫採:a → 影響:若裁決成 b,LAW-7 的 (b)(c) 兩款刪掉、EX-8 / EX-9 的預期從「不命中」改成「命中」;
    若裁決成 c,LAW-7 要改寫成兩段平台條文(違反本波「不要寫出只在某一個平台成立的 law」),
    而且 `Win32` 依賴要先過阻塞項
  - **裁決(2026-08-29 WAVE-4 閘門):編排者降級,不上閘門,暫採 a 即定案。** 理由記在 `build-log.md`:
...
- ASM-3: `PATH` 那一層**自己走 `PATH` 環境變數**(`lookupEnv "PATH"` → `splitSearchPath` →
  ...
  **只試 `exeExtension` 一種副檔名,不展開 `PATHEXT`**。design.md「使用的技術」那一段**原本**點名
  ...
  (**2026-08-29 WAVE-4 閘門已依本條的理由改掉 design.md 那一行**,現在寫的是「路徑正規化
  ...
  - 暫採:a → 影響:若裁決成 b,LAW-10 整條改寫(第二層不再有展開式,`tsSearched` 在這一層要嘛
    空、要嘛只有兩個查詢名)、LAW-6(b) 改成「`tsPath` 等於 `findExecutable` 對 `"7z"` 或 `"7zz"`
    的回傳」、EX-4 / EX-5 / EX-15 全部要改成不可控環境的版本或刪掉;若裁決成 c,LAW-10 的展開式多一層
    副檔名迴圈,EX-4 / EX-5 / EX-6 的 `tsSearched` 期望值跟著變長
  - **裁決(2026-08-29 WAVE-4 閘門):編排者降級,不上閘門,暫採 a 即定案。** 理由記在 `build-log.md`:
...
- ASM-4: `tsSearched` 記錄的**粒度**是「每一個**被判準問過的完整檔案路徑**」(而不是 `PATH` 的
  ...
  - 傾向:a。理由是它讓 `tsSearched` 的每一項有**同一種**意義(「一個被 `doesFileExist` /
    ...
    簽名,也不會讓任何既有中樞檔或指令失效
  - 暫採:a → 影響:若裁決成 b,LAW-1(b)(c)、LAW-6(b)、LAW-9 要改寫(`tsPath` 不再是 `tsSearched` 的
    最後一項),EX-3–EX-7、EX-11、EX-12 的 `tsSearched` 期望值全部改成目錄形式;若裁決成 c,再加上
    LAW-8 對第二層那一款整條刪除(裸檔名沒有「逐字對應來源」可言)
  - **裁決(2026-08-29 WAVE-4 閘門):編排者降級,不上閘門,暫採 a 即定案。** 理由記在 `build-log.md`:
    粒度由 ASM-3 的**實測結果**連帶決定(既然第二層是自己展開 `PATH`,被問過的就是完整檔案路徑),
    而 b / c 都會讓契約卡驗收標準第 1、2 條(「`tsSearched` 只有那一個」「該路徑仍出現在
    `tsSearched`」)需要額外解釋才對得上——**寫不出真實的第二方案**。
    → spec、laws、examples **一字不改**。

- ASM-5: `tsPath` 與 `tsSearched` 的每一項一律**逐字**捧著,**不做任何正規化**
  ...
  - 暫採:a → 影響:若裁決成 b 或 c,LAW-8 整條改寫成「`tsPath` 是 `<該正規化函式>` 施用於來源
    路徑的結果」,EX-13 / EX-14 的預期改成正規化後的形式,EX-1 / EX-3 / EX-16 的斷言從「等於 fixture 路徑」
    改成「等於 fixture 路徑的正規化」;另外要在本 spec 明寫「驗收標準第 1 條的『等於它』理解為
    『等於它的正規化』」。若編排者同時想補上 `loadHub` 對 `[tools]` 路徑的合規檢查,那是 F001
    的修訂,不在本 feature
  - **裁決(2026-08-29 WAVE-4 閘門):編排者降級,不上閘門,暫採 a 即定案。** 理由記在 `build-log.md`:
    本條**沿用 WAVE-1 的 `loadHub` 合規判準**——`[tools].seven_zip` 已被要求必須是絕對路徑
    ...
    (殘留的「`PATH` 裡有相對目錄」不在中樞管得到的範圍,由 LAW-8 如實記錄,不另設檢查。)
