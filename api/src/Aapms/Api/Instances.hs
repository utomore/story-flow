...
--
-- 與 "Aapms.Core.Json" 同一個理由的孤兒實例:capture 段、query parameter
...
-- 不會有人發現。
module Aapms.Api.Instances () where
...
import Aapms.Core.Id (Id, Ref, parseId, parseRef, renderId, renderRef)
import Aapms.Core.Level (Level, Node, NodeKind, allNodeKinds, parseNodeKind, renderNodeKind)
import Aapms.Core.Link (Link, LinkKind, coreLinkKinds, parseLinkKind, renderLinkKind)
...
import Aapms.Workshop (Session, StageDraft)
...
-- | 純量型別的 schema。"Aapms.Core.Json" 的第一條約定是這些型別編成
...
-- ("Aapms.Service.Json" 的第三條約定:Agent 讀得懂鍵名,讀不懂位置)。
data SourcedLink
...
-- | __五欄全部選配__:'Aapms.Conflict.Json' 的 @FromJSON ConflictOpts@ 逐欄退回
...
-- (@layer@ + 這三欄),沒有任何 @$ref@ 指向它,而 @aapmsOpenApi@ 只收集
...
--
-- 這也是 @Aapms.Api.SchemaSpec@ 對它用 @alignsSubset@(子集)而不是既有
...
-- | 訊息的角色。'Aapms.Workshop.Session' 的 @messageJson@\/@roleWire@ 把它編
...
-- ("Aapms.Llm.Client" 刻意不定義它——門面之外的內部細節),wire 形狀由
-- 'Aapms.Workshop.Session' 的 @ToJSON Session@ 手動編碼;這裡的 schema
...
-- required 裡(與 'Aapms.Workshop.Session' 的 @ToJSON StageDraft@ 同一個
-- @Maybe@ 約定)。
...
-- | 一次工作坊。九個欄位逐字對齊 'Aapms.Workshop.Session' 的 @ToJSON
