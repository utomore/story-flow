...
module Aapms.Api.OpenApiSpec (spec) where
...
import Aapms.Api (deriveOperationId, aapmsOpenApi)
import Test.Hspec
...
spec = describe "OpenAPI 文件" $ do
  ...
  -- llm-workshop-mcp/F005:aapms-mcp 的 tool 名字直接讀 operationId 這個欄位
  ...
  it "info 有 title、version 與 description" $ do
    doc ^. info . title `shouldBe` "aapms API"
