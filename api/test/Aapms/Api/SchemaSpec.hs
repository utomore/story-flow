...
-- ("Aapms.Api.Fixtures"),否則 @Maybe@ 欄位「沒值就整個鍵不出現」的約定會讓
-- 兩邊看似不一致。
module Aapms.Api.SchemaSpec (spec) where
...
import Aapms.Core.Meta (Meta, Timeline)
...
spec = describe "ToSchema 與 ToJSON 逐欄對齊" $ do
  ...
  describe "工作坊(llm-workshop-mcp/F004)" $ do
    ...
    -- Message 沒有 Data.Aeson.ToJSON 實例("Aapms.Llm.Client" 刻意不定義它),
    -- 所以不能像其餘型別一樣直接 aligns。wire 形狀只透過 Session 的 ToJSON
    -- 手動編碼現身(Aapms.Workshop.Session 的 messageJson),這裡改為拿
    ...
    -- schema 的 enum_ 逐字等於 Aapms.Workshop.Session 的 roleWire 三個值。
