...
-- "Aapms.Cli.Backend" 的那組操作,而渲染器只有一份。service-and-interfaces/F003 驗收標準 4
...
import Aapms.Api (WorkshopStepResp (wssReply))
import Aapms.Cli.Backend
import Aapms.Cli.Doctor
import Aapms.Cli.Error
import Aapms.Cli.Options
import Aapms.Cli.Render
import Aapms.Cli.Resolve
import Aapms.Conflict.Json ()
import Aapms.Conflict.Types (Draft (..))
import Aapms.Core.Id (Id, renderId, renderRef)
import Aapms.Core.Link (Link (..), LinkKind, isCoreKind, renderLinkKind, suggestCoreKind)
...
runCliWith io args = do
  ...
  case parseCli args of
    ...
    CompletionInvoked cr -> do
      msg <- execCompletion cr "aapms"
      TIO.hPutStr (cliOut io) (T.pack msg)
      pure ExitSuccess
    Failure f -> case renderFailure f "aapms" of
...
handle io b = \case
  ...
  -- Aapms.Cli.Options 的 F006 註解)。exit code 恆為 0:這是一份報告,不是
  -- 一個判定,命中是不是真的衝突由作者決定。
...
-- | 非核心關聯的提示。
--
-- __提示不是阻擋__:'Aapms.Core.Link.parseLinkKind' 是全函式,自訂關聯一律
...
-- (開發機是 cp950),於是 @aapms --json entity show 琳達 | jq@ 拿到的
