...
import Aapms.Conflict.Types (ConflictOpts, ConflictReport, ContextHit, Draft)
import Aapms.Cli.Options (GlobalOpts (..))
...
-- | token 走 header,不進 API 型別(見 @aapms-server@ 的
-- "Aapms.Server.Auth")。
...
-- 'Aapms.Service.errorCode' \/ 'Aapms.Service.renderServiceError' 產生的,
...
( cListVaults
  ...
  :<|> (cTypes :<|> cSearch)
  :<|> (cContext :<|> cCheck)
  :<|> (cWorkshopStart :<|> cWorkshopStep :<|> cWorkshopCommit) = client (Proxy :: Proxy AapmsAPI)
...
-- 'Aapms.Service.ServiceM' 裡呼叫 @linkGraph@。
...
-- 看不出差別。
--
-- __第 3 層要用的 'Aapms.Llm.LlmClient' 不跨 HTTP__:遠端模式送出去的是
...
-- 比照 @conflict\/src\/Aapms\/Conflict\/Pipeline.hs@ 的 @acquireJudge@,但更
