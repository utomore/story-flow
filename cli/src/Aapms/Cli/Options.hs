...
-- 'Command','Aapms.Cli' 再拿它去呼叫 @service@。這條界線讓「引數怎麼寫」
-- 能被單元測試釘死,不必開 Vault。
--
...
-- 的說明文字是在有 'Aapms.Service.Monad.Env' 之前就要組出來的,那時拿不到
-- 註冊表。
module Aapms.Cli.Options
  ( -- * 型別
    GlobalOpts (..)
  , Command (..)
  , Selector (..)
...
import Paths_aapms_cli (version)
import Aapms.Conflict.Types (ConflictOpts (..), defaultConflictOpts)
import Aapms.Core.Id (Id, Ref, parseId, parseRef)
import Aapms.Core.Level (NodeKind, allNodeKinds, parseNodeKind, renderNodeKind)
...
data GlobalOpts = GlobalOpts
  { goVault :: Maybe Text
  -- ^ 原樣傳給 'Aapms.Service.Monad.openEnv';ADR-008 的解析規則不在 CLI 重寫
  , goJson :: Bool
  , goRemote :: Maybe Text
...
-- | @--version@ 印的那一行。格式與 @aapms-serve@ / @aapms-mcp@ 相同:
-- 執行檔名、一個空白、版本。版本來自 cabal 的 @version@ 欄,程式碼裡不寫第二份。
cliVersion :: String
cliVersion = "aapms " <> showVersion version
...
-- @aapms conflict context@ ——它是給外部 Agent 用的日常入口,而 @conflict@
...
-- 規則,連 UTF-8 強制解碼與「讀不到檔」的錯誤訊息都沿用 'Aapms.Cli' 的
-- @readBody@。
...
-- @aapms context@ 這條路的第 1 層永遠不會有輸出(見 F004 待確認假設 A1)。
refOpt :: Parser Id
refOpt =
  option
    (eitherReader (readWith (fmap snd . parseId) "id 的格式應為 <prefix>-<十六進位>,如 ent-7f3a"))
...
typeOpt =
  txtOption
    ( long "type"
        <> metavar "<型別>"
        <> help "型別註冊表裡的鍵;以 aapms type list 查看可用型別"
    )
...
-- (ADR-005)。CLI 能做的只有事後提示,那在 'Aapms.Cli' 裡。
