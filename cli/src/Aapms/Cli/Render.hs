...
-- 'Aapms.Core.Tree.buildTree' 已經驗證過合法性,再重建一次就是第二份樹邏輯。
...
renderVaults [] = "(全域註冊表裡還沒有 Vault;用 aapms vault init 建一個)"
...
--
-- 標籤字串走 'Aapms.Conflict.Types.layerTag',__不另寫一份__ ——@--json@ 的
...
-- 真的衝突由作者決定("Aapms.Cli" 的 @handle@ 負責這件事,這裡只管渲染)。
