...
renderResolveError = \case
  ...
  where
    ...
    extraHint SubNode = "\n(節點 id 可以用 story-flow level show <Level> 取得)"
    extraHint _ = ""

listCommand :: Subject -> Text
listCommand = \case
  SubEntity -> "story-flow entity list"
  SubLevel -> "story-flow level list"
  SubNode -> "story-flow level show <Level>"
...
-- 'StoryFlow.Service.errorCode' 產生,與內嵌模式是同一個函式,所以
...
renderRemoteError = \case
  RemoteUnavailable d ->
    "連不上遠端伺服器:" <> d <> "\n請確認 story-flow-serve 正在跑,而且 --remote 的網址正確"
  RemoteBadResponse d ->
    "遠端伺服器的回應看不懂:" <> d <> "\n這通常代表 --remote 指到的不是 story-flow 的伺服器"
