...
-- "Aapms.Cli.EntityWriteSpec" ——'Aapms.Store.StoreError' 的建構子沒有被
-- service 的門面重新匯出,而 CLI 依定義碰不到 @aapms-store@(T1)。
module Aapms.Cli.EnvelopeSpec (spec) where

import Data.Aeson (Value, decodeStrict, object, toJSON, (.=))
import Data.Text (Text)
import qualified Data.Text.Encoding as TE
import Aapms.Cli.Fixtures (shouldContainT)
