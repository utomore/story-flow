...
-- 使用者拿到的全部,這裡不准有驚喜。
module Aapms.Cli.ReleaseScriptSpec (spec) where
...
spec = describe "scripts/release(組裝段)" $
  it "產出目錄恰好是 3 個執行檔 + registry/ 5 份 TOML + README.md,且 zip 存在" $ do
    ...
    case mInterp of
      ...
      Just (prog, mkArgs) ->
        withSystemTempDirectory "aapms-stage" $ \stage -> do
          -- 三個假執行檔:組裝段只 copy,不執行它們
          mapM_ (\b -> writeFile (stage </> b) "fake") ["aapms.exe", "aapms-serve.exe", "aapms-mcp.exe"]
          ...
          case (dirs, zips) of
            ([d], [z]) -> do
              takeFileName d `shouldSatisfy` ("aapms-9.9.9-" `isPrefixOf`)
              z `shouldBe` d <> ".zip"
