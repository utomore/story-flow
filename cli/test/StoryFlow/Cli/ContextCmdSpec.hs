-- | conflict-detection/F004 T12 \/ T13 \/ T14:@story-flow context@。
--
-- 三件事要驗:兩條後端路徑都跑得通、兩種輸出格式都對、__兩條路徑回同一批結果__。
--
-- 最後那一條(T14)是契約卡的驗收標準 4。它在結構上已經成立
-- ——'StoryFlow.Cli.Backend.gatherContextB' 兩條分支回的是同一個型別,渲染器只有
-- 一份——但仍然有一條逐字元比對守著,作法與 "StoryFlow.Cli.ParitySpec" 相同。
module StoryFlow.Cli.ContextCmdSpec (spec) where
