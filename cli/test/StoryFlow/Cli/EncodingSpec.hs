...
--
-- 2026-08-22 實測:@story-flow vault init … > out.txt@ 在 Windows 得到 CP950 位元組
-- ——任何 UTF-8 工具讀它都是亂碼。@--json@ 那條路徑是對的('jsonLine' 自己編 UTF-8),
-- 壞的只有人類模式。
--
-- __為什麼 1435 條測試抓不到__:'StoryFlow.Cli.Fixtures.withTempHandle' 對每個測試
...
module StoryFlow.Cli.EncodingSpec (spec) where

import qualified Data.ByteString as BS
import qualified Data.Text as T
import qualified Data.Text.Encoding as TE
import StoryFlow.Cli (CliIO (..), runCliWith)
...
spec = describe "人類模式的輸出編碼(B002)" $
  it "導向非終端機的 handle 時是 UTF-8,不是 locale 的 code page" $
    withSystemTempFile "storyflow-raw-out" $ \_ hOut ->
      withSystemTempFile "storyflow-raw-err" $ \_ hErr ->
        withSystemTempFile "storyflow-raw-in" $ \_ hIn -> do
