...
withTempHandle act = withSystemTempFile "storyflow-cli-io" $ \_ h -> do
  hSetEncoding h utf8
...
withCliVault act =
  withSystemTempDirectory "storyflow-cli" $ \dir -> do
    reg <- registryDir
    withEnvVars
...
-- | 臨時 Vault + 一台跑在隨機埠上的 story-flow 伺服器。
--
