-- | T12:@--remote@ 解析與併用檢查。
module StoryFlow.Cli.RemoteOptSpec (spec) where
...
spec = describe "--remote" $ do
  ...
  describe "連不上" $ do
    ...
    it "訊息說得出下一步" $ do
      r <- sfRemote "http://127.0.0.1:1" ["entity", "list"]
      crErr r `shouldContainT` "story-flow-serve"
  ...
  describe "token" $
    ...
    -- remote_bad_response(「對面不是 story-flow 伺服器」),那個訊息會把人帶偏。
    it "token 錯時說的是認證失敗,不是「對面不是 story-flow 伺服器」" $
