...
import StoryFlow.Cli.Options (Selector (..), mkSelector)
import StoryFlow.Cli.Resolve
import StoryFlow.Core.Id (Id, parseId, renderId)
import StoryFlow.Core.Level (NodeKind (KScene))
...
spec = describe "Selector → Id(內嵌後端)" $ do
  ...
  it "不存在的標題 → title_not_found,訊息附上該查哪一條指令" $ do
    ...
    case r of
      Left e@(CliResolve (NotFound SubEntity "沒這個人")) -> do
        ...
        cliErrorMessage e `shouldContainT` "story-flow entity list"
  ...
  it "Level 的定址走 listLevels,錯誤訊息指向 level list" $ do
    ...
    case r of
      Left e -> cliErrorMessage e `shouldContainT` "story-flow level list"
  ...
  it "節點多筆命中時提示去 level show 抄 id" $ do
    ...
    case r of
      Left e@(CliResolve (Ambiguous SubNode _ ms)) -> do
        length ms `shouldBe` 2
        cliErrorMessage e `shouldContainT` "story-flow level show"
      other -> expectationFailure ("預期 Ambiguous,實際 " <> show other)
