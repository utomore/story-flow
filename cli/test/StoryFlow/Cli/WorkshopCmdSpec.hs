...
--
-- 仿 "StoryFlow.Cli.ContextCmdSpec" 的形狀:一個真的跑在本機的 OpenAI 相容 stub
-- 端點(@step@ 需要它),把臨時 Vault 的 @.storyflow\/config.toml@ 指過去
-- ——與 @llm\/test\/StoryFlow\/Llm\/Fixtures.hs@ 的 @appendConfig@ 同一份寫法。
module StoryFlow.Cli.WorkshopCmdSpec (spec) where
