...
import Aapms.Conflict.Retrieval (Candidate (..), metaSnippet)
...
import Aapms.Llm (LlmClient, LlmError (..), Message (..), Role (..), chat, renderLlmError)
import Aapms.Service (EntityView (..), ServiceM, getEntity)
...
-- @FromJSON@ 私有地留在本模組,不進 "Aapms.Conflict.Json"(先例:
-- @aapms-llm@ 的 @ChatResponse@ \/ @ChatChoice@)。
...
-- 'Aapms.Llm.renderLlmError' 的原文,__不重寫下層訊息__。
...
data JudgeResult = JudgeResult
  ...
  , jrNotes :: [ReportNote]
  -- ^ 逐對失敗、中止、預算為零 → 'Aapms.Conflict.Types.ConflictReport'
  -- 的 @crNotes@
  }
  deriving stock (Show, Eq)
