...
--
-- 出口 A(F004)__對外真正交付__把第 1 層("Aapms.Conflict.Graph")與第 2 層
-- ("Aapms.Conflict.Retrieval")接上 @service@,合流成 'gatherContext',
-- 再由 CLI 的 @aapms context@ 與 REST 的 @POST \/conflict\/context@ 兩種
-- 形式露出去。ADR-007 那條與三層同等重要的需求——__外部 Agent 常常只需要精準的
-- context,不需要 aapms 代它判斷__——就是靠這個出口滿足的:回的是「和這段
-- 草稿有關的既有片段,連內容一起給你」,不是一份誰對誰錯的報告。
--
-- 出口 B(F006)把第 3 層("Aapms.Conflict.Judge")也接上來,合流成
-- 'checkConflict',再由 @aapms conflict check@ 與 @POST \/conflict\/check@
-- 兩種形式露出去:回的是「和這段草稿矛盾的是哪幾筆、憑什麼」——一份報告。
--
-- 三個性質是規格而不只是實作:
--
-- * __完全沒有模型__是 'gatherContext' 這條路徑的性質,不是整個模組的:它不
--   import @aapms-llm@,不發任何外部請求。'checkConflict' 這條路徑則會在
--   第 3 層真的要跑時才建立 'Aapms.Llm.LlmClient' 並發出請求
...
--        → ConflictReport                                          -- 出口 B
-- @
module Aapms.Conflict.Pipeline
  ( -- * 出口(Level 2 對外契約)
    gatherContext
  , checkConflict
...
import Aapms.Conflict.Graph (graphHits, unlinkedRefs)
import Aapms.Conflict.Judge
  ( JudgeResult (..)
  , JudgeRunner
  , JudgeSkip (..)
  , judgeCandidatesWith
...
import Aapms.Conflict.Retrieval
  ( Candidate
  , RetrievalResult (..)
...
import Aapms.Core.Link (renderLinkKind)
import Aapms.Core.Meta (Meta (..))
-- 走門面 'Aapms.Llm' 而不是內部模組:門面的匯出清單就是設計文檔
-- llm-workshop-mcp/F001 列的名字,一個不多一個不少,繞過它等於讓本模組依賴
-- @aapms-llm@ 內部怎麼切模組(閘門裁定 B-1)。
import Aapms.Llm (LlmError (..), llmConfig, newLlmClient)
import Aapms.Service (EntityView (..), ServiceM, getEntity, linkGraph)
...
-- * 兩者皆空 → 回 @[]@
--
-- __'Aapms.Conflict.Types.coTopN' 不在這裡再截一次__:它是__第 2 層的候選
...
-- 完整三元組('Aapms.Conflict.Types.GraphEvidence'),JSON 出去就是
-- @{"layer":"graph","from":…,"kind":…,"to":…}@。
-- 'Aapms.Conflict.Graph.renderGraphReason' 產出的那句繁中本來就只是這個三元組的
...
--   'Aapms.Conflict.Types.sortHits' 的約定同一個方向——第 1 層是__事實__,
--   排前面
  ...
  Just $
    ReportNote
      ...
      ( "你引用的 "
          ...
          <> " 在本 Vault 沒有任何關聯,第 1 層對它們幫不上忙;用 `aapms link list` 確認,或補上關聯"
      )
      ...
      Just $
        ReportNote
          ...
          ( "第 3 層判定 "
              ...
              <> "(aapms link add <草稿片段 id> --kind contradicts --target "
...
--    下一步
-- 2. 'Aapms.Llm.llmConfig' 回 'Left' → 'SkipNotConfigured'
...
-- 結果餵進來;本函式因此完全不知道 'Aapms.Llm.LlmClient' 存在,也不會自己
