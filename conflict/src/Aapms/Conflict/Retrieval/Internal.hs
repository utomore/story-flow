-- | 衝突偵測第 2 層的__實作__。
--
-- __本模組不是契約__。它匯出全部部件——含調校常數與純函式部件——供__同套件的測試__
-- 觀測;`conflict/src/` 底下除了 "Aapms.Conflict.Retrieval" 之外
-- __不准有任何檔案 import 它__(由 @CabalSpec@ 釘住,見 conflict-detection/E001)。
--
-- 要用第 2 層請走門面 "Aapms.Conflict.Retrieval",那裡只有 10 個名字,
-- 而那 10 個就是 @design.md@ 認得的全部。
--
-- __為什麼要拆這一層__:契約卡寫著「候選撈取策略本身是本模組的內部抽象,對外只露
-- 『候選』這個結果」,而 ADR-007 把 embedding 語意檢索列為__被推遲而非被否決__的選項。
-- 那個承諾要兌現,前提是第 1、3 層從來沒碰過第 2 層的內部——而「碰了」這件事
-- __不會有任何測試變紅__,所以只能靠可見度本身把門關上。
--
-- 以下是這一層本來就有的說明。
--
...
--   → 排序 → take coTopN
-- @
module Aapms.Conflict.Retrieval.Internal
  ( -- * 門面
    retrieveCandidates
...
import Aapms.Core.Link (Link (..), LinkKind (OccursIn, PartOf), renderLinkKind)
import Aapms.Core.Meta (Meta (..), Status (Canon), Timeline (..), isCanon)
...
-- 斷詞問題影響。順序沿用 'Aapms.Service.aliasIndex'(id 字典序)以保證
-- 確定性。
--
