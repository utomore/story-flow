...
data ConflictOpts = ConflictOpts
  ...
  -- ^ 比對 'Aapms.Core.Meta.tlOrder' 的容許距離;@Nothing@ = 不做時序過濾。
  --
  -- 只有 @tlOrder@ 有值的片段受它影響:@tlLabel@ 是模糊字串(「崩塌前後」),
  -- 無從算距離。
  , coGraphDepth :: Int
  -- ^ 第 1 層的遍歷深度,對應 "Aapms.Core.Graph" 的 @follow@ 的 @Int@ 參數。
  }
  deriving stock (Show, Eq)
...
-- | 撈出來的素材:這一筆和草稿有關。
--
-- 直接帶 'Meta' 而不是只帶 id:@aapms context@ 的使用者(通常是 claude code)
