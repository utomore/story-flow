...
--
-- 孤兒實例是刻意的,理由與 "StoryFlow.Core.Json" / @StoryFlow.Service.Json@
-- 完全相同:CLI 的 @--json@、REST 的 body、未來 MCP 用的是同一套編碼規則,
-- 規則只該有一份;把實例散在型別模組會讓「規則有一份」變成靠自律維持。
--
-- 編碼約定沿用 "StoryFlow.Core.Json" 的三條:
--
-- * 欄位名去掉 Haskell 的前綴,多字用 snake_case
-- * @Maybe@ 沒值時__整個鍵不出現__,不是 @null@
-- * 'StoryFlow.Core.Id.Id' 與 'StoryFlow.Core.Id.Ref' 是__字串__,不是物件
--
...
-- 標籤值與 'StoryFlow.Conflict.Types.layerTag' 共用同一份,不各自寫一次。
...
instance ToJSON ConflictReport where
  toJSON ConflictReport {..} =
    object ["hits" .= crHits, "scanned" .= crScanned, "llm_used" .= crLlmUsed]

instance FromJSON ConflictReport where
  parseJSON = withObject "ConflictReport" $ \o ->
    ConflictReport <$> o .:? "hits" .!= [] <*> o .:? "scanned" .!= 0 <*> o .:? "llm_used" .!= False
