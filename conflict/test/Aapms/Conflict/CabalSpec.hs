-- | T1:套件邊界。
--
-- 這份 spec 的驗收標準 4 原本說「@build-depends@ 只有 @aapms-core@,沒有
-- @aapms-service@ / @aapms-store@ / @aapms-llm@」。那句話唯一守得住
-- 的形式是 @.cabal@ 檔裡沒有那些名字,所以用測試釘住,不是靠 code review 記得。
--
-- __conflict-detection/F003 T4:@aapms-service@ 這一項被明確改掉了__ ——
-- 這正是 F001 註解預告的那一刻:
--
-- > @aapms-conflict@ 後續會依賴 @service@(第 2 層要用它的 @searchEntity@),
...
--
-- __conflict-detection/F005 T4:@aapms-llm@ 這一項也被明確改掉了__——第 3 層
-- (@Conflict.Judge@)消費 @llm-workshop-mcp@ 的 @LlmClient@ / @chat@,存在前提是
-- @aapms-llm@ 已經到位(llm-workshop-mcp/F001 已 done)。forbidden 清單因此從
-- 四項縮成三項:@aapms-store@ / @aapms-md@ / @sqlite-simple@。
--
-- 放行的同時把剩下三項守得__更緊__:斷言從單向的「不含」改成雙向的
-- 「必須含 service、llm,仍然不含其餘三項」,而逐字釘住的相依清單則保證沒有第七個
-- 名字趁這一次順道混進來。
module Aapms.Conflict.CabalSpec (spec) where
...
spec = describe "第 3 層放行 llm,其餘三項仍然擋住" $ do
  it "build-depends 含 aapms-service(第 2 層的候選來源)" $ do
    deps <- dependencyLines <$> readCabal
    any ("aapms-service" `isInfixOf`) deps `shouldBe` True

  -- conflict-detection/F005 T4:library 與 test-suite 各一次(假 runner 的型別
  -- LlmClient / Message / Role / LlmError 都從 aapms-llm 來)。
  it "build-depends 含 aapms-llm(library 與 test-suite 各一次)" $ do
    src <- readCabal
    let deps = dependencyLines src
    length (filter ("aapms-llm" `isInfixOf`) deps) `shouldBe` 2
  ...
  it "build-depends 含 aapms-core" $ do
    deps <- dependencyLines <$> readCabal
    any ("aapms-core" `isInfixOf`) deps `shouldBe` True

  -- conflict-detection/F002 T7:第 1 層是純函式,長出一個新模組不該把
  -- service / store 拖進來。新模組要在 exposed-modules 裡,相依維持原樣。
  it "新增模組未帶進新相依" $ do
    src <- readCabal
    ("Aapms.Conflict.Graph" `isInfixOf` src) `shouldBe` True
    let deps = dependencyLines src
    mapM_ (\p -> (p, any (p `isInfixOf`) deps) `shouldBe` (p, False)) forbidden
  ...
  it "exposed-modules 含 Aapms.Conflict.Retrieval" $ do
    src <- readCabal
    ("Aapms.Conflict.Retrieval" `isInfixOf` src) `shouldBe` True

  -- conflict-detection/E001 T5:門面與實作拆開之後,Internal 也要註冊得到,
  -- 否則測試 import 不到它。
  it "exposed-modules 含 Aapms.Conflict.Retrieval.Internal(E001)" $ do
    src <- readCabal
    ("Aapms.Conflict.Retrieval.Internal" `isInfixOf` src) `shouldBe` True
  ...
  -- ——下一個 feature 隨手 @import Aapms.Conflict.Retrieval.Internal@ 就把閘門
  ...
  -- @aapms-store@ 的 @loadLinkGraph@ —— __相依清單卻一個字都不該變__:
  -- 那正是「整張圖只經 ServiceM 拿」這條界線的證明。相依若在這裡長出
  -- @aapms-store@,就代表 Pipeline 自己去讀索引了。
  it "exposed-modules 含 Aapms.Conflict.Pipeline,且相依清單一個字沒變" $ do
    src <- readCabal
    ("Aapms.Conflict.Pipeline" `isInfixOf` src) `shouldBe` True
    let deps = dependencyLines src
    map trim deps `shouldBe` libraryDeps ++ testDeps
    mapM_ (\p -> (p, any (p `isInfixOf`) deps) `shouldBe` (p, False)) forbidden

  -- conflict-detection/F005 T4:第 3 層的模組要註冊得到,否則 F006 import 不到它。
  it "exposed-modules 含 Aapms.Conflict.Judge,且相依清單逐字相符" $ do
    src <- readCabal
    ("Aapms.Conflict.Judge" `isInfixOf` src) `shouldBe` True
    let deps = dependencyLines src
    map trim deps `shouldBe` libraryDeps ++ testDeps
    mapM_ (\p -> (p, any (p `isInfixOf`) deps) `shouldBe` (p, False)) forbidden
...
-- @context@ 管線失敗。
--
-- @aapms-llm@ 是第 3 層(conflict-detection/F005)放行的新相依:
...
-- @aapms-service@ 的門面就夠了——@aapms-store@ 一次都不必露臉。
-- @aapms-llm@ 是第 3 層假 runner 的型別來源(@LlmClient@ / @Message@ /
...
-- @aapms-llm@ 自 conflict-detection/F005 起已放行(見上方 library 段落),
-- 不再是禁用名單的一員。
forbidden :: [String]
forbidden =
  [ "aapms-store"
...
internalName :: String
internalName = "Aapms.Conflict.Retrieval.Internal"
...
readCabal :: IO String
readCabal = go ["aapms-conflict.cabal", "conflict/aapms-conflict.cabal"]
  where
    go [] = fail "找不到 aapms-conflict.cabal"
    go (c : rest) = do
      ok <- doesFileExist c
      if ok
