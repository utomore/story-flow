...
--
-- 建臨時 Vault __只靠 @aapms-service@ 的門面__:與
-- "Aapms.Conflict.RetrievalEnvSpec" 同一條界線——@aapms-store@ 一次都
-- 不必露臉。__hermetic__(D6):全程用假 runner,不建立任何真正指向網路端點的
-- 用戶端,不發任何網路請求。
module Aapms.Conflict.JudgeEnvSpec (spec) where
...
import Aapms.Conflict.Fixtures (metaOf)
import Aapms.Conflict.Judge
import Aapms.Conflict.Retrieval (Candidate (..), CandidateOrigin (FromKeyword), metaSnippet)
import Aapms.Conflict.Types (ConflictOpts (..), Draft (..), ReportNote (..), defaultConflictOpts)
...
-- 名字,不必知道套件內部分了幾個模組。
import Aapms.Llm (LlmError, Message)
...
-- "Aapms.Conflict.RetrievalEnvSpec" 的 @withVault@ 是同一份寫法。
withVault :: (Env -> IO a) -> IO a
withVault act =
  withSystemTempDirectory "aapms-conflict-judge" $ \dir -> do
    reg <- registryDir
    withEnvVars
