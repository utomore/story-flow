...
-- 不建立任何真正指向網路端點的用戶端__。'Aapms.Conflict.Judge.judgeLoop'
-- 的型別參數是 @Monad m@ 而不是 @IO@,它因此碰不到真正的網路呼叫,只能用
-- runner 給它的東西。
module Aapms.Conflict.JudgeSpec (spec) where
...
import Aapms.Conflict.Fixtures (idOf)
import Aapms.Conflict.Judge
import Aapms.Conflict.Types (ConflictHit (..), Draft (..), HitLayer (..), ReportNote (..))
import Aapms.Core.Id (Id, renderId)
-- 走門面 'Aapms.Llm' 而不是內部模組(閘門裁定 B-1):消費者只 import 一個
-- 名字,不必知道套件內部分了幾個模組。
import Aapms.Llm (LlmError (..), Message (..), Role (..), renderLlmError)
...
-- | 以 UTF-8 讀,理由與 "Aapms.Conflict.CabalSpec" 的 @readCabal@ 相同:
...
-- | @aapms-llm@ 建立真正用戶端的函式名——__刻意拆成兩段字串再接起來__,
