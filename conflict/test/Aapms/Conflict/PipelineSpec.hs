...
-- @runService@),@aapms-store@ 一次都不必露臉——這正是子系統界線
-- 「所有讀取經 ServiceM」在測試端的證明。
--
-- __完全沒有模型__:這一整檔跑得完就是驗收標準 1(「在完全沒有模型的環境跑得完」)
-- 的證明,沒有任何 stub 或旗標。
module Aapms.Conflict.PipelineSpec (spec) where
...
import Aapms.Conflict.Retrieval (metaSnippet)
import Aapms.Conflict.Types
import Aapms.Core.Entity (Entity (..))
import Aapms.Core.Id (Id, localRef, parseId, renderId)
import Aapms.Core.Link (Link (..), LinkKind (Contradicts, Supersedes))
import Aapms.Core.Meta (Meta (..), Source (Human), emptyTimeline)
...
withVault act =
  withSystemTempDirectory "aapms-pipeline" $ \dir -> do
    reg <- registryDir
    withEnvVars
