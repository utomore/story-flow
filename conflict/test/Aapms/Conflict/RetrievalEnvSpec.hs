...
module Aapms.Conflict.RetrievalEnvSpec (spec) where
...
import Aapms.Core.Id (Id, Ref (..), localRef, parseId, renderId)
import Aapms.Core.Link (Link (..), LinkKind (Involves, OccursIn, PartOf, References))
-- @Status@ 的 @Draft@ 建構子與 'Aapms.Conflict.Types.Draft' 同名,
-- 所以 core 的 'Status' 走 qualified。
import Aapms.Core.Meta (Meta (..), Source (Human), Timeline (..), emptyTimeline)
...
withVault act =
  withSystemTempDirectory "aapms-conflict" $ \dir -> do
    reg <- registryDir
    withEnvVars
