...
module Aapms.Conflict.RetrievalSpec (spec) where
...
import Aapms.Core.Link (LinkKind (OccursIn, PartOf))
import Aapms.Core.Meta (Meta (..), Timeline (..), emptyTimeline)
import Test.Hspec
...
-- "Aapms.Conflict.RetrievalEnvSpec" 驗:expandOneHop 是私有的,而拿真的候選
...
-- | 三筆既有名稱,依 id 字典序 —— 'Aapms.Service.aliasIndex' 的輸出形狀。
sampleIndex :: [(Id, [Text])]
sampleIndex =
  [ (idOf "ent-1001", ["琳達", "小琳"])
  , (idOf "ent-1002", ["織紋刀"])
