...
-- @runService@),@storyflow-store@ 一次都不必露臉——與
-- "StoryFlow.Conflict.PipelineSpec" \/ "StoryFlow.Conflict.JudgeEnvSpec" 同一條
-- 界線。__hermetic__(D6):全程用假 runner 或直接餵 'JudgeStage',不建立任何真正
-- 指向網路端點的用戶端,不發任何網路請求(@acquireJudge@ 分支 3 只建 'Manager',
-- 不呼叫 @chat@)。
module StoryFlow.Conflict.CheckEnvSpec (spec) where
...
import StoryFlow.Conflict.Judge (JudgeRunner, JudgeSkip (..))
import StoryFlow.Conflict.Pipeline
import qualified StoryFlow.Conflict.Retrieval as Retrieval
...
-- ——'StoryFlow.Conflict.Retrieval.matchedNames' 靠「既有名稱是否出現在草稿裡」
...
withVault act =
  withSystemTempDirectory "storyflow-conflict-check" $ \dir -> do
    reg <- registryDir
    withEnvVars
...
withLlmVault act =
  withSystemTempDirectory "storyflow-conflict-check-llm" $ \dir -> do
    reg <- registryDir
    withEnvVars
      [ ("STORYFLOW_VAULTS", dir </> "vaults.toml")
      , ("STORYFLOW_REGISTRY", reg)
