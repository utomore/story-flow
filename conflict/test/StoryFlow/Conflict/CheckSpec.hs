...
-- ("StoryFlow.Conflict.MergeSpec" 對 @mergeContextHits@ \/ @sortContextHits@ 是
-- 同一種做法)。需要真的開 Vault 的部分在 "StoryFlow.Conflict.CheckEnvSpec"。
module StoryFlow.Conflict.CheckSpec (spec) where
...
import StoryFlow.Core.Link (LinkKind (Contradicts, Supersedes))
import Test.Hspec
