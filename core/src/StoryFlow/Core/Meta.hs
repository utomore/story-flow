-- | 統一 Meta:'Entity' / 'Level' / 'Node' 共用同一組欄位(ADR-0003)。
--
-- 統一的用意是抽象與管理成本只付一次——索引表、API 序列化、CLI 輸出、
-- 衝突偵測全部對同一組欄位工作。
module StoryFlow.Core.Meta
  ( -- * 狀態
    Status (..)
  , renderStatus
  , parseStatus
...
-- | 逐欄對應 architecture.md 的欄位表。
--
-- 'metaType' 刻意是 'Text' 而非封閉 enum:ADR-0005 的宣告式註冊表要求 core
-- 不認識任何具體型別。合法性由 "StoryFlow.Core.Registry" 在載入後檢查,
-- 不由編譯器檢查——這是明確買下的取捨。
data Meta = Meta
  { metaId :: Id
  , metaVault :: Text
...
--
-- "StoryFlow.Core.Registry" 以這份清單判斷型別宣告裡的 @fields@ 是否寫錯欄位名。
-- 新增 'Meta' 欄位時這裡也要加——順序與 architecture.md 的欄位表一致。
metaFieldNames :: [Text]
metaFieldNames =
  [ "id"
  , "vault"
