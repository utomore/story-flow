...
# ADR-0006: Schema 用版本追蹤的正向 Migration,不提供 Down-Migration
...
- Migration 3(AI 分類欄位/表)透過字串拼接輔助函式(`upd`/`sub`)組出 `ALTER TABLE` 等
  SQL —— 值全部是編譯期字面值,無注入風險,但屬於已知的脆弱寫法(見 `docs/enhance/`)。
