...
涵蓋 `storyflow-conflict` 一個套件。設計完全依 ADR-0007。

**明確不做**:

- **永不自動修改資料**。衝突報告只是報告,是否成立由作者判斷。第 3 層的品質完全取決於模型,
  地端小模型可能誤判 —— 自動改資料的風險與收益完全不成比例
- 不自己碰檔案與索引。所有讀取經 `subarch-0002` 的 `service`
- 不實作 LLM 端點。那是 `subarch-0004` 的 `llm-endpoint`,本子系統只消費它的介面
...
ADR-0007 分析過兩個直覺解法都不可行:把整個世界觀丟給 LLM,在片段幾百個之後 context 塞不下、
...
**第 2 層的候選撈取策略刻意設計成可替換**。ADR-0007 決定先不做 embedding 語意檢索
...
checkConflict
  :: Maybe LlmClient        -- subarch-0004 的端點;Nothing = 不跑第 3 層
  -> ConflictOpts           -- topN、是否展開 body、timeline 容忍範圍
  -> Draft                  -- 草稿文字 + 已引用的片段 id
  -> ServiceM ConflictReport

-- 只跑前兩層,把相關片段撈出來就交還(ADR-0007 的 context 指令)。
...
以不同前綴區分,`--json` 走 `subarch-0002` 的統一信封。
...
所有讀取都經 `subarch-0002` 的 `ServiceM`,本子系統不直接碰 `store`。
...
`storyflow-service`,加上 `subarch-0004` 的 `storyflow-llm`。**沒有新的外部相依**
——三層用的東西前面的子系統都已經提供。
...
1. **前兩層可交付**:完全確定性、可寫測試、可在沒有任何模型的環境跑。`context` 指令此時就能
   給外部 Agent 用
2. **三層完整**:需要 `subarch-0004` 的 `llm-endpoint` 先到位
...
| # | feature | 一句話說明 | 依賴 | spec |
|---|---------|-----------|------|------|
| 5 | conflict-llm | 第 3 層:草稿 × 候選逐對判斷,優先送 `summary` | #3, subarch-0004 #1 | - |
...
**#6 的關聯建議提示**是 ADR-0007「影響/中立」那條的實作:第 1 層的價值取決於作者是否勤於
