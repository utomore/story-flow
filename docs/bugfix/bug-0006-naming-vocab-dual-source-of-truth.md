...
- [x] T2: 依決定的方案實作(刪表改註解)
- [x] T3: 更新 `docs/architecture.md` 與 ADR-0004 反映最終決策
...
1. **ADR-0004 的原始訴求已經達成。** `parseLogicalName` 根本不驗證 `domain`,任何合法
...
- 新增 `store` migration 004(`DROP TABLE IF EXISTS naming_vocab`)。**不動 migration 001**
  —— 它已經在真實資料庫上跑過,ADR-0006 定的規則是只加不改。代價是新庫會先建表播種再
...
- `ai/Vocab.hs`:補上「分類詞彙可資料化、命名文法詞彙不可」的分界,原本那段把兩者
  相提並論的註解已修正。
- `docs/architecture.md` 命名文法段落與 ADR-0004 的「考慮過的替代方案」/「影響」同步更新。
