...
> TsTypesSpec。bug-0001 至 bug-0005 的修復已經補進 `server/AppSpec.hs`、
...
| **server** | **3** | AppSpec 覆蓋 db 路徑、綁定位址與 thumb(bug-0002/0004/0005);health、search/facets 的 `mkQuery` limit 夾制仍無測試 |
| **cli** | **2** | OptionsSpec 覆蓋 `resolveDbPath`、EndToEndSpec 覆蓋組合根接線(bug-0001);**參數解析本身仍無測試** |
| **web** | **0** | 無任何測試設施 |
...
- `web` 的測試設施 —— 原文件已判定投資報酬率低,主要風險在型別契約,由 enhance-0014 覆蓋。
...
| 套件 | 改善前 | 改善後 |
|---|---|---|
| store | 102 | 109(+7,來自 enhance-0004) |
| server | 43 | 54(+11) |
| project | 17 | 24(+7) |
| cli | 13 | 37(+24) |
| **全專案合計** | **485** | **534(+49)** |
