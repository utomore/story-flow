...
# ADR-001: 後端用 Haskell、前端用 TypeScript、儲存用 SQLite

## 狀態(Status)

Accepted。已完整實作(見 `.design/system.md` 開發階段 0–12)。
...
- **前端**:Vite + React + TypeScript,型別由後端手寫產生器(`server/TsTypes.hs`)輸出,
  不使用 OpenAPI(見 ADR 相關討論於 `.design/system.md` 資料結構章節)。
...
- Haskell 在影像雜活(TIFF/PSD/HEIC 解碼)與 rar/7z 解壓上生態較弱,需要外包給 CLI
  sidecar(見 ADR-005)。這是刻意的取捨,不是缺陷。
- 前端型別安全依賴 `TsTypesSpec` 與人工執行 `--emit-types`。磁碟上的
  `web/src/api/types.ts` 是否為最新產物,已由 `delivery/E005` 補上漂移檢查
  (跑在 `cabal test all` 裡)。
- 單檔 SQLite 意味著同時只能有一個寫入者持有長交易;`busy_timeout=5000` 與
  「LLM 呼叫絕不跨交易持有寫鎖」是這個選擇下游的具體設計後果(見 ADR-007)。
