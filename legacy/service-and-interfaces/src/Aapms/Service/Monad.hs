...
import Aapms.Service.Error (ServiceError (..))
import Aapms.Store
  ( IndexIssue
  , StoreError
  , Vault
...
import Aapms.Types.Loader (defaultRegistryDir, loadRegistry, registryBesideExecutable, registryEnvVar)
...
-- "Aapms.Service.Error" 委派回去,這一層不重寫。
...
-- | @~\/.config\/aapms\/vaults.toml@,或 'vaultsEnvVar' 指定的檔案。
--
...
-- 'Aapms.Core.Registry.checkEntity' 對每個 Entity 都回
