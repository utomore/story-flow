...
spec = describe "套件邊界" $ do
  ...
  -- B001 T3:逐字釘住。library 段只准這四個 aapms-* ——多一個就紅,
  -- 而「多出來的那一個」不必事先被列進任何黑名單。
  it "library 段的 aapms-* 相依逐字相符" $ do
    txt <- readCabal
    internalDeps (librarySection txt) `shouldBe` sort libraryInternal

  it "test-suite 段的 aapms-* 相依逐字相符" $ do
    txt <- readCabal
  ...
  -- aapms-conflict 不在黑名單上,測試卻依然全綠。
  it "守衛擋得住往契約層加 aapms-conflict(B001 重現)" $ do
    let mutated = map (", " ++) (libraryInternal ++ ["aapms-conflict"])
    internalDeps mutated `shouldNotBe` sort libraryInternal

  -- 同一個洞的另一半:未來才會出現的套件也要被擋住,而它們現在還不存在,
  -- 所以任何黑名單都列不到它們。
  it "守衛擋得住未來才出現的 aapms-workshop / aapms-mcp" $
    mapM_
      ( \p -> do
          let mutated = map (", " ++) (libraryInternal ++ [p])
          (p, internalDeps mutated == sort libraryInternal) `shouldBe` (p, False)
      )
      ["aapms-workshop", "aapms-mcp", "aapms-llm"]

-- | 介面層才該有的相依。出現在這裡就是架構違規(ADR-006)。
--
-- __只列第三方套件__:內部 @aapms-*@ 走下面的逐字清單,不靠黑名單。
forbidden :: [String]
forbidden = ["servant", "warp", "optparse-applicative", "http-client"]

required :: [String]
required = ["aapms-core", "aapms-md", "aapms-store", "aapms-types"]
...
libraryInternal = ["aapms-core", "aapms-md", "aapms-store", "aapms-types"]

-- | test-suite 段允許的內部相依,__完整清單__。比 library 多一個
-- @aapms-service@(測試要 import 受測套件本身)。
testInternal :: [String]
testInternal = "aapms-service" : libraryInternal

-- | 一段 @build-depends@ 裡出現的 @aapms-*@ 套件名,排序後回傳。
--
-- 取的是名字本身而不是整行,版本約束改動不會讓斷言紅。
internalDeps :: [String] -> [String]
internalDeps = sort . concatMap pick
  where
    pick l = [w | w <- words (map sep (trim l)), "aapms-" `isPrefixOf` w]
...
readCabal :: IO String
readCabal = go ["aapms-service.cabal", "service/aapms-service.cabal"]
  where
    go [] = fail "找不到 aapms-service.cabal"
    go (c : rest) = do
      ok <- doesFileExist c
      if ok
