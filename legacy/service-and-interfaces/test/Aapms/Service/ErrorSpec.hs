...
module Aapms.Service.ErrorSpec (spec) where

import Data.Char (isLower)
import Data.List (nub)
import qualified Data.Text as T
import Aapms.Core.Id (Id, IdPrefix (PEnt))
import Aapms.Core.Link (Link (..), LinkKind (Contradicts, PartOf))
import Aapms.Core.Registry (EntityWarning (..))
import Aapms.Core.Tree (TreeError (NoRoot))
import Aapms.Md (MdError (..), MdErrorKind (NoFrontmatter))
import Aapms.Service
import Aapms.Service.Fixtures (idOf, refOf)
import Aapms.Store (StoreError (..), renderStoreError)
import Aapms.Types.Loader (LoadError (RegistryDirMissing))
import Test.Hspec
