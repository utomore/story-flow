-- | @aapms-llm@ 的門面:__地端與雲端共用的同一組型別與呼叫路徑__。
--
-- 與 "Aapms.Store" \/ "Aapms.Service" 同一種形狀——消費者只 import 一個
-- 名字,不必知道套件內部分了幾個模組。
--
-- 典型用法:
--
-- @
-- Right cfg <- 'Aapms.Service.runService' env 'llmConfig'
...
-- (例如 "Aapms.Llm.Config" 的 @chatEndpoint@)一併穿透門面,公開面就不再
...
import Aapms.Llm.Client
import Aapms.Llm.Config
