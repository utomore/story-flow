...
-- 'StoryFlow.Service.vaultConfig' 這個內嵌出口拿 @VaultConfig@,本套件
-- __不直接依賴 @storyflow-store@__ ——與 @storyflow-conflict@「所有讀取經
-- @ServiceM@」是同一條紀律,由 @StoryFlow.Llm.CabalSpec@ 釘住。
--
-- __地端與雲端只差兩個值__:'lcBaseUrl' 與 'lcApiKey'。除此之外
-- 'StoryFlow.Llm.Client.newLlmClient' 與 'StoryFlow.Llm.Client.chat' 的程式路徑
...
import StoryFlow.Service (LlmSection (..), ServiceM, VaultConfig (..), vaultConfig)
...
-- 與 'StoryFlow.Llm.Error' 被拆出來是同一個理由。
...
-- ——那是兩個完全不同的下一步。
--
-- __未知鍵一律是錯__,理由與 'StoryFlow.Types.Loader' 的「不容忍未知鍵」同一條:
...
-- 錯誤型別,上層不重寫下層的訊息」——@storyflow-llm@ 在 @service@ __之上__,
