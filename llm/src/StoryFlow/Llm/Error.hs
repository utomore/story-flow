-- | LLM 存取層的錯誤語彙。
--
-- __為什麼是獨立的一個模組__:"StoryFlow.Llm.Client" 與 "StoryFlow.Llm.Config"
-- __都要用它__ ——設定載不起來與端點連不上是同一個消費者要處理的同一件事
-- (「這次拿不到模型的回覆」),放在任一邊都會讓另一邊反向 import。
--
-- __風格與 'StoryFlow.Service.Error.ServiceError' 一致但型別獨立__:system.md 的
...
-- 三件事對使用者是三個完全不同的動作。
--
-- __只有 'LlmUnavailable' 會被重試__(見 "StoryFlow.Llm.Client" 的 @chat@):
