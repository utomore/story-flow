...
spec = do
  describe "build-depends" $ do
    ...
    it "library 不含 aapms-core" $ do
      deps <- dependencyLines . librarySection <$> readCabal
      any ("aapms-core" `isInfixOf`) deps `shouldBe` False
  ...
  describe "cabal.project" $ do
    ...
    it "含 package aapms-llm 的 ghc-options 段" $ do
      src <- readProject
      ("package aapms-llm" `isInfixOf` src) `shouldBe` True
...
readCabal :: IO String
readCabal = readFirst ["aapms-llm.cabal", "llm/aapms-llm.cabal"]
