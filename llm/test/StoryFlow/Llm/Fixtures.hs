...
-- 把 'StoryFlow.Llm.Client.chat' 簡化成只測純函式,等於把唯一真正會發生的失敗
-- 路徑排除在測試之外。
--
-- __為什麼不用固定埠號__:任何寫死的埠都可能被別的東西佔著,那正是最典型的
-- 偶發失敗。'withStub' 與 'withDeadPort' 都讓 warp 自己配。
module StoryFlow.Llm.Fixtures
  ( -- * stub 端點
    Stub (..)
  , okStub
  , chatCompletion
...
import StoryFlow.Llm (LlmConfig (..))
...
--
-- 這個前提本身由 @StoryFlow.Llm.StubSpec@ 以一條斷言釘住,不讓它變成
-- 「大概沒人在那個埠上」這種隱性假設。
...
--
-- 傳空清單代表__不寫 @[llm]@ 段__(用來驗 'StoryFlow.Llm.LlmConfigMissing')。
--
-- __順序是硬的__:設定在 'openEnv' 那一步才被讀進 @Vault@,所以 @[llm]@ 段
-- 必須先寫進檔案再 'openEnv'。
--
-- 建 Vault __只靠 @storyflow-service@ 的門面__:@createVault@ \/ @openEnv@ \/
...
withLlmVault llmLines act =
  withSystemTempDirectory "storyflow-llm" $ \dir -> do
    reg <- registryDir
    withEnvVars
...
--
-- 路徑用字面字串組(而不是 @store@ 的 @configPath@):@storyflow-store@ 不在
...
-- | 全是字串值的 @[llm]@ 段。
--
-- 這個底稿的存在讓 @StoryFlow.LlmSpec@ 得以__只 import 門面一個模組__就走完
-- 整輪:'StoryFlow.Llm.Config.parseLlmConfig' 吃的 @LlmSection@ 來自
-- @storyflow-service@,而那個 import 正是門面測試要避開的。
