-- | T1:門面。
--
-- __這個檔案只 import @StoryFlow.Llm@ 一個模組__(加上測試底稿)就走完
-- @parseLlmConfig@ → @newLlmClient@ → @chat@ 一輪。與
-- @StoryFlow.Service.FacadeSpec@ 同一種證明:__消費者不必知道套件內部分了幾個
-- 模組__ ——@storyflow-workshop@ 與 @conflict-detection@ 第 3 層將來就是這樣用它。
--
-- 底稿的 'sectionOf' 存在的理由也在這裡:@LlmSection@ 來自
-- @storyflow-service@,而那個 import 正是這一節要避開的。
module StoryFlow.LlmSpec (spec) where
