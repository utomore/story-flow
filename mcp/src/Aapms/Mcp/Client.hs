...
--
-- __不使用 @servant-client@__(查證結果 4):'Aapms.Mcp.Tools.toolsFromOpenApi'
-- 反推出來的是路徑模板 + method,不是 28 個手寫的 client 函式——照抄
-- @cli\/src\/Aapms\/Cli\/Backend.hs@ 的 @client (Proxy :: Proxy AapmsAPI)@
-- 寫法會讓「新增路由忘了補 tool」不會有任何測試變紅,違反驗收標準。改用
-- @http-client@ 依 @(路徑模板, method, arguments)@ 直接組一次原生 HTTP 請求。
module Aapms.Mcp.Client
  ( probe
  , invoke
  ) where
...
-- @cli\/src\/Aapms\/Cli\/Error.hs@ 的 @RemoteUnavailable@\/@RemoteBadResponse@
-- 版本(查證結果 5)。
