...
import Paths_aapms_mcp (version)
...
-- @cli\/src\/Aapms\/Cli\/Backend.hs@ 的 @managerWith@ 對稱。
...
resolveConfig argv = do
  ...
  pure $ case flagUrl argv `orElse` urlEnv of
    Nothing ->
      Left
        "沒有設定 aapms 伺服器的位址:請用 --url <base> 或設定環境變數 STORYFLOW_URL"
...
-- | @--version@ 印的那一行,格式與 @aapms@ / @aapms-serve@ 相同(G-E002)。
mcpVersion :: Text
mcpVersion = "aapms-mcp " <> T.pack (showVersion version)
