-- | 從 'aapmsOpenApi' 反推全部 MCP tools(llm-workshop-mcp/F005 T5)。
--
-- __完全不連線__:'aapmsOpenApi' 是 @aapms-api@ 匯出的純值,在編譯期就
-- 已經確定,不是伺服器執行期反射自己的路由表。這正是驗收標準「tools 數 ==
-- OpenAPI operation 數」能在編譯期成立的原因——'AapmsAPI' 一改,
-- 'aapmsOpenApi' 自動跟著變,這個模組的輸出不需要任何人手動同步。
module Aapms.Mcp.Tools
  ( Tool (..)
  , toolsFromOpenApi
  , lookupTool
  ) where
...
-- 是 'Aapms.Mcp.Client.invoke' 組 HTTP 請求要用的路徑模板與方法,
...
-- | 依 tool 名字反查。名字不存在時回 'Nothing'——'Aapms.Mcp.Server' 據此
