-- | T1 / T10:套件骨架與套件邊界。
--
-- 「@aapms-mcp@ 不 import @aapms-service@(只打 HTTP)」這條硬性邊界唯一
-- 守得住的形式,是 @.cabal@ 檔裡__沒有那些名字__——作法逐字沿用
-- @Aapms.Workshop.CabalSpec@\/@Aapms.Llm.CabalSpec@ 的先例:同一條紀律
...
module Aapms.Mcp.CabalSpec (spec) where
...
spec = do
  ...
  it "autogen-modules 含 Paths_aapms_mcp(G-E002)" $ do
    src <- readCabal
    ("Paths_aapms_mcp" `isInfixOf` src) `shouldBe` True
  ...
  describe "build-depends" $ do
    ...
    it "library 含 aapms-api" $ do
      deps <- dependencyLines . librarySection <$> readCabal
      any ("aapms-api" `isInfixOf`) deps `shouldBe` True

    it "library 不含 aapms-service / store / md / conflict / workshop / llm / core" $ do
      deps <- dependencyLines . librarySection <$> readCabal
  ...
  describe "模組" $ do
    ...
    it "執行檔 aapms-mcp 存在" $ do
      src <- readCabal
      ("executable aapms-mcp" `isInfixOf` src) `shouldBe` True
  ...
  describe "cabal.project" $ do
    ...
    it "含 package aapms-mcp 的 ghc-options 段" $ do
      src <- readProject
      ("package aapms-mcp" `isInfixOf` src) `shouldBe` True
...
readCabal :: IO String
readCabal = readFirst ["aapms-mcp.cabal", "mcp/aapms-mcp.cabal"]
