-- | T7:'Aapms.Mcp.Client' 的連線探測與實際呼叫。
--
-- 起本機 warp stub 模擬 @aapms-serve@(見 'Aapms.Mcp.Fixtures'),
-- 不寫死埠號、不用固定長 sleep——與 llm\/workshop 兩個子系統既有的 Fixtures
-- 同一個先例。
module Aapms.Mcp.ClientSpec (spec) where
...
-- 一個 requestBody(@\"body\"@ 鍵)。不用真正的 aapmsOpenApi 反推,是刻意
-- 的:這個模組只測「Client 依這個形狀組出正確的 HTTP 請求」,不重複
-- 'Aapms.Mcp.ToolsSpec' 已經測過的反推邏輯。
