-- | 測試底稿:一個真的跑在本機、模擬 @aapms-serve@ 的 stub 端點,以及一個
-- 保證__拒絕連線__的埠。與 @llm\/test\/Aapms\/Llm\/Fixtures.hs@、
-- @workshop\/test\/Aapms\/Workshop\/Fixtures.hs@ 同一個先例:「連不上服務」
-- 是這一層最常發生的失敗路徑,必須被真的測到,不能只測純函式。
module Aapms.Mcp.Fixtures
  ( -- * stub 端點
    Rule
  , rule
  , RecordedRequest (..)
