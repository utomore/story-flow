...
-- 沿用 'StoryFlow.Mcp.Fixtures',@tools\/list@ 不連線的斷言則故意把 stub 晾在
-- 一邊、事後檢查它完全沒被打到。
module StoryFlow.Mcp.ServerSpec (spec) where
...
import StoryFlow.Mcp.Server (processLine)
import StoryFlow.Mcp.Tools (toolsFromOpenApi)
import Test.Hspec
  ...
  describe "initialize——成功" $
    it "echo protocolVersion,capabilities.tools 是空物件" $
      withStub [rule "GET" "/vaults" 200 (Array mempty)] $ \stub -> do
        ...
        v <- resultOf out
        v `shouldBe` Just (object ["protocolVersion" .= ("2026-06-18" :: String), "capabilities" .= object ["tools" .= object []], "serverInfo" .= object ["name" .= ("story-flow-mcp" :: String), "version" .= ("0.1.0" :: String)]])
