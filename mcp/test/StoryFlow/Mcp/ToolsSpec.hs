-- | T5 / T11:'StoryFlow.Mcp.Tools' 從 'storyFlowOpenApi' 反推出來的 tools。
--
-- 第一條斷言就是驗收標準的可測形式:tools 數 == OpenAPI operation 數,兩邊都從
-- 同一份 'storyFlowOpenApi' 算出來,不會出現「新增 REST 路由卻忘了補 tool」。
module StoryFlow.Mcp.ToolsSpec (spec) where
...
import StoryFlow.Mcp.Tools (Tool (..), lookupTool, toolsFromOpenApi)
import Test.Hspec

spec :: Spec
spec = do
  let tools = toolsFromOpenApi storyFlowOpenApi

  describe "tools 數" $
    it "等於 storyFlowOpenApi 的 operation 數(28)" $
  ...
  where
    schemaOf nm = do
      t <- lookupTool nm (toolsFromOpenApi storyFlowOpenApi)
...
allOperations =
  [op | (_, item) <- IOM.toList (storyFlowOpenApi ^. paths), (_, Just op) <- verbsOf item]
