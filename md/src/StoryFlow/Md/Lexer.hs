-- | 逐行切塊 —— 兩階段解析的第一階段(ADR-0010)。
--
-- 本模組只認得四件事:哪幾行是 frontmatter、哪一行是節標題、哪一段是
-- @```meta@ 區塊、其餘是正文。每一塊的__原始文字原封不動保留__,解讀是
-- "StoryFlow.Md.Parse" 的事。
--
-- == 分節界線(func-0003 實作備註 2)
--
-- __第一個帶 @{#id}@ 的標題才開始分節__。architecture.md 的琳達範例裡
-- @# 琳達@ 因此留在 'StoryFlow.Md.Document.docPreamble';第一個節之後的標題
...
-- | 由 'StoryFlow.Md.Document.secMetaRaw' 取出 YAML 內容,
