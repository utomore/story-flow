...
doc :: Document
doc = docOf classroomMd

lvl :: Level
nodes :: [Node]
(lvl, nodes) = levelOf doc

nodeBy :: Text -> Node
nodeBy i = case filter ((== idOf i) . metaId . nodMeta) nodes of
  (n : _) -> n
...
spec = do
  describe "檔案的判別" $
    it "frontmatter 的 type: level 判為 LevelDoc" $
      docKind doc `shouldBe` LevelDoc
  ...
  describe "Level 本身" $ do
    it "lvlMeta 逐欄比對 frontmatter" $ do
      ...
      metaType m `shouldBe` typeOf "level"
    ...
    it "frontmatter 寫了相符的 root 時照收" $ do
      ...
      fmap (lvlRoot . fst) (toLevel (docOf withRoot))
        `shouldBe` Right (idOf "nod-0001")

    it "frontmatter 的 root 與第一個節不符 → RootMismatch" $ do
      let withRoot = T.replace "type: level" "type: level\nroot: nod-0009" classroomMd
      leftKind (toLevel (docOf withRoot))
        `shouldBe` Just (RootMismatch (idOf "nod-0009") (idOf "nod-0001"))
