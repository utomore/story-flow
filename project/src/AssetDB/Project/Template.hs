...
skillMd name credits =
  T.unlines
    ...
    , "而且授權閘門也不會看過它。"
    , ""
    , "增量加入素材用 `assetdb project sync`。它以專案名稱找到這個專案(建立時就登記了),"
    , "把符合條件的素材與專案現況對帳成四類 ——「新增」「已存在」「來源已更新」「本地已修改」"
    , "—— 然後**只加入「新增」那一類**。"
    , ""
    , "```bash"
    , "# 先預覽:列出四類的筆數與清單,不會動到任何檔案"
    , "assetdb project sync --name <名稱> --pack <包1> --match <關鍵字>"
    , ""
    , "# 確認結果沒問題後再真的加入"
    , "assetdb project sync --name <名稱> --pack <包1> --match <關鍵字> --confirm"
    , "```"
    , ""
    , "`--confirm` 之後 `manifest.json` 與 `Assets.hs` 會依登記的**全集**重新產生,"
    , "其餘檔案(這份 SKILL.md、README.md、`.cabal`、`docs/`)一個都不重寫。"
    , ""
    , "同步**不刪除、不覆蓋、不搬移**專案裡任何既有檔案。「來源已更新」與「本地已修改」"
    , "只會被列出來讓你知道,要怎麼處理由你決定。"
    , ""
    , "## 授權"
    , ""
    , credits
