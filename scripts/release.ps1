...
#
# 產出:dist-release\aapms-<版本>-windows-x64\ 與同名 .zip,內含恰好:
#   aapms.exe  aapms-serve.exe  aapms-mcp.exe  registry\*.toml  README.md
#
...
if ($Stage -eq "") {
  $Stage = Join-Path ([System.IO.Path]::GetTempPath()) ("aapms-release-" + [System.IO.Path]::GetRandomFileName())
...
$Exe = Join-Path $Stage "aapms.exe"
if (-not (Test-Path $Exe)) { Write-Host "$Stage 裡沒有 aapms.exe"; exit 1 }

if ($Version -eq "") {
  # 輸出是「aapms <版本>」一行
  $line = (& $Exe --version) | Select-Object -First 1
  if ($LASTEXITCODE -ne 0 -or -not $line) { Write-Host "aapms --version 沒有回版本"; exit 1 }
...
foreach ($bin in @("aapms.exe", "aapms-serve.exe", "aapms-mcp.exe")) {
