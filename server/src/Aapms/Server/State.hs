...
import Aapms.Server.Error (toServerError)
import Aapms.Service (Env, ServiceError, ServiceM, closeEnv, openEnv, runService)
...
-- | 不需要 'Env' 的兩個操作('Aapms.Service.listVaults' \/
-- 'Aapms.Service.createVault')走這條。
...
-- __第二種__錯誤 @e@(例如工作坊的 'Aapms.Workshop.WorkshopError'),由呼叫端
