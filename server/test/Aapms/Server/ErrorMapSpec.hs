-- | T7:每個 'Aapms.Service.ServiceError' 都對到規格表的狀態碼。
--
-- 分派鍵是 'Aapms.Service.errorCode' 的字串,不是 @StoreError@ 的建構子
-- (理由見 "Aapms.Server.Error")。代價是新增建構子時這裡不會編譯失敗,
-- 所以有最後那一條測試:拿 service 真的產得出來的代碼去比對 'knownCodes',
-- 少一個就代表有代碼會靜靜地落到預設的 500。
module Aapms.Server.ErrorMapSpec (spec) where
...
import Aapms.Core.Id (Ref, parseRef)
import Aapms.Core.Registry (EntityWarning (MissingRequiredField))
import Aapms.Server.Error (errorBody, knownCodes, statusForCode, toServerError)
...
--
-- @StoreFailed@ 的那些構造不出來——@StoreError@ 的建構子在 @aapms-store@ 裡,
