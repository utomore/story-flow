...
--
-- client 由 'client' 'aapmsAPI' 產生而不是自己拼 URL:這一組測試要驗的正是
-- 「server 與 client 由同一個型別產生,不可能悄悄長歪」,自己拼 URL 就把那個保證
-- 繞過去了。
module Aapms.Server.Fixtures
  ( -- * 環境
    withServer
  , withServerToken
  , withServerDir
...
import Aapms.Api
import Aapms.Conflict.Types (ConflictReport, ContextHit)
...
withVaultDir act =
  withSystemTempDirectory "aapms-server" $ \dir -> do
    reg <- registryDir
    absReg <- makeAbs reg
    withEnvVars
...
-- 'Aapms.Server.State.AppState' 的 'Aapms.Service.Env' 是延遲取得的
