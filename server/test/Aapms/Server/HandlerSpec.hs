...
-- 編譯期就死了;這裡驗的是執行期真的通。
module Aapms.Server.HandlerSpec (spec) where

import Data.List (sort)
import qualified Data.Text as T
import Aapms.Api (BodyReq (..), CheckReq (..), ContextReq (..), NewVaultReq (..))
...
import Aapms.Core.Id (localRef, renderId)
import Aapms.Core.Level (Level (..), Node (..))
import Aapms.Core.Link (Link (..), LinkKind (Contradicts, PartOf, References))
-- @Status@ 的 @Draft@ 建構子與 'Aapms.Conflict.Types.Draft' 同名,
...
spec = describe "每條路由" $ do
  describe "vault / index / types" $ do
    ...
    it "POST /vaults 建第二個 Vault" $ withServer $ \env -> do
      root <- pure "/tmp/aapms-second-vault-does-not-matter"
  ...
  describe "conflict(conflict-detection/F006)" $ do
    ...
    -- opts 鍵缺席時退回 defaultConflictOpts 的解析行為由 Aapms.Api.HttpDataSpec
