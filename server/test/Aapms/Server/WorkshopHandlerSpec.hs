...
--
-- 仿 "Aapms.Server.HandlerSpec" T9:以 warp 起臨時伺服器 + @servant-client@,
...
-- "Aapms.Server.Fixtures" 的 'withServerDir' 註解——'Env' 延遲取得,第一個
-- 請求送出前寫檔就來得及)。
module Aapms.Server.WorkshopHandlerSpec (spec) where
...
import Aapms.Api (WorkshopCommitResp (..), WorkshopStartReq (..), WorkshopStepReq (..), WorkshopStepResp (..))
...
-- @workshop\/test\/Aapms\/Workshop\/StepSpec.hs@ 的 @draftReply@ 同一種寫法。
draftReply :: Text
draftReply =
  T.unlines
    [ "以下是這個階段可以定案的草稿。"
...
-- stub 的埠。與 @llm\/test\/Aapms\/Llm\/Fixtures.hs@ 的 @appendConfig@
-- 同一份寫法(本測試套件不依賴 @aapms-store@,路徑用字面組)。
