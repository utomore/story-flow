...
--
-- 仿 "StoryFlow.Server.ErrorMapSpec" 的表格式斷言,分派鍵同樣是字串
-- ('StoryFlow.Workshop.workshopErrorCode' 的回傳值),不 pattern match
-- 'WorkshopError' 的建構子。12 個 code = 7 個工作坊自己的 + 5 個
-- 'WsLlmFailed' 原樣沿用的 @llmErrorCode@ ——後者第一次跨過 HTTP,狀態碼由本
-- feature(F004)決定。
module StoryFlow.Server.WorkshopErrorMapSpec (spec) where
