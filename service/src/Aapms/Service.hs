-- | @aapms-service@ 的門面:__所有業務操作的唯一定義處__(ADR-006)。
--
...
-- __業務操作__。差距具體是四件事:
--
-- * 型別註冊表的驗證有人呼叫了('Aapms.Core.Registry.checkEntity' 在
--   @store@ 那一層是沒人用的純函式)
...
module Aapms.Service
  ...
  , LlmSection (..)
    ...
    -- ** 沿用 @types@ 的定義(不重造)
    --
    -- G-E002:CLI 的 @doctor@ 要說出註冊表從哪一層載入。CLI 不依賴 @aapms-types@,
    -- 與它不依賴 @aapms-store@ 同一條紀律——所有讀取經本門面。
  , RegistrySource (..)
  , locateRegistry
  , loadRegistry
  , renderLoadError
...
import Aapms.Md (applyOverride, overrideOf, renderMdWarning)
import Aapms.Service.Error
import Aapms.Service.Json ()
import Aapms.Service.Monad
import Aapms.Service.Types
import Aapms.Store hiding
  ( addFragment
  , addNode
  , deleteEntity
  , deleteLevel
...
import qualified Aapms.Store as S
import Aapms.Store.Edit (Located (..), locate, locateNode)
...
--
-- __只開內嵌出口__:不進 @AapmsAPI@、不進 CLI 的指令樹。理由與 'linkGraph' /
-- 'aliasIndex' 同一條——Vault 設定不是作者用指令查的東西,是__子系統之間的讀取__;
-- 作者要看設定就直接打開那個檔案,而序列化整份設定送給外部客戶端只會多一個
-- 得跟著維護的 DTO。
--
-- 存在的理由是硬性的:@[llm]@ 那張表('cfgLlm')住在 @aapms-store@,而
-- @aapms-llm@ 的 @build-depends@ 逐字擋著 @aapms-store@ ——與
-- @aapms-conflict@ 完全相同的界線。那個套件拿得到 Vault 設定的唯一合法途徑
-- 就是經 'ServiceM'。
--
-- __不解讀 @[llm]@__:這裡原樣交出 'LlmSection' 捧著的 TOML 表。設定的形狀
-- (@base_url@ / @model@ / …)由 @aapms-llm@ 定義並解析,service 這一層
-- 認得的只有「有沒有那張表」。
...
-- @aapms-workshop@ 的 session 快照要寫 @\<root\>\/.storyflow\/workshops\/@,
-- 而它與 @aapms-llm@ 同樣不准依賴 @aapms-store@,拿不到 'S.vaultRoot'。
--
...
-- ('Aapms.Store.Query.normalizeBm25'),這一層再壓一次只會讓兩處各有一份
-- 規則。
...
-- ('Aapms.Md.Inherit.inheritMeta')——@tags@ 聯集、@summary@ 不繼承、
