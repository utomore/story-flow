...
--
-- __本檔刻意只 import @Aapms.Service@__:與 'Aapms.Service.VaultRootSpec'
-- 同一個理由——被測的是 service 的匯出面,不是底層 @Vault@ 的欄位存取子。
module Aapms.Service.LocateSpec (spec) where

import Control.Exception (bracket)
import Data.List (isInfixOf)
import qualified Data.Text as T
import Aapms.Service
import Aapms.Service.Fixtures (orDieS, shouldFailWith, withEnvVars, withVaultDir)
      ...
      withVaultDir $ \dir -> do
        ...
        r <-
          withoutEnv "STORYFLOW_REGISTRY" $
            -- Paths_aapms_types 自己的覆寫鉤子:把 data-files 指到不存在的地方,
            -- 這樣第三層一定找不到,測試才不受「cabal test 有沒有替相依套件設 datadir」影響
            withEnvVars [("aapms_types_datadir", dir </> "no-datadir")] $
              openEnv Nothing dir
