...
--
-- @
-- LAW-13,EX-14     讀取範圍與裁決一致             -> prop_with_read_matches_scope_resolution, test_with_read_all_registered_example
-- LAW-14,EX-15,EX-16 ScopeIssue 不中止 / 空範圍      -> prop_with_read_scope_issue_does_not_abort, test_with_read_broken_vault_excluded_example, test_with_read_empty_hub_example
-- LAW-15         VaultSet 關掉、handle 不關       -> prop_with_read_closes_vault_set_not_handles
-- LAW-16,EX-17     沒有寫入目標即失敗              -> prop_with_write_no_target_fails, test_with_write_outside_no_selector_example
-- LAW-17,EX-18     寫入目標唯一                   -> prop_with_write_target_matches_resolve, test_with_write_selector_story_example
-- LAW-18,EX-19,EX-20 管線只給符合 kind 的            -> prop_with_pipeline_filters_by_kind, test_with_pipeline_asset_example, test_with_pipeline_story_empty_example
-- @
...
spec = describe "F001 Aapms.Service.Scope" $ do
  --------------------------------------------------------------------------
  describe "LAW-13/EX-14: withRead 的讀取範圍與 resolveRead 的裁決一致" $ do
    it "prop_with_read_matches_scope_resolution (LAW-13): 對成功解析的 sel,withRead 兩個回傳與 resolveRead 的 rsVaults 三者的 vmId 清單相同" $
      hedgehog $ do
    ...
    it "test_with_read_all_registered_example (EX-14): sel == Nothing 時兩個清單都等於中樞順序 [VA, VB]" $
      withFixedLayout $ \fl ->
  ...
  --------------------------------------------------------------------------
  describe "LAW-14/EX-15/EX-16: ScopeIssue 不中止 withRead" $ do
    it "prop_with_read_scope_issue_does_not_abort (LAW-14): 中樞多一個路徑不存在的 vault 時,withRead 仍回 Right,壞的那個被排除" $
      hedgehog $ do
    ...
    it "test_with_read_broken_vault_excluded_example (EX-15): 壞的 vault 被排除,vaultSetIds 只剩 [VA, VB]" $
    ...
    it "test_with_read_empty_hub_example (EX-16): 中樞沒有任何 vault 時仍回 Right [],k 有被呼叫" $
  ...
  --------------------------------------------------------------------------
  describe "LAW-15: withRead 結束後 VaultSet 被關、handle 仍在快取" $
    it "prop_with_read_closes_vault_set_not_handles (LAW-15): withRead 結束後同一個 env 再跑一次 withRead 仍成功" $
      hedgehog $ do
        outcome <- liftIO $ withFixedLayout $ \fl ->
          withOpenEnv Nothing (flOutsidePath fl) $ \env -> do
  ...
  --------------------------------------------------------------------------
  describe "LAW-16/EX-17: 沒有寫入目標即失敗" $ do
    it "prop_with_write_no_target_fails (LAW-16): 起點在 outside/ 之下、sel == Nothing 時,withWrite 回 Left (WorkspaceFailed (NoWriteTarget _)),k 不被呼叫" $
      hedgehog $ do
        outcome <- liftIO $ withFixedLayout $ \fl ->
          withOpenEnv Nothing (flOutsidePath fl) $ \env -> do
    ...
    it "test_with_write_outside_no_selector_example (EX-17): NoWriteTarget 帶正規化後的起點,k 一次都沒被呼叫" $
      withFixedLayout $ \fl ->
        withOpenEnv Nothing (flOutsidePath fl) $ \env -> do
  ...
  --------------------------------------------------------------------------
  describe "LAW-17/EX-18: 寫入目標唯一" $ do
    it "prop_with_write_target_matches_resolve (LAW-17): sel 解得到時,withWrite 交出去的目標 vmId 恆等於 resolveWrite 的 wsTarget,讀取範圍等於 wsRead" $
      hedgehog $ do
    ...
    it "test_with_write_selector_story_example (EX-18): withWrite 交出 (VA, [VA])" $
      withFixedLayout $ \fl ->
  ...
  --------------------------------------------------------------------------
  describe "LAW-18/EX-19/EX-20: withPipeline 只給符合 kind 的" $ do
    it "prop_with_pipeline_filters_by_kind (LAW-18): 每個交給 k 的 handle 都符合要求的 kind" $
      hedgehog $ do
    ...
    it "test_with_pipeline_asset_example (EX-19): AssetVault + sel Nothing 只剩 VB" $
      withFixedLayout $ \fl ->
        withOpenEnv Nothing (flOutsidePath fl) $ \env -> do
          result <- runService env (withPipeline AssetVault (\hs -> pure (map vmidOfHandle hs)))
          result `shouldBe` Right [flVbId fl]

    it "test_with_pipeline_story_empty_example (EX-20): 中樞只剩 VB 時 StoryVault 回空清單,k 仍被呼叫" $
      withOnlyVbLayout $ \fl ->
        withOpenEnv Nothing (flOutsidePath fl) $ \env -> do
          called <- newIORef False
          result <-
