-- | T15:Level 與 Node。
--
-- 'getLevel' 回的是__樹__而不是扁平清單:'StoryFlow.Core.Tree.buildTree' 已經
-- 驗過合法性,把結果丟掉會讓 CLI 與 server 各自重建一次。而作者隨時可以直接
-- 編輯 Level 檔把樹改壞——那時候要回報,不是崩潰。
module StoryFlow.Service.LevelSpec (spec) where

import qualified Data.ByteString as BS
import qualified Data.Text as T
import qualified Data.Text.Encoding as TE
import StoryFlow.Core.Level (Level (..), Node (..), NodeKind (KCamera, KCast, KScene))
import StoryFlow.Core.Meta (Meta (..))
import StoryFlow.Core.Tree (NodeTree (..), preorder)
