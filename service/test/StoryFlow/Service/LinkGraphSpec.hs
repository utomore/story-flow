...
-- @StoryFlow.Store.Index.localize@ 那一端變了,這裡先紅。
module StoryFlow.Service.LinkGraphSpec (spec) where

import Data.List (sort)
import qualified Data.Map.Strict as M
import Data.Maybe (isNothing)
import StoryFlow.Core.Id (Id, Ref (..), localRef, renderId)
...
spec = describe "linkGraph" $ do
  ...
  it "指向本 Vault 的目標一律 refVault = Nothing(正規化由索引寫入端做掉)" $
    withServiceEnv $ \env -> do
      -- 刻意用帶著本 Vault 前綴的 Ref 寫進去。StoryFlow.Store.Index 的 localize
