...
--
-- __本檔刻意只 import @StoryFlow.Service@__:被測的是 service 的__匯出面__ ——
-- @VaultConfig (..)@ 與 @LlmSection (..)@ 有沒有真的 re-export 出來。若這裡改成
-- 從 @StoryFlow.Store@ 拿型別,測試會照樣通過,而 @storyflow-llm@(它的
-- @build-depends@ 逐字擋著 @storyflow-store@)卻會編不起來。
module StoryFlow.Service.VaultConfigSpec (spec) where
