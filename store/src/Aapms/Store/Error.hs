...
import Aapms.Core.Id (Id, IdPrefix, Ref, renderId, renderIdPrefix, renderRef)
...
renderStoreError = \case
  VaultNotFound name
    | T.null name ->
        "從目前目錄向上找不到 .storyflow/;"
          <> "請在 Vault 根目錄執行 aapms vault init,或以 --vault <名稱> 指定"
    | otherwise ->
        "全域註冊表裡沒有名為「"
          <> name
          <> "」的 Vault;"
          <> "請執行 aapms vault init 建立,或檢查 ~/.config/story-flow/vaults.toml"
  ...
  IndexUpdateFailed fp msg ->
    pack fp
      <> ": 檔案已寫入成功,但索引更新失敗 —— "
      <> msg
      <> ";資料是安全的,執行 aapms index rebuild 即可"
  ParseFailed fp es ->
