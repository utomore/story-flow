...
import Aapms.Core.Id (Id, IdPrefix (PNod))
import Aapms.Core.Level (Level (..))
...
-- 'Aapms.Md.Render.insertSection' 的「之後」指的是該節本身,所以子樹的
-- 尾端要自己算出來。
addNode
...
-- 請呼叫端改用 'Aapms.Store.Create.deleteLevel'。
--
