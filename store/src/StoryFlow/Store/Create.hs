-- | 建檔、增節、刪除。
--
-- 「改既有實體」在 "StoryFlow.Store.Write",「Level 樹編輯」在
-- "StoryFlow.Store.Node";共用的紀律在 "StoryFlow.Store.Edit"。
--
-- == 檔案放哪裡
--
-- 目錄是__宣告式__的:'StoryFlow.Core.Registry.lookupDir' 查型別註冊表,
...
module StoryFlow.Store.Create
  ...
  , deleteLevel
    ...
    -- * 檔名(供 "StoryFlow.Store.Node" 與測試使用)
  , sanitizeFileName
  , preambleOf
  , freshPath
  , guardReferences
...
import StoryFlow.Core.Id (Id, IdPrefix (PEnt, PLvl, PNod), localRef, renderId)
import StoryFlow.Core.Level (Level (..), Node (..), NodeKind)
import StoryFlow.Core.Link (Link (..))
import StoryFlow.Core.Meta
import StoryFlow.Core.Registry (TypeRegistry, emptyRegistry, lookupDir)
import StoryFlow.Md
import StoryFlow.Store.Edit
import StoryFlow.Store.Error (StoreError (..), renderStoreError, trySqlite)
import StoryFlow.Store.Index (unindexFile)
import StoryFlow.Store.Query (linksTo)
...
-- | 建一份新的 Entity 檔。
--
-- __先檢查檔案不存在再寫__:'StoryFlow.Store.Atomic.atomicWriteText' 是覆蓋
...
-- 請用 'StoryFlow.Store.Node.removeNode'。
deleteLevel
  :: Connection -> Vault -> Id -> Int -> DeleteMode -> IO (Either StoreError DeleteResult)
