...
--
-- 'refreshStale' 是 ADR-0002「檔案被外部改動後索引過時」的答案:每次查詢前跑
...
import StoryFlow.Core.Id (Ref (..), renderId, renderRef)
...
import StoryFlow.Store.Schema (closeIndex, openIndex, setVaultInfo)
import StoryFlow.Store.Vault (Vault (..), vaultAbsPath, vaultRelPath)
...
-- | 索引一份檔案時遇到的問題。
--
-- 帶 @[MdWarning]@ 是刻意的:@storyflow-md@ 是純函式庫,警告只能靠呼叫端輸出,
...
--
-- 這是 @service@(P2)該用的進入點。'StoryFlow.Store.Schema.openIndex' 只保證
