...
import Aapms.Store.Schema (closeIndex, openIndex)
import Aapms.Store.Vault (Vault (..), initVault, vaultAbsPath)
...
withEmptyVault act =
  withSystemTempDirectory "aapms-vault" $ \dir -> do
...
withTempVault = withSystemTempDirectory "aapms-plain"
...
-- | 測試用的型別註冊表。
--
-- 刻意__不去讀 @types\/registry\/@ 的實檔__:那是 @aapms-types@ 的測試
