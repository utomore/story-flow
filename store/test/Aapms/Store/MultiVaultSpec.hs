...
-- LAW-1b  openVaultSet 撞號 -> VaultIdCollision,優先於上限          -> prop_LAW1b_collision / test_LAW1b_collision_over_limit
-- LAW-2   closeVaultSet 之後單一 vault 讀取不受影響                -> test_LAW2
-- LAW-3   openVaultSet 之後單一 vault 讀取不受任何 *Across 呼叫影響 -> test_LAW3
-- LAW-4   listAcross 的多重集合等於逐 vault listNodes 的多重集合   -> prop_LAW4
-- LAW-5   listAcross 全域排序(metaId,平手比 vault)               -> prop_LAW5
-- LAW-6   listAcross 分頁是對整體切窗                              -> prop_LAW6
-- LAW-7   listAcross 每筆的 vault 欄與 metaVault 一致              -> prop_LAW7
-- LAW-8   searchAcross 逐 vault 等價(四欄全比)                    -> prop_LAW8
-- LAW-9   searchAcross 全域排序                                    -> prop_LAW9
-- LAW-10  searchAcross 分頁與 srTotal                              -> prop_LAW10
-- LAW-11  searchAcross 每筆 vault 欄一致                           -> prop_LAW11
-- LAW-12  facet(Just/Nothing、fcVaults 求和、四維度求和、零命中濾除)-> prop_LAW12_toggle / prop_LAW12_sum / test_LAW12_zero_hit_filtered
-- LAW-13  lookupRef 帶 vault 的 Ref 只認自己那個 vault              -> prop_LAW13
-- LAW-14  lookupRef 預設 vault                                     -> prop_LAW14
-- LAW-15  lookupRef vault 不在集合裡 -> Nothing                    -> prop_LAW15
-- LAW-16  checkReferences 完整且不多報                             -> test_LAW16
-- LAW-17  renderDanglingRef                                        -> test_LAW17
-- LAW-18  空集合退化                                                -> test_LAW18
-- LAW-19  單一 vault 退化成 F006\/F007                              -> test_LAW19
-- LAW-20  TooManyVaults\/VaultIdCollision 的 renderStoreError       -> prop_LAW20 / test_LAW20_examples
-- EX-1   searchAcross 一次回兩種 vault                             -> test_EX1
-- EX-2   listAcross 排序與分頁跨 vault                             -> test_EX2
-- EX-3   lookupRef 預設 vault 解析                                 -> test_EX3
-- EX-4   lookupRef 帶 vault 覆蓋預設 vault                         -> test_EX4
-- EX-5   lookupRef 目標 vault 不在集合裡                           -> test_EX5
-- EX-6   openVaultSet 第 11 個 vault -> TooManyVaults              -> test_EX6
-- EX-7   同路徑重複 -> 保序去重                                    -> test_EX7
-- EX-8   checkReferences 兩種懸空各一筆,不誤報                    -> test_EX8
-- EX-9   renderDanglingRef 兩則訊息                                -> test_EX9
-- EX-10  空集合下 *Across 全空、checkReferences 全 TargetVaultAbsent -> test_EX10
-- EX-11  單一 vault 退化                                           -> test_EX11
-- EX-12  facet 的 vault 維度、VaultSet 不接管把手生命週期          -> test_EX12
-- EX-13  異路徑撞號 -> VaultIdCollision                            -> test_EX13
-- EX-14  撞號優先於上限                                            -> test_EX14
-- EX-15  renderStoreError 的兩則訊息                               -> test_EX15
-- EX-16  nfIncludeReference 的兩處裸表名之一                       -> test_EX16
-- EX-17  nfTags 的兩處裸表名之二                                   -> test_EX17
-- @
--
...
--   __獨立__的檔案,__不是__ 'linkedTopicMd' 的片段(GAP-19:節層繼承對 @tags@ 是聯集
--   去重,同檔片段會連檔案層的 @"琳達"@ 標籤都繼承走)。
-- * __L12\/S3-like 共同 facet 值__:F-A 的 @ent-00000001@ 與 F-B 的 @ast-00000002@
--   都帶標籤 @"canon"@(見 'linkedTopicMd'\/'potionsPackMd'),求和與取其中一個才會
--   給出不同答案。
-- * __L4\/EX-16\/EX-17 的裸表名__:兩個 vault __各自都有__ reference pack('loreRefPackMd'\/
...
-- * __L12 零命中__:'test_LAW12_zero_hit_filtered' 用只有一個 vault 命中的查詢字，
--   斷言 @fcVaults@ 不含命中數 0 的那個 vault。
-- * __短 id 跨 vault 撞號(S3)__:F-A 與 F-B __都有__ @ent-00000001@(標題不同),
--   LAW-13\/LAW-5\/LAW-9 等一律用 @(VaultId, Id)@ 這一對驗證，抓得到「用 Id 當鍵」的合併。
--
-- __已知的最佳努力(best-effort)之處__:'searchTermForOrdering'\/LAW-9 的分數交錯
-- (S2 的分數版)依賴 SQLite FTS5 的 bm25 對詞頻\/文件長度的實際計算，qa 端只能
-- 透過刻意安排不同詞頻與文件長度來__提高__交錯機率，無法在骨架全 @undefined@
-- 的階段執行驗證；若之後仍證明是恆真（未交錯），留給 impl 完成後的仲裁處理，
-- 不影響本檔的紅\/綠判準（LAW-9 的斷言本身是「相鄰兩筆滿足排序關係」，不依賴任何
-- 事先假設的分數值)。
...
-- | F-A 的主題檔:__只有__主體 'ent-00000001'(標題含「藥水」,帶 S4 的三種關聯、
-- 專屬標籤 @"琳達"@)。__不__在這個檔案裡放片段——'design.md' 的節層繼承規則對
-- @tags@ 是聯集去重,同檔的片段會把檔案層(= 主體自己)的 @tags@ 也繼承走
-- (GAP-19:曾經讓 @nfTags=[琳達]@ 連片段一起回三筆)。'ent-00000005'\/'ent-00000007'
...
-- 'ent-00000005',標題__不__含「藥水」,只用來讓 LAW-5 的排序鍵交錯前提成立
...
-- | F-A 的 reference pack(@library\/reference\/@ 底下),供 LAW-4\/EX-16 用。
loreRefPackMd :: Text
loreRefPackMd =
  T.unlines
    [ "---"
...
-- 二字——GAP-19:pack 本身的標題\/正文會被 FTS 命中,曾讓 EX-1「恰兩筆」與 EX-12 的
-- facet 計數落空)+ 'ast-00000002'(標題逐字「魔法藥水瓶」,帶 S4\/LAW-4\/EX-17 用的
...
-- | F-B 的另一份主題檔:主體節點 id __逐字等於__ 'ent-00000001'(S3;短 id 跨
...
-- | F-B 的 reference pack,供 LAW-4\/EX-16 用。
...
-- | 對同一個路徑再 'openVault' 一次(EX-7:同路徑重複 → 保序去重),沿用已經
...
-- 的測試:LAW-2)。收尾時自動 'closeVault' 兩個把手。
...
-- 負責 'closeVault' 兩個把手(EX-12:測試本身就要驗證「呼叫端自己
...
-- 一個節點」的迷你 vault,供 EX-6\/EX-7\/EX-13\/EX-14\/LAW-1\/LAW-1b 這種只在意「id 撞不撞、
-- 數量夠不夠」而不需要豐富內容的測試使用。
...
-- | 與 'byIdThenVault' 相同的排序鍵,但作用在 @(VaultId, Id)@ 上(EX-17 用)。
...
--------------------------------------------------------------------------------
-- 建立、上限與生命週期(LAW-1 / LAW-1b / LAW-2 / LAW-3 / EX-6 / EX-7 / EX-13 / EX-14)

l1Spec :: Spec
l1Spec = describe "LAW-1: 去重與上限(maxAttachedVaults == 10)" $ do
...
l1bSpec :: Spec
l1bSpec = describe "LAW-1b: 撞號(VaultIdCollision),優先於上限" $ do
...
-- | 兩個__不同路徑__、__都非空索引__、vmId 都是 'vidA' 的把手(LAW-1b 的非退化前提:
...
l2Spec :: Spec
l2Spec = describe "LAW-2: closeVaultSet 之後,單一 vault 的讀取不受影響" $
...
e6Spec :: Spec
e6Spec = describe "EX-6: 第 11 個 vault -> TooManyVaults,10 個成功" $ do
...
e7Spec :: Spec
e7Spec = describe "EX-7: 同一個路徑重複傳 -> 保序去重" $
...
e13Spec :: Spec
e13Spec = describe "EX-13: 異路徑撞號 -> VaultIdCollision(不是 Right、也不是去重後的成功)" $
...
e14Spec :: Spec
e14Spec = describe "EX-14: 撞號優先於上限" $
...
--------------------------------------------------------------------------------
-- listAcross(LAW-4 / LAW-5 / LAW-6 / LAW-7 / EX-2 / EX-16 / EX-17)

-- | LAW-4 的 8 個維度各自的候選值(每個都在 F-A/F-B 的 fixture 裡__對預設值造成真實差異__)。
...
l4Spec :: Spec
l4Spec = describe "LAW-4: listAcross 的多重集合等於逐 vault listNodes 的多重集合" $ do
...
l5Spec :: Spec
l5Spec = describe "LAW-5: 全域排序(metaId 遞增,平手比 vault)" $
...
l6Spec :: Spec
l6Spec = describe "LAW-6: 分頁是對整體切窗" $
...
l7Spec :: Spec
l7Spec = describe "LAW-7: listAcross 每筆的 vault 欄與 metaVault 一致" $
...
e2Spec :: Spec
e2Spec = describe "EX-2: 排序與分頁跨 vault" $ do
...
e16Spec :: Spec
e16Spec = describe "EX-16: nfIncludeReference 的裸表名之一(packs 的 reference 子查詢)" $
...
e17Spec :: Spec
e17Spec = describe "EX-17: nfTags 的裸表名之二(node_tags 存在性子查詢)" $ do
...
--------------------------------------------------------------------------------
-- searchAcross(LAW-8 / LAW-9 / LAW-10 / LAW-11 / LAW-12 / EX-1 / EX-12)
...
l8Spec :: Spec
l8Spec = describe "LAW-8: searchAcross 逐 vault 等價(四欄逐欄相同)" $
...
l9Spec :: Spec
l9Spec = describe "LAW-9: searchAcross 全域排序(score 遞減,平手比 metaId,再平手比 vault)" $
...
l10Spec :: Spec
l10Spec = describe "LAW-10: searchAcross 分頁與 srTotal" $
...
l11Spec :: Spec
l11Spec = describe "LAW-11: searchAcross 每筆 vault 欄一致(檢索側)" $
...
l12Spec :: Spec
l12Spec = describe "LAW-12: facet" $ do
  ...
  it "facet 不因該維度自己的過濾條件改變(比照 F007 LAW-17)" $
    withDualVaultSet $ \(vs, _hA, _hB) -> do
...
e1Spec :: Spec
e1Spec = describe "EX-1: searchAcross 一次回兩種 vault" $
...
e12Spec :: Spec
e12Spec = describe "EX-12: facet 的 vault 維度跨 vault;VaultSet 不接管把手的生命週期" $
...
--------------------------------------------------------------------------------
-- lookupRef(LAW-13 / LAW-14 / LAW-15 / EX-3 / EX-4 / EX-5)

l13Spec :: Spec
l13Spec = describe "LAW-13: 帶 vault 的 Ref 只認自己那個 vault" $
...
l14Spec :: Spec
l14Spec = describe "LAW-14: 預設 vault" $
...
e4Spec :: Spec
e4Spec = describe "EX-4: 帶 vault 的 Ref 覆蓋預設 vault" $
...
e5Spec :: Spec
e5Spec = describe "EX-5: 目標 vault 不在集合裡" $
...
--------------------------------------------------------------------------------
-- checkReferences(LAW-16 / LAW-17 / EX-8 / EX-9)

-- | LAW-16 公式的直接翻譯:用 'loadLinkGraph' + 'lookupRef' + 'vaultSetIds' 重新
...
l16Spec :: Spec
l16Spec = describe "LAW-16: checkReferences 完整且不多報" $
...
-- | 判準與 F008 LAW-15 相同:以 @;@\/@；@\/@,@\/@，@\/@。@ 切子句,去頭尾空白後
-- 比對開頭是不是「請」。
...
e8Spec :: Spec
e8Spec = describe "EX-8: 兩種懸空都找得到,解析得到的那一筆不會被誤報" $
...
e9Spec :: Spec
e9Spec = describe "EX-9: renderDanglingRef 兩則訊息" $
...
--------------------------------------------------------------------------------
-- 退化情形(LAW-18 / LAW-19 / EX-10 / EX-11)

l18Spec :: Spec
l18Spec = describe "LAW-18: 空集合" $
...
l19Spec :: Spec
l19Spec = describe "LAW-19: 單一 vault 退化成 F006/F007" $
...
e10Spec :: Spec
e10Spec = describe "EX-10: 空集合不是錯誤,空集合下 checkReferences 對每一筆關聯都回 TargetVaultAbsent" $
...
--------------------------------------------------------------------------------
-- StoreError 新增建構子(LAW-20 / EX-15)

l20Spec :: Spec
l20Spec = describe "LAW-20: TooManyVaults / VaultIdCollision 的 renderStoreError" $
...
e15Spec :: Spec
e15Spec = describe "EX-15: renderStoreError 的兩則訊息" $
