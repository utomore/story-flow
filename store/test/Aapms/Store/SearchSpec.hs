...
-- LAW-13 有文字條件時每筆命中 shScore > 0                            -> test_LAW13
-- LAW-14 命中 id 相異、分數非遞增、同分 id 遞增、重複查詢逐筆相同    -> test_LAW14
-- LAW-15 srTotal 與分頁無關,>= 命中數;命中數 <= nfLimit             -> test_LAW15
-- LAW-19 每筆 shVault 等於本 vault 的 vmId                            -> test_LAW19
-- LAW-20 對同一檔案連續 indexFile 兩次,search 結果與一次相同         -> test_LAW20
-- LAW-21 unindexFile 之後,該檔案節點不再出現在任何 search 結果       -> test_LAW21
-- LAW-22 schema_version 改壞後 openVault + rebuildIndex 結果不變      -> test_LAW22
-- LAW-24 純 ASCII 查詢,search t 與 search (toUpper t) 結果相同        -> test_LAW24
-- EX-6  二字中文"藥水"命中含"魔法藥水瓶"的節點,snippet 含"藥水"     -> test_EX6
-- EX-7  二字中文"琳達"命中同名角色主體                               -> test_EX7
-- EX-8  英文子字串"travel-book"命中 name 含該字串的 asset            -> test_EX8
-- EX-9  三字以上中文"魔法藥水"走 trigram 並有分數                    -> test_EX9
-- EX-10 標題同含"藥水"與 potion,查詢"藥水 potion" 只回一筆           -> test_EX10
-- EX-12 emptySearchQuery 退化成純結構查詢                            -> test_EX12
-- EX-13 查無此文字時空結果、不是錯誤                                 -> test_EX13
-- EX-14 純 ASCII 二字查詢"ui"因雙索引已知代價回空結果                -> test_EX14
-- @
...
spec = describe "graph-core/F007 search" $ do
  describe "LAW-12-LAW-15/LAW-19/LAW-24: search 的通用行為(property)" $ do
    ...
    around withIndexedAssetVault $
      it "LAW-12: 無文字條件(Nothing 或去頭尾空白後為空字串)時退化成 listNodes,分數與片段皆為預設值" $
        \vh -> hedgehog $ do
    ...
    around withFtsVault $ do
      ...
      it "LAW-15: srTotal 與分頁無關,srTotal >= 命中數,命中數 <= nfLimit" $
        \vh -> hedgehog $ do
          q <- forAll genQueryCandidate
  ...
  describe "LAW-20-LAW-22: 索引維護與 search 的一致性" $ do
    ...
    it "LAW-22: schema_version 被改壞後重開 + rebuildIndex,search 結果不變且回報一筆 SchemaRebuilt" $
...
-- | LAW-22:openVault 回報的 'IndexIssue' 是否含一筆 schema 重建紀錄。
isSchemaRebuilt :: IndexIssue -> Bool
isSchemaRebuilt (SchemaRebuilt _ _) = True
isSchemaRebuilt _ = False

--------------------------------------------------------------------------------
-- 自訂 fixture:二字中文\/三字以上中文\/中英混合\/英文子字串(EX-6~EX-10、EX-13、EX-14)。
--
...
-- | 專供全文檢索 Examples(EX-6~EX-10、EX-13、EX-14)用的 vault:混合角色與 asset-pack
