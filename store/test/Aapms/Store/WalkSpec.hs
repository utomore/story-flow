...
--
-- @
-- R3  vaultMarkdownFiles:略過 . 開頭目錄與非 .md 檔,回排序後的相對路徑,行為與搬移前相同 -> prop_R3
-- R4  statOf:現存檔案的 size 分量 = 實際位元組數;同一未變動檔案重複讀取結果相同        -> prop_R4_size / prop_R4_stable
-- E3  vault 目錄含 .aapms\/config.toml、.git\/HEAD、foo.txt、bar.md -> ["bar.md"]        -> test_E3(= 搬移前 IndexSpec T3)
-- E4  完全空的 vault 目錄 -> []                                                          -> test_E4
-- E5  不存在的檔案路徑 -> statOf 回 Left,不拋例外                                        -> test_E5
-- @
--
-- __禁止讀實作__:本檔只 import "Aapms.Store.Walk" 的骨架簽名,不讀
-- "Aapms.Store.Index" 現有的函式本體;R3\/R4 的產生器範圍刻意只涵蓋 spec 與既有
-- E3\/E4 example 已經寫明的邊界(根層檔案 + 整個 . 開頭子目錄略過),不假設是否遞迴
...
spec = describe "graph-core/E001 Aapms.Store.Walk" $ do
  describe "R3 / E3 / E4: vaultMarkdownFiles" $ do
    it "E3: 略過 . 開頭目錄與非 .md 檔,只回排序後的 .md 相對路徑(= 搬移前 IndexSpec T3)" $
    ...
    it "E4: 完全空的 vault 目錄回 []" $
      withTempVault $ \dir -> do
        files <- vaultMarkdownFiles dir
        files `shouldBe` []

    it "R3: 任意根層檔案與 . 開頭子目錄組合,回傳值 = 排序後、根層 .md 檔名的集合" $
      hedgehog $ do
  ...
  describe "R4 / E5: statOf" $ do
    it "E5: 不存在的路徑回 Left 的 StoreError,不拋例外" $
      withTempVault $ \dir -> do
    ...
    it "R4: 現存檔案 statOf 結果的第二個分量(size)= 檔案的實際位元組數" $
      hedgehog $ do
    ...
    it "E6: 內容恰為 5 個位元組的檔,statOf 回 Right (m, 5)——第二個分量是 5" $
      withTempVault $ \dir -> do
    ...
    it "R4: 同一個未變動的檔案重複 statOf 兩次,結果相同" $
      hedgehog $ do
