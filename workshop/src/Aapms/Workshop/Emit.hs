...
--
-- __不直接碰 @aapms-store@__:寫入走與 CLI 相同的 'ServiceM' 操作
...
import Aapms.Core.Meta (Source (..), Status (Draft), Timeline (..), emptyTimeline)
...
import Aapms.Workshop.Session (Session (..), StageDraft (..), saveSession)
...
resolveOwner session = case wsOwner session of
  ...
  Nothing -> do
    ...
    case wsPending session of
      [] ->
        error
          "Aapms.Workshop.Emit.resolveOwner: wsPending 是空的,\
