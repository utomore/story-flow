...
-- 各階段的定案草稿——以及它的可序列化快照。
--
-- __狀態只在這裡變動__:'Aapms.Workshop.Stages' 讀寫 'Session' 都經本模組的
-- 'saveSession' \/ 'loadSession',不自己組快照的位元組。
module Aapms.Workshop.Session
  ( -- * 型別
    Session (..)
  , StageDraft (..)
...
-- | 一個還沒寫進圖譜的片段草稿,由 'Aapms.Workshop.Stages.stepWorkshop' 從
...
-- @.Json@ 模組——與 aapms-conflict 的 @Verdict@ 同一個先例。
...
-- | 'Aapms.Llm.Client' 刻意不定義 'Message' \/ 'Role' 的 aeson 實例(它們
-- 是門面之外的內部細節),所以這裡手動轉成 @{"role":…,"content":…}@,與
-- 'Aapms.Llm.Client' 內部的 @roleWire@ 同樣的字串但各自一份——不對
-- 'aapms-llm' 的公開型別加孤兒實例。
...
-- | 路徑字串用字面組,不 import @aapms-store@ 的 @storyflowDir@——本套件的
-- build-depends 裡沒有 @aapms-store@,與 @llm/test/.../Fixtures.hs@ 的
-- @appendConfig@ 同一個理由。
...
-- 同構於 @Aapms.Store.Atomic.atomicWriteText@ 的邏輯,而不是呼叫它:
-- 本套件的相依清單擋著 @aapms-store@,兩份實作各自完整、互不依賴。
...
-- 碰撞重試。__不重用 'Aapms.Core.Id.mkId'__:它的 'Aapms.Core.Id.IdPrefix'
-- 只認得四個封閉前綴('Aapms.Core.Id.PEnt' \/ 'Aapms.Core.Id.PLvl' \/
-- 'Aapms.Core.Id.PNod' \/ 'Aapms.Core.Id.PVlt'),工作坊 session 不是
-- 其中之一,只重用它有匯出的雜湊原語 'fnv1a64'。
--
-- 碰撞以「快照檔是否已存在」判斷;重試到保守上限後仍碰撞,理論上不會發生
-- (雜湊空間 32 位元、salt 隨時間與內容變動),直接採用最後一個候選 id——
-- 與 core 的 'Aapms.Core.Id.mkId' 同一個立場:「唯一性不在雜湊這一層」,
