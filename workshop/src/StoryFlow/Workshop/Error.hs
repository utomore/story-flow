...
-- 'StoryFlow.Service' 的門面註解明寫著「明確不做的:conflict(P4)、
-- workshop(P5)、LLM」。與 'StoryFlow.Llm.Error.LlmError' 是同一種形狀
-- ——下層的錯誤型別不認識上層,呈現一律交給介面層(workshop-interface)。
module StoryFlow.Workshop.Error
  ( WorkshopError (..)
  , renderWorkshopError
  , workshopErrorCode
  ) where
...
-- 不必再改這個模組。
data WorkshopError
  = -- | 'StoryFlow.Workshop.Session.loadSession' 找不到這個 session id 的快照檔
    WsSessionNotFound Text
  | -- | 快照檔在,但不是合法的 'StoryFlow.Workshop.Session.Session' JSON
    WsSnapshotCorrupt FilePath Text
  | -- | 寫快照時的 IO 失敗(磁碟滿、權限…)
    WsSnapshotWriteFailed FilePath Text
  | -- | 型別沒有宣告任何 stages('StoryFlow.Workshop.Stages.startWorkshop')
    WsNoStages Text
  | -- | session 已經沒有下一階段('StoryFlow.Workshop.Stages.stepWorkshop')
...
-- | 繁中訊息,每一則都說出下一步。形狀比照 'StoryFlow.Service.Error.renderServiceError'。
...
-- 再同步一次。'WsLlmFailed' 往內取 'llmErrorCode',與 'StoryFlow.Service.Error.errorCode'
