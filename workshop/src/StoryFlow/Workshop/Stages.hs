...
module StoryFlow.Workshop.Stages
  ( startWorkshop
  , stepWorkshop
    ...
    -- @storyflow-conflict@ 的 "StoryFlow.Conflict.Judge" 同一個先例
...
import StoryFlow.Llm (LlmClient, Message (..), Role (..), chat)
import StoryFlow.Service (EntityView (..), ServiceError (..), ServiceM, getEntity, listEntityTypes)
import StoryFlow.Workshop.Error (WorkshopError (..))
import StoryFlow.Workshop.Session (Session (..), StageDraft, newSessionId, saveSession)
...
-- (獨立實作,本套件不依賴 @storyflow-conflict@,只重用做法):
--
