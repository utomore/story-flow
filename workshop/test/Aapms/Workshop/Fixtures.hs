...
-- 與 @llm\/test\/Aapms\/Llm\/Fixtures.hs@ 同一個理由。
--
-- __臨時 Vault 只靠 @aapms-service@ 的門面__:@createVault@ \/ @openEnv@ \/
-- @runService@ 都在門面上,@aapms-store@ 一次都不必露臉。
module Aapms.Workshop.Fixtures
  ( -- * stub 端點
    Stub (..)
  , okStub
  , chatCompletion
...
-- | 一個保證沒有人在聽的埠,與 @llm\/test\/Aapms\/Llm\/Fixtures.hs@ 同一個
...
withWorkshopVaultDir act =
  withSystemTempDirectory "aapms-workshop-vault" $ \dir ->
...
withCustomRegistryDir tomlContent act =
  withSystemTempDirectory "aapms-workshop-registry" $ \regDir -> do
