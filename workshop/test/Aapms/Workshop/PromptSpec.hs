-- | T9:'Aapms.Workshop.Stages.buildMessages'(純函式的組裝規則,以及
-- 「改 TOML 就改得動」這條驗收標準的直接證明)。
module Aapms.Workshop.PromptSpec (spec) where
...
import Aapms.Service (Env, listEntityTypes)
import Aapms.Workshop.Fixtures (runS, withCustomVault)
import Aapms.Workshop.Session (Session (..))
import Aapms.Workshop.Stages (buildMessages)
import Test.Hspec
