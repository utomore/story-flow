-- | T7:'StoryFlow.Workshop.Session' 的 session id 產生。
module StoryFlow.Workshop.SessionIdSpec (spec) where
...
import StoryFlow.Service (vaultRoot)
import StoryFlow.Workshop.Fixtures (runS, withWorkshopVault)
import StoryFlow.Workshop.Session (newSessionId, newSessionIdAt, sessionIdCandidate)
