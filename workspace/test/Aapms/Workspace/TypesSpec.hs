-- | F001:'Aapms.Workspace.Types' 的 'renderWorkspaceError'(LAW-14\/LAW-15\/EX-21–EX-26)、
...
-- LAW-15  renderWorkspaceError 含逐條列出的攜帶值                          -> carriedValues 迴圈
-- LAW-16  mkHub 與五個 selector 互逆(預期綠)                              -> prop_LAW16
-- LAW-17(a) Types.hs 不 import 本套件其他模組                              -> test_types_imports_no_sibling_module
-- LAW-17(b) Location.hs 不 import Aapms.Workspace.Hub                      -> test_location_does_not_import_hub
-- LAW-17(c) Location.hs/Hub.hs 完全不得 import Aapms.Store.Marker          -> test_location_and_hub_never_import_marker
-- LAW-17(d) Types.hs 對 Aapms.Store.Marker 的 import 逐字只拿 VaultMarker  -> test_types_imports_marker_type_only
-- LAW-17(e) 三檔都不 import openIndexAt/closeIndex/System.Process         -> test_no_index_or_process_imports
-- EX-21–EX-26 renderWorkspaceError 的具體例子                               -> it "EX-21".."EX-26"
--
-- 2026-08-29 閘門裁決:原 LAW-17(c)「三個檔案都不 import Aapms.Store.Marker」與契約 C
...
-- | 契約 F 全部 21 個建構子的代表值(EX-21\/EX-23\/EX-24\/EX-25\/EX-26 用 spec 給的具體值;
-- 其餘用同一種風格自己造;'WriteTargetIdDrift' 是 2026-08-29 WAVE-3 閘門新增,
-- 'ProjectSelectorAmbiguous' \/ 'ProjectAlreadyRegistered' \/ 'VaultInitFailed' \/
-- 'DeleteTargetIdDrift' 是同一天 WAVE-4 閘門新增)。
...
allConstructors =
  ...
  , ("WriteTargetIdDrift", WriteTargetIdDrift (VaultId "vlt-7f3b2a91") "D:/vaults/liftgame" (VaultId "vlt-a0c4e1f8"))
...
carriedValues = \case
  ...
  WriteTargetIdDrift regVid path markerVid -> [vaultIdText regVid, T.pack path, vaultIdText markerVid]
...
spec = describe "F001 Aapms.Workspace.Types" $ do
  describe "LAW-14: renderWorkspaceError 全建構子非空、含中文、可行動、不含 show 痕跡" $
    mapM_
  ...
  describe "LAW-15: renderWorkspaceError 含「訊息規格」表逐條列出的攜帶值" $
    mapM_
  ...
  describe "Examples EX-21-EX-26" $ do
    it "EX-21: HubNotFound 訊息非空繁中、含路徑、含下一步指示" $ do
    ...
    it "EX-22: VaultSelectorAmbiguous 含 selector 字串與兩列各自的 veId/vePath" $ do
      let e1 = sampleVault1
          e2 = sampleVault2
    ...
    it "EX-23: VaultKindMismatch 含 id、asset、story,不含 Haskell 建構子名" $ do
    ...
    it "EX-24: VaultIdCollision 三個值都在" $ do
    ...
    it "EX-25: InvalidName 含以「」夾住的原始字串(全空白也驗得出來)" $
      renderWorkspaceError (InvalidName "   ") `shouldSatisfy` T.isInfixOf "「   」"

    it "EX-26: MarkerUnreadable 含 root 與 renderStoreError 對該 StoreError 的輸出" $ do
  ...
  describe "LAW-16(預期綠): mkHub 與五個 selector 互逆" $
  ...
  -- 2026-08-29 閘門裁決(本次-1 已由開發者裁決、spec 改寫):LAW-17 從 3 條子斷言改成
  ...
  describe "LAW-17(預期綠): 依賴方向與職責界線,以 import 清單驗證" $ do
...
-- LAW-17 五條子斷言 (a)-(e) 全部經由本函式讀 import 行,行尾正規化只做這一處。
